...
const cr::TimeDelta kUpdateOcclusionDelay =
    cr::TimeDelta::FromMilliseconds(16);
...
void NativeWindowOcclusionTrackerWin::Enable(Window* window) {
  ...
  update_occlusion_task_runner_->PostTask(
      ...
      cr::BindOnce(
          ...
          cr::Unretained(WindowOcclusionCalculator::GetInstance()),
          root_window_hwnd));
}
...
void NativeWindowOcclusionTrackerWin::OnWindowVisibilityChanged(Window* window,
  ...
  update_occlusion_task_runner_->PostTask(
      CR_FROM_HERE,
      cr::BindOnce(&WindowOcclusionCalculator::HandleVisibilityChanged,
                   cr::Unretained(WindowOcclusionCalculator::GetInstance()),
                   visible));
}
...
NativeWindowOcclusionTrackerWin::NativeWindowOcclusionTrackerWin()
    :  // Use a COMSTATaskRunner so that registering and unregistering
       ...
       // EventHookCallback.
      ///update_occlusion_task_runner_(cr::CreateCOMSTATaskRunner(
      ///    {cr::ThreadPool(), cr::MayBlock(),
      ///     // This may be needed to determine that a window is no longer
      ///     // occluded.
      ///     cr::TaskPriority::USER_VISIBLE,
      ///     // Occlusion calculation doesn't need to happen on shutdown.
      ///     // event hooks should also be cleaned up by Windows.
      ///     cr::TaskShutdownBehavior::SKIP_ON_SHUTDOWN})),
      update_occlusion_task_runner_(cr::WorkerPool::GetTaskRunner(false)),
      session_change_observer_(
          cr::BindRepeating(&NativeWindowOcclusionTrackerWin::OnSessionChange,
                            cr::Unretained(this))) {
  WindowOcclusionCalculator::CreateInstance(
      update_occlusion_task_runner_, cr::SequencedTaskRunnerHandle::Get(),
      cr::BindRepeating(
...
NativeWindowOcclusionTrackerWin::~NativeWindowOcclusionTrackerWin() {
  ...
  update_occlusion_task_runner_->PostTask(
      CR_FROM_HERE,
      cr::BindOnce(&WindowOcclusionCalculator::DeleteInstanceForTesting,
                   &done_event));
  done_event.Wait();
}
...
bool NativeWindowOcclusionTrackerWin::IsWindowVisibleAndFullyOpaque(
  ...
  crui::win::ScopedRegion region(CreateRectRgn(0, 0, 0, 0));
...
void NativeWindowOcclusionTrackerWin::UpdateOcclusionState(
    const cr::flat_map<HWND, Window::OcclusionState>&
...
NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
    WindowOcclusionCalculator(
        cr::RefPtr<cr::SequencedTaskRunner> task_runner,
        cr::RefPtr<cr::SequencedTaskRunner> ui_thread_task_runner,
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
    DeleteInstanceForTesting(cr::WaitableEvent* done_event) {
  CR_DCHECK(instance_);
  delete instance_;
  instance_ = nullptr;
  done_event->Signal();
...
BOOL CALLBACK NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
  ...
  if (instance_) {
    return instance_->ProcessComputeNativeWindowOcclusionStatusCallback(
        hwnd, reinterpret_cast<cr::flat_set<DWORD>*>(lParam));
  }
  return FALSE;
}
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
  ...
  if (should_unregister_event_hooks) {
    ...
    cr::flat_set<DWORD> current_pids_with_visible_windows;
    ...
    cr::flat_set<DWORD> pids_to_remove;
    ...
    if (!pids_to_remove.empty()) {
      ...
      cr::EraseIf(pids_for_location_change_hook_,
  ...
  ui_thread_task_runner_->PostTask(
      CR_FROM_HERE, cr::BindOnce(update_occlusion_state_callback_,
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
  ...
  if (!occlusion_update_timer_.IsRunning())
    occlusion_update_timer_.Start(
        CR_FROM_HERE, kUpdateOcclusionDelay, this,
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
    RegisterEventHooks() {
  CR_DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  CR_DCHECK(global_event_hooks_.empty());
...
bool NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::
  ...
  if (window_is_occluding) {
    ...
    if (!cr::Contains(process_event_hooks_, pid))
      RegisterEventHookForProcess(pid);
...
cr::Optional<bool> NativeWindowOcclusionTrackerWin::
