...
class CursorState {
 public:
  ...
  CursorState()
      : cursor_(crui::CursorType::kNone),
        visible_(true),
        cursor_size_(crui::CursorSize::kNormal),
  ...
  crui::CursorSize cursor_size() const { return cursor_size_; }
  void set_cursor_size(crui::CursorSize cursor_size) {
...
void CursorManager::ShowCursor() {
  ...
  if (cursor_lock_count_ == 0 &&
    ...
    if (GetCursor().native_type() != crui::CursorType::kNone) {
...
void CursorManager::SetCursorSize(crui::CursorSize cursor_size) {
...
crui::CursorSize CursorManager::GetCursorSize() const {
...
void CursorManager::UnlockCursor() {
  cursor_lock_count_--;
  CR_DCHECK(cursor_lock_count_ >= 0);
  if (cursor_lock_count_ > 0)
    return;
...
bool CursorManager::ShouldHideCursorOnKeyEvent(
    const crui::KeyEvent& event) const {
  return false;
}
...
void CursorManager::CommitVisibility(bool visible) {
  ...
  for (auto& observer : observers_) {
    observer.OnCursorVisibilityChanged(
        GetCursor().native_type() == crui::CursorType::kNone ? false : visible);
  }
  current_state_->SetVisible(visible);
}

void CursorManager::CommitCursorSize(crui::CursorSize cursor_size) {
