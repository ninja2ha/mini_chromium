...
template <typename T>
using WindowProperty = crui::ClassProperty<T>;
...
class CRUI_EXPORT Window : public crui::LayerDelegate,
 ...
 public:
  ...
  const cr::string16& GetTitle() const;
  void SetTitle(const cr::string16& title);
  ...
  ///const gfx::Region& occluded_region() const { return occluded_region_; }
  ...
  void SetLayoutManager(WindowLayoutManager* layout_manager);
  WindowLayoutManager* layout_manager() { return layout_manager_.get(); }
  ...
  ///    cr::OnceCallback<void()> allocation_task);
  ...
  ///    const cr::Optional<viz::LocalSurfaceIdAllocation>&
 ...
 private:
  ...
  ///cr::ReentrantObserverList<WindowObserver, true> observers_;
  cr::ObserverList<WindowObserver, true> observers_;
  ...
  ///cr::WeakPtr<cc::LayerTreeFrameSink> frame_sink_;
};
