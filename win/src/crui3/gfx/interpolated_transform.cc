...
// either 0, 1 or -1.
bool MassageRotationIfMultipleOfNinetyDegrees(crui::gfx::Transform* rotation,
                                              float degrees) {
  ...
  int n = crui::gfx::ToRoundedInt(degrees_by_ninety);
  ...
  if (n == 0) {
    ...
    *rotation = crui::gfx::Transform::Make90degRotation();
  } else if (n == 2) {
    *rotation = crui::gfx::Transform::Make180degRotation();
  } else if (n == 3) {
    *rotation = crui::gfx::Transform::Make270degRotation();
  }
  return true;
}
...
inline float InterpolatedTransform::ValueBetween(float time,
  ...
  // can't handle NaN
  CR_DCHECK(time == time && start_time_ == start_time_ 
            && end_time_ == end_time_);
  ...
  CR_DCHECK(end_time_ >= start_time_);
...
void InterpolatedTransformAboutPivot::Init(
  ...
  to_pivot.Translate(static_cast<float>(-pivot.x()), 
                     static_cast<float>(-pivot.y()));
  from_pivot.Translate(static_cast<float>(pivot.x()), 
                       static_cast<float>(pivot.y()));
