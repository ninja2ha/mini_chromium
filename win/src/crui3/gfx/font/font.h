...
class CRUI_EXPORT Font {
 ...
 private:
  ...
  cr::RefPtr<PlatformFont> platform_font_;
};

#ifndef NDEBUG
CRUI_EXPORT std::ostream& operator<<(std::ostream& stream,
                                     const Font::Weight weight);
#endif

// Returns the Font::Weight that matches |weight| or the next bigger one.
CRUI_EXPORT Font::Weight FontWeightFromInt(int weight);
