...
class RTree : public RTreeBase {
 public:
  typedef std::unordered_set<Key> Matches;
 ...
 private:
  ...
  typedef std::unordered_map<Key, Record*> RecordMap;
...
void RTree<Key>::Insert(const Rect& rect, Key key) {
  std::unique_ptr<NodeBase> record;
  ...
  InsertNode(std::move(record), &highest_reinsert_level);
}
