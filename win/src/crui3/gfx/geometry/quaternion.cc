...
Quaternion::Quaternion(const Vector3dF& axis, double theta) {
  ...
  normalized.Scale(static_cast<float>(1.0 / length));
...
Quaternion Quaternion::Slerp(const Quaternion& q, double t) const {
  ...
  dot = cr::ClampToRange(dot, -1.0, 1.0);
...
std::string Quaternion::ToString() const {
  ...
  float abs_theta = static_cast<float>(acos(w_) * 2);
  float scale = static_cast<float>(1.0 / sin(abs_theta * 0.5));
  gfx::Vector3dF v(static_cast<float>(x_), 
                   static_cast<float>(y_), 
                   static_cast<float>(z_));
  v.Scale(scale);
  return cr::StringPrintf("[%f %f %f %f], v:", x_, y_, z_, w_) +
         v.ToString() +
         cr::StringPrintf(", θ:%fπ", abs_theta / cr::kPiFloat);
}
