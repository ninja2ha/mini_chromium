...
class CRUI_EXPORT Insets {
 public:
  ...
  explicit Insets(int vertical, int horizontal)
      : top_(vertical),
        left_(horizontal),
        bottom_(GetClampedValue(vertical, vertical)),
        right_(GetClampedValue(horizontal, horizontal)) {}
  explicit Insets(int top, int left, int bottom, int right)
      : top_(top),
        left_(left),
  ...
  void operator+=(const Insets& insets) {
    top_ = cr::ClampAdd(top_, insets.top_);
    left_ = cr::ClampAdd(left_, insets.left_);
    bottom_ = GetClampedValue(top_, cr::ClampAdd(bottom_, insets.bottom_));
    right_ = GetClampedValue(left_, cr::ClampAdd(right_, insets.right_));
  }

  void operator-=(const Insets& insets) {
    top_ = cr::ClampSub(top_, insets.top_);
    left_ = cr::ClampSub(left_, insets.left_);
    bottom_ = GetClampedValue(top_, cr::ClampSub(bottom_, insets.bottom_));
    right_ = GetClampedValue(left_, cr::ClampSub(right_, insets.right_));
  }

  Insets operator-() const {
    return Insets(-cr::MakeClampedNum(top_), -cr::MakeClampedNum(left_),
                  -cr::MakeClampedNum(bottom_),
                  -cr::MakeClampedNum(right_));
  }

  Insets Scale(float scale) const { return Scale(scale, scale); }

  Insets Scale(float x_scale, float y_scale) const {
    return Insets(static_cast<int>(cr::ClampMul(top(), y_scale)),
                  static_cast<int>(cr::ClampMul(left(), x_scale)),
                  static_cast<int>(cr::ClampMul(bottom(), y_scale)),
                  static_cast<int>(cr::ClampMul(right(), x_scale)));
  }
 ...
 private:
  ...
  static bool AddWouldOverflow(int a, int b) {
  ...
  static bool AddWouldUnderflow(int a, int b) {
  ...
  // bottom_or_right.
  // TODO(enne): this should probably use cr::ClampAdd, but that
  // function is not a constexpr.
  static int GetClampedValue(int top_or_left, int bottom_or_right) {
    // Warning: vs 2015 not support if (constexpr function())
