...
struct FloatGeometrySaturationHandler {
  ...
  static constexpr float max() {
    return std::numeric_limits<float>::max() / (float)1e6;
  }
  static constexpr float lowest() {
    return std::numeric_limits<float>::lowest() / (float)1e6;
  }
};
...
constexpr float ClampFloatGeometry(T value) {
  return cr::saturated_cast<float, FloatGeometrySaturationHandler, T>(value);
}
