...
#include "crui/aura/client/capture_client_observer.h"
...
bool CompareWeakPtrs(const cr::WeakPtr<DesktopCaptureClient>& lhs,
                     const cr::WeakPtr<DesktopCaptureClient>& rhs) {
...
DesktopCaptureClient::~DesktopCaptureClient() {
  ...
  cr::EraseIf(*clients_, [this](const auto& c) { return c.get() == this; });
}
...
void DesktopCaptureClient::SetCapture(aura::Window* new_capture_window) {
  ...
  CR_DCHECK(!new_capture_window ||
            (new_capture_window->GetRootWindow() == root_));
  CR_DCHECK(!capture_window_ || capture_window_->GetRootWindow());
