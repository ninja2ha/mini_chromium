...
#include "crui/base/accelerators/accelerator_manager.h"
...
class CRUI_EXPORT FocusTraversable {
 public:
...
class CRUI_EXPORT FocusChangeListener {
 public:
...
class CRUI_EXPORT FocusManager : public ViewObserver {
 public:
  ...
  // further.
  bool OnKeyEvent(const crui::KeyEvent& event);
  ...
  void RegisterAccelerator(const crui::Accelerator& accelerator,
                           crui::AcceleratorManager::HandlerPriority priority,
                           crui::AcceleratorTarget* target);

  // Unregister the specified keyboard accelerator for the specified target.
  void UnregisterAccelerator(const crui::Accelerator& accelerator,
                             crui::AcceleratorTarget* target);

  // Unregister all keyboard accelerator for the specified target.
  void UnregisterAccelerators(crui::AcceleratorTarget* target);
  ...
  bool ProcessAccelerator(const crui::Accelerator& accelerator);

  // Resets menu key state if |event| is not menu key release.
  // This is effective only on x11.
  void MaybeResetMenuKeyState(const crui::KeyEvent& key);
  ...
  bool HasPriorityHandler(const crui::Accelerator& accelerator) const;
  ...
  // pressed).
  static bool IsTabTraversalKeyEvent(const crui::KeyEvent& key_event);
 ...
 private:
  ...
  bool ProcessArrowKeyTraversal(const crui::KeyEvent& event);
  ...
  crui::AcceleratorManager accelerator_manager_;
  ...
  ///cr::ObserverList<FocusChangeListener, true>::Unchecked
  cr::ObserverList<FocusChangeListener, true>
