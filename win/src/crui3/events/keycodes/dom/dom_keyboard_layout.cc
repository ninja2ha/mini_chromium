...
const size_t kWritingSystemKeyDomCodeEntries =
    cr::size(writing_system_key_domcodes);
...
cr::flat_map<std::string, std::string> DomKeyboardLayout::GetMap() {
  auto dom_map = cr::flat_map<std::string, std::string>();
  for (size_t i = 0; i < kWritingSystemKeyDomCodeEntries; ++i) {
    crui::DomCode dom_code = crui::writing_system_key_domcodes[i];
    ...
    if (unicode >= 0x0300 && unicode <= 0x036f) {
      ...
      for (size_t j = 0; j < cr::size(kCombiningKeyMapping); ++j) {
