...
class GestureProvider::GestureListenerImpl : public ScaleGestureListener,
 ...
 public:
  ...
  void OnTouchEvent(const MotionEvent& event) {
    ...
    if (action == MotionEvent::Action::DOWN) {
      ...
      current_longpress_time_ = cr::TimeTicks();
      ignore_single_tap_ = false;
      scroll_event_sent_ = false;
      pinch_event_sent_ = false;
      show_press_event_sent_ = false;
    ...
    if (action == MotionEvent::Action::UP ||
      ...
      if (action != MotionEvent::Action::CANCEL ||
        ...
        current_down_action_event_time_ = cr::TimeTicks();
  ...
  void Send(GestureEventData gesture) {
    CR_DCHECK(!gesture.time.is_null());
    ...
    CR_DCHECK(!current_down_action_event_time_.is_null() ||
    ...
    switch (gesture.type()) {
      case ET_GESTURE_LONG_PRESS:
        CR_DCHECK(!IsScaleGestureDetectionInProgress());
  ...
  void OnLongPress(const MotionEvent& e) override {
    CR_DCHECK(!IsDoubleTapInProgress());
    SetIgnoreSingleTap(true);
 ...
 private:
  ...
  gfx::Vector2dF ComputeFirstScrollDelta(
    ...
    CR_DCHECK(ev2.GetPointerCount() < 3);
    ...
    delta.Scale(static_cast<float>(1.0 / ev2.GetPointerCount()));
    return delta;
  }
  ...
  // sequence.
  cr::TimeTicks current_down_action_event_time_;
  ...
  cr::TimeTicks current_longpress_time_;
...
GestureProvider::GestureProvider(const Config& config,
  ...
  CR_DCHECK(client);
  CR_DCHECK(!config.min_gesture_bounds_length ||
...
bool GestureProvider::OnTouchEvent(const MotionEvent& event) {
  ...
  CR_DCHECK(0u != event.GetPointerCount());
...
void GestureProvider::ResetDetection() {
  MotionEventGeneric generic_cancel_event(
      MotionEvent::Action::CANCEL, cr::TimeTicks::Now(), PointerProperties());
