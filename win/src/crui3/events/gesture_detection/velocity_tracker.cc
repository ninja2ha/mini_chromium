...
class VelocityTrackerStrategy {
 public:
  ...
  virtual void AddMovement(const cr::TimeTicks& event_time,
                           BitSet32 id_bits,
...
VelocityTrackerStrategy* CreateStrategy(VelocityTracker::Strategy strategy) {
  ...
  CR_NOTREACHED() << "Unrecognized velocity tracker strategy: " << strategy;
...
void VelocityTracker::AddMovement(const TimeTicks& event_time,
  ...
  if ((current_pointer_id_bits_.value & id_bits.value) &&
      (event_time - last_event_time_) >=
          cr::TimeDelta::FromMilliseconds(kAssumePointerMoveStoppedTimeMs)) {
  ...
  switch (event.GetAction()) {
    ...
    case MotionEvent::Action::POINTER_UP:
      ...
      if ((event.GetEventTime() - last_event_time_) >=
          cr::TimeDelta::FromMilliseconds(kAssumePointerUpStoppedTimeMs))
        strategy_->Clear();
      return;
    default:
...
LeastSquaresVelocityTrackerStrategy::LeastSquaresVelocityTrackerStrategy(
  ...
  CR_DCHECK(degree_ < static_cast<uint32_t>(Estimator::kMaxDegree));
  Clear();
}
...
static bool SolveLeastSquares(const float* x,
  ...
  CR_DCHECK(m < M_ARRAY_LENGTH);
  CR_DCHECK(n < N_ARRAY_LENGTH);
#else
...
bool LeastSquaresVelocityTrackerStrategy::GetEstimator(
  ...
  const cr::TimeDelta horizon = cr::TimeDelta::FromMilliseconds(kHorizonMS);
...
void IntegratingVelocityTrackerStrategy::UpdateState(
    ...
    float xpos,
    float ypos) const {
  const cr::TimeDelta MIN_TIME_DELTA = TimeDelta::FromMicroseconds(2);
