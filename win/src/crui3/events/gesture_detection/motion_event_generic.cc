...
void PointerProperties::SetAxesAndOrientation(float radius_x,
  ...
  CR_DCHECK(!touch_major && !touch_minor && !orientation);
  float rotation_angle_rad = gfx::DegToRad(rotation_angle_degree);
  CR_DCHECK(radius_x >= 0) << "Unexpected x-radius < 0 (" << radius_x << ")";
  CR_DCHECK(radius_y >= 0) << "Unexpected y-radius < 0 (" << radius_y << ")";
  CR_DCHECK(0 <= rotation_angle_rad && rotation_angle_rad < cr::kPiFloat)
  ...
  if (rotation_angle_rad >= cr::kPiFloat / 2) {
    rotation_angle_rad -= cr::kPiFloat / 2;
  ...
  if (radius_x > radius_y) {
    ...
    orientation = rotation_angle_rad - cr::kPiFloat / 2;
  } else {
...
int MotionEventGeneric::GetActionIndex() const {
  CR_DCHECK(action_ == Action::POINTER_DOWN || action_ == Action::POINTER_UP);
  CR_DCHECK(action_index_ >= 0);
  CR_DCHECK(action_index_ < static_cast<int>(pointers_->size()));
...
int MotionEventGeneric::GetPointerId(size_t pointer_index) const {
  CR_DCHECK(pointer_index < pointers_->size());
...
cr::TimeTicks MotionEventGeneric::GetEventTime() const {
  return event_time_;
}
...
cr::TimeTicks MotionEventGeneric::GetHistoricalEventTime(
    size_t historical_index) const {
  CR_DCHECK(historical_index < historical_events_.size());
...
std::unique_ptr<MotionEventGeneric> MotionEventGeneric::CloneEvent(
  ...
  return cr::WrapUnique(new MotionEventGeneric(event, with_history));
}
...
void MotionEventGeneric::PushHistoricalEvent(
  ...
  CR_DCHECK(event);
  CR_DCHECK(event->GetAction() == Action::MOVE);
  CR_DCHECK(event->GetPointerCount() == GetPointerCount());
  CR_DCHECK(event->GetAction() == GetAction());
  CR_DCHECK(event->GetEventTime() < GetEventTime());
