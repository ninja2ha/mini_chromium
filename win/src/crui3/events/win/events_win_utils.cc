...
cr::TimeTicks EventTimeFromMSG(const MSG& native_event) {
...
gfx::Point EventLocationFromMSG(const MSG& native_event) {
  ...
  if ((native_event.message == WM_MOUSELEAVE ||
    ...
    CR_DCHECK(IsNonClientMouseEvent(native_event) ||
...
gfx::Vector2d GetMouseWheelOffsetFromMSG(const MSG& native_event) {
  CR_DCHECK(native_event.message == WM_MOUSEWHEEL ||
            native_event.message == WM_MOUSEHWHEEL);
...
int GetModifiersFromKeyState() {
  ...
  if (crui::win::IsAltRightPressed() && PlatformKeyMap::UsesAltGraph()) {
    ...
    if (crui::win::IsAltPressed())
      modifiers |= EF_ALT_DOWN;
  }
  if (crui::win::IsWindowsKeyPressed())
    modifiers |= EF_COMMAND_DOWN;
  if (crui::win::IsNumLockOn())
    modifiers |= EF_NUM_LOCK_ON;
  if (crui::win::IsCapsLockOn())
    modifiers |= EF_CAPS_LOCK_ON;
  if (crui::win::IsScrollLockOn())
...
KeyEvent KeyEventFromMSG(const MSG& msg) {
  ...
  DomKey key;
  cr::TimeTicks time_stamp = EventTimeFromMSG(msg);

  if (IsCharFromMSG(msg)) {
    flags = PlatformKeyMap::ReplaceControlAndAltWithAltGraph(flags);
    return KeyEvent(static_cast<cr::char16>(msg.wParam), 
                    key_code, code, flags, time_stamp);
  } else {
