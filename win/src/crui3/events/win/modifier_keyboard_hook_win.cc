...
class ModifierKeyboardHookWinImpl : public KeyboardHookWinBase {
 public:
  ModifierKeyboardHookWinImpl(
      const ModifierKeyboardHookWinImpl&) = delete;
  ModifierKeyboardHookWinImpl& operator=(
      const ModifierKeyboardHookWinImpl&) = delete;

  ModifierKeyboardHookWinImpl(cr::Optional<cr::flat_set<DomCode>> dom_codes,
...
bool ModifierKeyboardHookWinImpl::ProcessKeyEventMessage(WPARAM w_param,
  ...
  CR_DCHECK(vk != static_cast<DWORD>(VK_CONTROL));
  CR_DCHECK(vk != static_cast<DWORD>(VK_MENU));
...
void ModifierKeyboardHookWinImpl::UpdateModifierState(DWORD vk,
  ...
  if (!SetKeyboardState(keyboard_state))
    CR_PLOG(Error) << "SetKeyboardState() failed: ";
}
...
std::unique_ptr<KeyboardHook> KeyboardHook::CreateModifierKeyboardHook(
  ...
  auto raw_ptr = static_cast<ModifierKeyboardHookWinImpl*>(keyboard_hook.get());
  if (!raw_ptr->Register())
    return nullptr;
