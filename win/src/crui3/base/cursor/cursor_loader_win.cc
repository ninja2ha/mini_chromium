...
///#include "crui/resources/grit/ui_unscaled_resources.h"

namespace crui {

namespace {

cr::LazyInstance<cr::string16>::DestructorAtExit
...
void CursorLoaderWin::SetPlatformCursor(gfx::NativeCursor* cursor) {
  if (cursor->native_type() != CursorType::kCustom) {
    if (cursor->platform()) {
      ...
      PlatformCursor platform_cursor = ::LoadCursorW(NULL, cursor_id);
      if (!platform_cursor && !g_cursor_resource_module_name.Get().empty()) {
        platform_cursor = ::LoadCursorW(
            ::GetModuleHandleW(g_cursor_resource_module_name.Get().c_str()),
                               cursor_id);
      }
