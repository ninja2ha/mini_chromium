...
#include "crui/base/clipboard/scoped_clipboard_writer.h"
...
ScopedClipboardWriter::~ScopedClipboardWriter() {
  ...
  CR_DCHECK(objects_.empty() || platform_representations_.empty())
      << "Portable and Platform representations should not be written on the "
         "same write.";
  CR_DCHECK(platform_representations_.size() < kMaxRepresentations);
  if (!objects_.empty()) {
...
void ScopedClipboardWriter::WriteText(const cr::string16& text) {
  std::string utf8_text = cr::UTF16ToUTF8(text);
...
void ScopedClipboardWriter::WriteHTML(const cr::string16& markup,
                                      const std::string& source_url) {
  std::string utf8_markup = cr::UTF16ToUTF8(markup);
...
void ScopedClipboardWriter::WriteBookmark(const cr::string16& bookmark_title,
                                          const std::string& url) {
  if (bookmark_title.empty() || url.empty())
    return;

  std::string utf8_markup = cr::UTF16ToUTF8(bookmark_title);
...
void ScopedClipboardWriter::WriteHyperlink(const cr::string16& anchor_text,
                                           const std::string& url) {
  ...
  html += cr::EscapeForHTML(url);
  html += "\">";
  html += cr::EscapeForHTML(cr::UTF16ToUTF8(anchor_text));
  html += "</a>";
  WriteHTML(cr::UTF8ToUTF16(html), std::string());
}
