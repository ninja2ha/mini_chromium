...
#include "crui/base/clipboard/clipboard_buffer.h"
...
class CRUI_EXPORT Clipboard : public cr::ThreadChecker {
 public:
...
#if !defined(MINI_CHROMIUM_OS_WIN) && !defined(MINI_CHROMIUM_OS_MACOSX)
        return true;
#else
        return false;
#endif
  ...
  virtual std::vector<cr::string16> ReadAvailablePlatformSpecificFormatNames(
      ClipboardBuffer buffer) const = 0;

  // Reads Unicode text from the clipboard, if available.
  virtual void ReadText(ClipboardBuffer buffer,
                        cr::string16* result) const = 0;
  ...
  virtual void ReadBookmark(cr::string16* title, std::string* url) const = 0;
  ...
  virtual cr::Time GetLastModifiedTime() const;
 ...
 protected:
  ...
  using ObjectMap = cr::flat_map<PortableFormat, ObjectMapParams>;
 ...
 private:
  ...
  static cr::PlatformThreadId GetAndValidateThreadID();
  ...
  static std::vector<cr::PlatformThreadId>& AllowedThreads();

  // Mapping from threads to clipboard objects.
  using ClipboardMap =
      cr::flat_map<cr::PlatformThreadId, std::unique_ptr<Clipboard>>;
  static ClipboardMap* ClipboardMapPtr();

  // Mutex that controls access to |g_clipboard_map|.
  static cr::Lock& ClipboardMapLock();
};
