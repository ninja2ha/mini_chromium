...
enum : uint32_t {
  kLogNull = 0,
  kLogToFile = 1 << 0,
  kLogToSystemDebugLog = 1 << 1,
  kLogToStdErr = 1 << 2,
  kLogToAll = kLogToFile | kLogToSystemDebugLog | kLogToStdErr,

  // On Windows, use a file next to the exe.
  // On POSIX platforms, where it may not even be possible to locate the
  // executable on disk, use stderr.
#if defined(MINI_CHROMIUM_OS_WIN)
  kLogDefault = kLogToFile,
#elif defined(MINI_CHROMIUM_OS_POSIX)
  kLogDefault = kLogToSystemDebugLog | kLogToStdErr,
#endif
};
...
struct CRBASE_EXPORT LoggingConfig {
  ...
  // destinations.
  uint32_t logging_dest = kLogDefault;
  ...
  void(*fn_log_message_init)(const LoggingConfig& config,
                             LogSeverity severity, const char* file, 
                             int line, std::ostringstream& oss)
  ...
  void(*fn_log_assert_handler)(const LoggingConfig& config, 
                               const char* file, int line,
                               const cr::StringPiece message,
                               const cr::StringPiece stack_trace) 
      = nullptr;
...
CRBASE_EXPORT bool ShouldCreateLogmessage(const LoggingConfig& config, 
                                          LogSeverity severity);
CRBASE_EXPORT LoggingConfig& GetDefaultLoggingConfig();

CRBASE_EXPORT bool LoggingConfigInit(LoggingConfig& config);
CRBASE_EXPORT void LoggingConfigUninit(LoggingConfig& config);
...
#define CR_DEFAULT_LOGGING_CONFIG ::cr::logging::GetDefaultLoggingConfig()

#define CR_LOG_STREAM(severity)                                                \
  COMPACT_LOG_##severity(LogMessage, CR_DEFAULT_LOGGING_CONFIG, __FILE__,      \
                         __LINE__).stream()
...
#define CR_CHECK(condition)                                                    \
  CR_UNLIKELY(!(condition)) ?                                                  \
      [](){CR_IMMEDIATE_CRASH();}() : CR_EAT_STREAM_PARAMETERS
...
#define CR_VLOG_STREAM(verboselevel)                                           \
  ::cr::logging::LogMessage(CR_DEFAULT_LOGGING_CONFIG, __FILE__,               \
...
#define CR_DLOG(severity) CR_EAT_STREAM_PARAMETERS
#define CR_DLOG_IF(severity, condition) CR_EAT_STREAM_PARAMETERS

#define CR_DPLOG(severity) CR_EAT_STREAM_PARAMETERS
#define CR_DPLOG_IF(severity, condition) CR_EAT_STREAM_PARAMETERS

#define CR_DVLOG(verboselevel) CR_EAT_STREAM_PARAMETERS
#define CR_DVLOG_IF(verboselevel, condition) CR_EAT_STREAM_PARAMETERS

#define CR_DCHECK(condition) CR_EAT_STREAM_PARAMETERS
#define CR_DPCHECK(condition) CR_EAT_STREAM_PARAMETERS
#else
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_LOGGING_H_