...
void SetTlsVectorValue(PlatformThreadLocalStorage::TLSKey key,
  ...
  CR_DCHECK(tls_data || (state == TlsVectorState::kUninitialized) ||
...
TlsVectorEntry* ConstructTlsVector() {
  ...
  CR_CHECK(GetTlsVectorStateAndValue(key) == TlsVectorState::kUninitialized);
...
void OnThreadExitInternal(TlsVectorEntry* tls_data) {
  ...
  {
    cr::AutoLock auto_lock(*GetTLSMetadataLock());
...
void PlatformThreadLocalStorage::OnThreadExit() {
  ...
  CR_DCHECK(state != TlsVectorState::kDestroyed);
  ...
  // Grab a new slot.
  {
    ...
    for (int i = 0; i < kThreadLocalStorageSize; ++i) {
      ...
      if (g_tls_metadata[slot_candidate].status == TlsStatus::FREE) {
        ...
        CR_DCHECK(kInvalidSlotValue == slot_);
        slot_ = checked_cast<int>(slot_candidate);
        version_ = g_tls_metadata[slot_candidate].version;
        break;
      }
    }
  }
  CR_CHECK(slot_ != kInvalidSlotValue);

  CR_CHECK(slot_ < kThreadLocalStorageSize);
}

void ThreadLocalStorage::Slot::Free() {
  CR_DCHECK(slot_ != kInvalidSlotValue);
  CR_DCHECK(slot_ < kThreadLocalStorageSize);
  {
    cr::AutoLock auto_lock(*GetTLSMetadataLock());
...
void* ThreadLocalStorage::Slot::Get() const {
  ...
  const TlsVectorState state = GetTlsVectorStateAndValue(
      cr::subtle::NoBarrier_Load(&g_native_tls_key), &tls_data);
  CR_DCHECK(state != TlsVectorState::kDestroyed);
  if (!tls_data)
    return nullptr;
  CR_DCHECK(slot_ != kInvalidSlotValue);
