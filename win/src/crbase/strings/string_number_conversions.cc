...
#include "crbase/third_party/double_conversion/double-conversion.h"
...
string16 NumberToString16(double value) {
  ...
  return cr::string16(&buffer[0], &buffer[builder.position()]);
}
...
bool StringToUint(StringPiece input, unsigned int* output) {
  return StringToIntImpl(input, output);
}

bool StringToUint(StringPiece16 input, unsigned int* output) {
  return String16ToIntImpl(input, output);
}
...
std::string HexEncode(cr::Span<const uint8_t> bytes) {
...
bool HexStringToSpan(StringPiece input, cr::Span<uint8_t> output) {
  size_t count = input.size();
  if (count == 0 || (count % 2) != 0)
    return false;
