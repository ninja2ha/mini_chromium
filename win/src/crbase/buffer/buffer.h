...
class BufferT {
 ...
 public:
  ...
  //
  //   size_t setter(Span<U> view)
  //
  // |setter| is given an appropriately typed Span of length exactly
  ...
  // the whole Span, it should leave the unused space at the end.)
  template <typename U = T,
            typename F,
            typename std::enable_if<
                internal::BufferCompat<T, U>::value>::type* = nullptr>
  ...
  // |max_elements| that describes the area where it should write the data; it
  // should return the number of elements actually written. (If it doesn't fill
  // the whole Soab, it should leave the unused space at the end.)
  template <typename U = T,
            typename F,
            typename std::enable_if<
                internal::BufferCompat<T, U>::value>::type* = nullptr>
  size_t AppendData(size_t max_elements, F&& setter) {
    ...
    size_t written_elements = setter(cr::Span<U>(base_ptr, max_elements));

    CR_CHECK(written_elements <= max_elements);
 ...
 private:
  ...
  void ZeroTrailingData(size_t count) {
    CR_DCHECK(IsConsistent());
    CR_DCHECK(count <= capacity_ - size_);
...
using ByteBuffer = internal::BufferT<uint8_t>;
using CharBuffer = internal::BufferT<char>;

// A buffer that zeros memory before releasing it.
template <typename T>
using ZeroOnFreeBuffer = internal::BufferT<T, true>;
