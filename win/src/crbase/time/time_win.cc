...
void MicrosecondsToFileTime(int64_t us, FILETIME* ft) {
  CR_DCHECK(us >= 0LL) << "Time is less than 0, negative values are not "
...
cr::Lock* GetHighResLock() {
  static auto* lock = new cr::Lock();
  return lock;
}
...
bool SafeConvertToWord(int in, WORD* out) {
  cr::CheckedNumeric<WORD> result = in;
...
void Time::EnableHighResolutionTimer(bool enable) {
  cr::AutoLock lock(*GetHighResLock());
...
union LastTimeAndRolloversState {
  ...
  cr::subtle::Atomic32 as_opaque_32;
...
cr::subtle::Atomic32 g_last_time_and_rollovers = 0;
static_assert(
...
TimeDelta RolloverProtectedNow() {
  ...
  while (true) {
    ...
    int32_t original = cr::subtle::Acquire_Load(&g_last_time_and_rollovers);
    ...
    int32_t check = cr::subtle::Release_CompareAndSwap(
...
TimeTicks::TickFunctionType TimeTicks::SetMockTickFunction(
  ...
  cr::subtle::NoBarrier_Store(&g_last_time_and_rollovers, 0);
  return old;
}
...
ThreadTicks ThreadTicks::GetForThread(
    const cr::PlatformThreadHandle& thread_handle) {
...
bool ThreadTicks::IsSupportedWin() {
  static bool is_supported = cr::CPU().has_non_stop_time_stamp_counter() &&
