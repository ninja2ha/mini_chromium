...
bool PathIsWritable(const FilePath& path) {
  ThreadRestrictions::AssertIOAllowed();
  HANDLE dir =
      ::CreateFileW(path.value().c_str(), FILE_ADD_FILE, kFileShareAll, NULL,
...
bool CreateTemporaryFileInDir(const FilePath& dir, FilePath* temp_file) {
  ...
  if (!create_file_success) {
    CR_DPLOG(Warning) << "Failed to get temporary file name in " << dir;
    return false;
  }
...
bool CreateTemporaryDirInDir(const FilePath& base_dir,
  ...
  for (int count = 0; count < 50; ++count) {
    ...
    new_dir_name.append(NumberToString16(::GetCurrentProcessId()));
...
bool CreateWinHardLink(const FilePath& to_file, const FilePath& from_file) {
  ...
  return !!::CreateHardLinkW(
      to_file.value().c_str(), from_file.value().c_str(), nullptr);
}
...
bool AppendToFile(const FilePath& filename, const char* data, int size) {
  ...
  win::ScopedHandle file(::CreateFileW(filename.value().c_str(), 
                                       FILE_APPEND_DATA, 0, NULL, OPEN_EXISTING,
                                       0, NULL));
  if (!file.IsValid()) {
    ///CR_VPLOG(1) << "CreateFile failed for path " << filename;
    CR_PLOG(Warning) << "CreateFile failed for path " << filename;
    return false;
  }
  ...
  if (!result) {
    // WriteFile failed.
    ///CR_VPLOG(1) << "Writing file " << filename << " failed";
    CR_PLOG(Warning) << "Writing file " << filename << " failed";
  } else {
    // Didn't write all the bytes.
    ///CR_VPLOG(1) << "Only wrote " << written << " out of " << size 
    ///            << " byte(s) to " << filename;
    CR_PLOG(Warning) << "Only wrote " << written << " out of " << size
                     << " byte(s) to " << filename;
...
int GetMaximumPathComponentLength(const FilePath& path) {
  ...
  if (!GetVolumePathNameW(path.NormalizePathSeparators().value().c_str(),
                          volume_path, 
                          static_cast<DWORD>(cr::size(volume_path)))) {
    return -1;
  }
...
///// dynamically.
///typedef BOOL(WINAPI* PrefetchVirtualMemoryPtr)(
...
///
///  const ::SIZE_T length =
///      std::min(cr::saturated_cast<::SIZE_T>(max_bytes),
///               cr::saturated_cast<::SIZE_T>(mapped_file.length()));
