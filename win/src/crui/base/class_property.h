...
class CRUI_EXPORT PropertyHandler {
 public:
  PropertyHandler();
...
template <>
class ClassPropertyCaster<cr::TimeDelta> {
 public:
  static int64_t ToInt64(cr::TimeDelta x) { return x.InMicroseconds(); }
  static cr::TimeDelta FromInt64(int64_t x) {
    return cr::TimeDelta::FromMicroseconds(x);
  }
};
...
class CRUI_EXPORT PropertyHelper {
 public:
  template<typename T>
  static void Set(::crui::PropertyHandler* handler,
                  const ::crui::ClassProperty<T>* property, T value) {
  ...
  template<typename T>
  static T Get(const ::crui::PropertyHandler* handler,
  ...
  template<typename T>
  static void Clear(::crui::PropertyHandler* handler,
                    const ::crui::ClassProperty<T>* property) {
...
#define DEFINE_UI_CLASS_PROPERTY_KEY(TYPE, NAME, DEFAULT)                    \
  ...
  const ::crui::ClassProperty<TYPE> NAME##_Value = {DEFAULT, #NAME, nullptr};\
  const ::crui::ClassProperty<TYPE>* const NAME = &NAME##_Value;

#define DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(TYPE, NAME, DEFAULT)         \
  namespace {                                                           \
  void Deallocator##NAME(int64_t p) {                                   \
    enum { type_must_be_complete = sizeof(TYPE) };                      \
    delete ::crui::ClassPropertyCaster<TYPE*>::FromInt64(p);            \
  }                                                                     \
  const ::crui::ClassProperty<TYPE*> NAME##_Value = {DEFAULT, #NAME,    \
                                                   &Deallocator##NAME}; \
  } /* namespace */                                                     \
  const ::crui::ClassProperty<TYPE*>* const NAME = &NAME##_Value;
