...
///#include "crui/gfx/text_elider.h"
...
aura::Window* GetTooltipTarget(const crui::MouseEvent& event,
                               gfx::Point* location) {
  switch (event.type()) {
    case crui::ET_MOUSE_CAPTURE_CHANGED:
...
void TooltipController::OnKeyEvent(crui::KeyEvent* event) {
...
void TooltipController::OnMouseEvent(crui::MouseEvent* event) {
...
void TooltipController::OnTouchEvent(crui::TouchEvent* event) {
...
void TooltipController::OnCancelMode(crui::CancelModeEvent* event) {
  tooltip_->Hide();
  SetTooltipWindow(nullptr);
}
...
void TooltipController::OnWindowPropertyChanged(aura::Window* window,
  ...
  if ((key == wm::kTooltipIdKey || key == wm::kTooltipTextKey) &&
      wm::GetTooltipText(window) != cr::string16() &&
...
void TooltipController::UpdateIfRequired() {
  ...
  if (tooltip_text_ != tooltip_text || !tooltip_->IsVisible() || ids_differ) {
    ...
    cr::string16 trimmed_text = tooltip_text_;
    ///*cr::string16 trimmed_text =
    ...
    cr::TrimWhitespace(trimmed_text, cr::TRIM_ALL,
    ...
    if (tooltip_text_whitespace_trimmed_.empty()) {
      ...
      if (tooltip_defer_timer_.IsRunning()) {
        ...
        tooltip_defer_timer_.Start(CR_FROM_HERE, 
...
void TooltipController::ShowTooltip() {
  ...
  if (timeout > 0) {
    tooltip_shown_timer_.Start(CR_FROM_HERE,
                               cr::TimeDelta::FromMilliseconds(timeout), this,
