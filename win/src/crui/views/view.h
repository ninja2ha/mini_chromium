...
#include "crbase/functional/callback_list.h"
...
struct CRUI_EXPORT ViewHierarchyChangedDetails {
...
using PropertyChangedCallbacks = cr::RepeatingCallbackList<void()>;
...
class CRUI_EXPORT View : ///public crui::LayerDelegate,
 ...
 public:
   ...
   private:
    ...
    void AddOffset(const gfx::Vector2d& offset_to_parent) {
      ...
      gfx::Vector2dF integral_pixel_offset(
          static_cast<float>(gfx::ToRoundedInt(fractional_pixel_offset.x())),
          static_cast<float>(gfx::ToRoundedInt(fractional_pixel_offset.y())));
  ...
  T* AddChildView(std::unique_ptr<T> view) {
    CR_DCHECK(!view->owned_by_client())
  ...
  PropertyChangedSubscription AddVisibleChangedCallback(
      PropertyChangedCallback callback) CR_WARN_UNUSED_RESULT;
  ...
  ///crui::NativeTheme* GetNativeTheme() {
  ///  return const_cast<crui::NativeTheme*>(
  ///      const_cast<const View*>(this)->GetNativeTheme());
  ///}
  ///const crui::NativeTheme* GetNativeTheme() const;
  ...
  virtual gfx::NativeCursor GetCursor(const crui::MouseEvent& event);
  ...
  // set, false otherwise. Override as needed.
  //
  virtual bool OnMouseDragged(const crui::MouseEvent& event);
  ...
  virtual bool OnMouseWheel(const crui::MouseWheelEvent& event);
  ...
  ///  return const_cast<crui::InputMethod*>(
  ///      const_cast<const View*>(this)->GetInputMethod());
  ///}
  ///const crui::InputMethod* GetInputMethod() const;
  ...
  void ConvertEventToTarget(const crui::EventTarget* target,
                            crui::LocatedEvent* event) const override;
  gfx::PointF GetScreenLocationF(const crui::LocatedEvent& event) const 
      override;

  // Overridden from crui::EventHandler:
  ...
  void AddAccelerator(const crui::Accelerator& accelerator);

  // Removes the specified accelerator for this view.
  void RemoveAccelerator(const crui::Accelerator& accelerator);
  ...
  virtual bool SkipDefaultKeyEventProcessing(const crui::KeyEvent& event);
  ...
  virtual cr::string16 GetTooltipText(const gfx::Point& p) const;
  ...
  ///virtual void GetAccessibleNodeData(crui::AXNodeData* node_data) {}
 ...
 private:
  ...
  void ProcessMouseDragged(crui::MouseEvent* event);
  void ProcessMouseReleased(const crui::MouseEvent& event);
  ...
  cr::Optional<gfx::Size> preferred_size_;
  ...
  // widget).
  ///crui::NativeTheme* native_theme_ = nullptr;
  ...
  std::unique_ptr<std::vector<crui::Accelerator>> accelerators_;
  ...
  ///cr::ObserverList<ViewObserver>::Unchecked observers_;
  cr::ObserverList<ViewObserver> observers_;
