...
///#include "crui/gfx/image/image.h"
///#include "crui/gfx/image/image_skia.h"
///#include "crui/gfx/image/image_skia_operations.h"
///#include "crui/gfx/nine_image_painter.h"
...
class DashedFocusPainter : public Painter {
 public:
  DashedFocusPainter(const DashedFocusPainter&) = delete;
  DashedFocusPainter& operator=(const DashedFocusPainter&) = delete;
...
class GradientPainter : public Painter {
 public:
  GradientPainter(const GradientPainter&) = delete;
  GradientPainter& operator=(const GradientPainter&) = delete;
...
void GradientPainter::Paint(gfx::Canvas* canvas, const gfx::Size& size) {
  ...
  skia::RefPtr<SkShader> s = skia::AdoptRef(SkGradientShader::CreateLinear(
      p, colors_.get(), pos_.get(), static_cast<int>(count_), 
...
std::unique_ptr<Painter> Painter::CreateDashedFocusPainter() {
  return cr::WrapUnique(new DashedFocusPainter(gfx::Insets()));
}

// static
std::unique_ptr<Painter> Painter::CreateDashedFocusPainterWithInsets(
    const gfx::Insets& insets) {
  return cr::WrapUnique(new DashedFocusPainter(insets));
}
...
std::unique_ptr<Painter> Painter::CreateSolidFocusPainter(
    SkColor color,
    const gfx::Insets& insets) {
  return cr::WrapUnique(new SolidFocusPainter(color, insets));
}
