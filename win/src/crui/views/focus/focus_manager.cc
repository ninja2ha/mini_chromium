...
bool FocusManager::OnKeyEvent(const crui::KeyEvent& event) {
  const int key_code = event.key_code();

  if (event.type() != crui::ET_KEY_PRESSED && 
      event.type() != crui::ET_KEY_RELEASED)
    return false;
  ...
  if (event.type() == crui::ET_KEY_PRESSED) {
    ...
    bool is_left = key_code == crui::VKEY_LEFT || key_code == crui::VKEY_UP;
    bool is_right = key_code == crui::VKEY_RIGHT || key_code == crui::VKEY_DOWN;
...
View* FocusManager::GetNextFocusableView(View* original_starting_view,
                                         ...
                                         bool reverse,
                                         bool dont_loop) {
  CR_DCHECK(!focused_view_ || ContainsView(focused_view_))
...
void FocusManager::SetFocusedViewWithReason(View* view,
  ...
  CR_CHECK(!view || ContainsView(view));
  ...
  if (view) {
    if (ContainsView(view)) {
      if (!view->IsFocusable() && view->IsAccessibilityFocusable()) {
        ...
        cr::AutoReset<bool> in_restore_bit(&in_restoring_focused_view_, true);
        view->RequestFocus();
      }
    }
...
void FocusManager::RegisterAccelerator(
    ...
    crui::AcceleratorManager::HandlerPriority priority,
    crui::AcceleratorTarget* target) {
  accelerator_manager_.Register({accelerator}, priority, target);
}

void FocusManager::UnregisterAccelerator(const crui::Accelerator& accelerator,
                                         crui::AcceleratorTarget* target) {
  accelerator_manager_.Unregister(accelerator, target);
}

void FocusManager::UnregisterAccelerators(crui::AcceleratorTarget* target) {
  accelerator_manager_.UnregisterAll(target);
}

bool FocusManager::ProcessAccelerator(const crui::Accelerator& accelerator) {
...
bool FocusManager::HasPriorityHandler(
    const crui::Accelerator& accelerator) const {
  return accelerator_manager_.HasPriorityHandler(accelerator);
}

// static
bool FocusManager::IsTabTraversalKeyEvent(const crui::KeyEvent& key_event) {
  return key_event.key_code() == crui::VKEY_TAB &&
...
bool FocusManager::ProcessArrowKeyTraversal(const crui::KeyEvent& event) {
  if (event.IsShiftDown() || event.IsControlDown() || event.IsAltDown() ||
      event.IsAltGrDown())
    return false;

  const crui::KeyboardCode key = event.key_code();
  if (key != crui::VKEY_UP && key != crui::VKEY_DOWN && 
      key != crui::VKEY_LEFT && key != crui::VKEY_RIGHT) {
    return false;
  }
