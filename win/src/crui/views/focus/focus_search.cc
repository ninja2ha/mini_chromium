...
View* FocusSearch::FindNextFocusableView(
  ...
  CR_DCHECK(!root_->children().empty());
  ...
  *focus_traversable_view = nullptr;
  ...
  if (!starting_view) {
    ...
    CR_DCHECK(Contains(root_, starting_view));
  }

  cr::flat_set<View*> seen_views;
  View* v = nullptr;
  ...
  if (*focus_traversable) {
    CR_DCHECK(*focus_traversable_view);
    CR_DCHECK(v == nullptr);
    return nullptr;
  }
...
View* FocusSearch::FindNextFocusableViewImpl(
  ...
  if (cr::Contains(*seen_views, starting_view)) {
    CR_LOG(Error) << "View focus cycle detected.";
    return nullptr;
  }
