...
struct ColumnMinResizeData {
  ColumnMinResizeData(Column* v1, int v2, int v3) 
      : column(v1), available(v2), delta(v3) {}
...
class LayoutElement {
 public:
  ...
  static int TotalSize(int start,
    ...
    CR_DCHECK(start >= 0);
    CR_DCHECK(length > 0);
    CR_DCHECK(static_cast<size_t>(start + length) <= elements->size());
...
struct ViewState {
  ViewState(ColumnSet* column_set,
      ...
      : column_set(column_set),
        ...
        width(pref_width),
        height(pref_height) {
    CR_DCHECK(view && start_col >= 0 && start_row >= 0 && col_span > 0 &&
...
void ColumnSet::CalculateMasterColumns() {
  for (const auto& column : columns_) {
    ...
    if (same_size_column_index != -1) {
      CR_DCHECK(same_size_column_index >= 0 &&
...
void ColumnSet::AccumulateMasterColumns() {
  CR_DCHECK(master_columns_.empty());
  for (const auto& column : columns_) {
    Column* master_column = column->GetLastMasterColumn();
    if (master_column && !cr::Contains(master_columns_, master_column)) {
...
void ColumnSet::CalculateSize(SizeCalculationType type) {
  ...
  // SizeCalculationType::PREFERRED.
  CR_DCHECK(type == SizeCalculationType::PREFERRED ||
...
void ColumnSet::ResizeUsingMin(int total_delta) {
  ...
  for (size_t i = 0; i < columns_.size(); ++i) {
    ...
    if (column->ResizePercent() <= 0 || available == 0)
      continue;
    resize_data.push_back(ColumnMinResizeData(column, available, 0));
...
#if CR_DCHECK_IS_ON()
    CR_DCHECK(next_iteration_delta < initial_delta);
#endif
...
ColumnSet* GridLayout::AddColumnSet(int id) {
  CR_DCHECK(GetColumnSet(id) == nullptr);
  column_sets_.push_back(cr::WrapUnique(new ColumnSet(id)));
...
void GridLayout::AddExistingView(View* view,
  ...
  CR_DCHECK(view->parent() && view->parent() == host_)
      << "Use AddView() to add a new View that isn't already parented to "
         "|host_|.";
  CR_DCHECK(current_row_col_set_ && col_span > 0 && row_span > 0 &&
  ...
  // you need it.
  CR_DCHECK(v_align != BASELINE || row_span == 1);
...
void GridLayout::ViewAdded(View* host, View* view) {
  CR_DCHECK(host_ == host && adding_view_);
}

void GridLayout::ViewRemoved(View* host, View* view) {
  CR_DCHECK(host_ == host);
}
...
void GridLayout::SizeRowsAndColumns(bool layout, int width, int height,
  ...
  CR_DCHECK(!adding_view_) << "GridLayout queried while adding a view.";
...
void GridLayout::AddViewState(std::unique_ptr<ViewState> view_state) {
  CR_DCHECK(view_state->view && view_state->view->parent() == host_);
...
void GridLayout::AddRow(std::unique_ptr<Row> row) {
  ...
  // that.
  CR_DCHECK(remaining_row_span_ <= 0 || row->column_set() == nullptr ||
