...
#include "crui/views/layout/flex_layout.h"
...
T GetViewProperty(const View* view,
                  const crui::PropertyHandler& defaults,
                  const crui::ClassProperty<T*>* property,
...
class FlexLayout::ChildViewSpacing {
 public:
  ...
  using GetViewSpacingCallback =
      cr::RepeatingCallback<int(cr::Optional<size_t>,
                                cr::Optional<size_t>)>;
 ...
 private:
  cr::Optional<size_t> GetPreviousViewIndex(size_t view_index) const;
  cr::Optional<size_t> GetNextViewIndex(size_t view_index) const;
...
FlexLayout::ChildViewSpacing::ChildViewSpacing(
    ...
    : get_view_spacing_(std::move(get_view_spacing)),
      trailing_space_(get_view_spacing_.Run(cr::nullopt, cr::nullopt)) {}
...
int FlexLayout::ChildViewSpacing::GetLeadingSpace(size_t view_index) const {
  ...
  CR_DCHECK(it != leading_spacings_.end());
  return it->second;
}
...
int FlexLayout::ChildViewSpacing::GetAddDelta(size_t view_index) const {
  if (HasViewIndex(view_index))
    return 0;
  cr::Optional<size_t> prev = GetPreviousViewIndex(view_index);
  cr::Optional<size_t> next = GetNextViewIndex(view_index);
...
void FlexLayout::ChildViewSpacing::AddViewIndex(size_t view_index,
                                                int* new_leading,
                                                int* new_trailing) {
  CR_DCHECK(!HasViewIndex(view_index));
  cr::Optional<size_t> prev = GetPreviousViewIndex(view_index);
  cr::Optional<size_t> next = GetNextViewIndex(view_index);
...
cr::Optional<size_t> FlexLayout::ChildViewSpacing::GetPreviousViewIndex(
...
cr::Optional<size_t> FlexLayout::ChildViewSpacing::GetNextViewIndex(
    size_t view_index) const {
...
FlexLayout& FlexLayout::SetMainAxisAlignment(
  ...
  CR_DCHECK(main_axis_alignment != LayoutAlignment::kStretch)
...
ProposedLayout FlexLayout::CalculateProposedLayout(
  ...
  ChildViewSpacing child_spacing(
      cr::BindRepeating(&FlexLayout::CalculateChildSpacing,
                        cr::Unretained(this), std::cref(data)));
...
NormalizedSize FlexLayout::GetPreferredSizeForRule(
    const FlexRule& rule,
    const View* child,
    const cr::Optional<int>& available_cross) const {
  ...
  const NormalizedSize stretch_size =
      Normalize(orientation(),
                rule.Run(child, SizeBounds(available_cross, cr::nullopt)));
...
void FlexLayout::InitializeChildData(
  ...
  for (View* child : host_view()->children()) {
    ...
    const cr::Optional<int> available_cross =
    ...
    if (main_axis_bounded) {
      ...
      CR_DCHECK(flex_child.preferred_size.main() >=
...
void FlexLayout::CalculateChildBounds(const SizeBounds& size_bounds,
  ...
  switch (main_axis_alignment()) {
    ...
    case LayoutAlignment::kStretch:
      CR_NOTIMPLEMENTED() << "Main axis stretch/justify is not yet supported.";
      break;
  }
...
void FlexLayout::CalculateNonFlexAvailableSpace(
  ...
  for (size_t index = 0; index < data->child_data.size(); ++index) {
    if (cr::Contains(all_flex_indices, index))
      continue;
...
cr::Optional<int> FlexLayout::GetAvailableCrossAxisSize(
...
int FlexLayout::CalculateChildSpacing(
    const FlexLayoutData& layout,
    cr::Optional<size_t> child1_index,
    cr::Optional<size_t> child2_index) const {
  ...
  for (const auto& flex_elem : order_to_index) {
    ...
    for (int view_index : view_indices) {
      ...
      if (flex_weight > 0) {
        ...
        flex_amount =
            gfx::ToCeiledInt(
                static_cast<float>(remaining * flex_weight / flex_total));
      }
