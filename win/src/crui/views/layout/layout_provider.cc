...
///#include "crui/base/material_design/material_design_controller.h"
///#include "crui/gfx/font_list.h"
...
gfx::Insets LayoutProvider::GetInsetsMetric(int metric) const {
  CR_DCHECK(metric >= VIEWS_INSETS_START);
  CR_DCHECK(metric < VIEWS_INSETS_MAX);
  switch (metric) {
...
int LayoutProvider::GetDistanceMetric(int metric) const {
  CR_DCHECK(metric >= VIEWS_DISTANCE_START);
  CR_DCHECK(metric <= VIEWS_DISTANCE_MAX);
  switch (metric) {
