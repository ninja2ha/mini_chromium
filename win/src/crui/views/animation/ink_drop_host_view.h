...
#include "crui/views/animation/ink_drop_event_handler.h"
...
class CRUI_EXPORT InkDropHostView : public View {
 public:
  ...
  virtual void AddInkDropLayer(crui::Layer* ink_drop_layer);

  // Removes |ink_drop_layer| from the layer tree.
  virtual void RemoveInkDropLayer(crui::Layer* ink_drop_layer);
  ...
  void set_ink_drop_highlight_opacity(cr::Optional<float> opacity) {
 ...
 protected:
  ...
  void InstallInkDropMask(crui::Layer* ink_drop_layer);
 ...
 private:
  ...
  cr::Optional<float> ink_drop_highlight_opacity_;
