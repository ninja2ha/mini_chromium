...
class CRUI_EXPORT HWNDMessageHandlerDelegate {
 public:
  ...
  virtual void HandleAccelerator(const crui::Accelerator& accelerator) = 0;
  ...
  virtual bool HandleInitialFocus(crui::WindowShowState show_state) = 0;
  ...
  virtual bool HandleMouseEvent(crui::MouseEvent* event) = 0;

  // Called when an untranslated key event is received (i.e. pre-IME
  // translation).
  virtual void HandleKeyEvent(crui::KeyEvent* event) = 0;

  // Called when a touch event is received.
  virtual void HandleTouchEvent(crui::TouchEvent* event) = 0;
  ...
  // handled by the delegate.
  virtual bool HandleScrollEvent(crui::ScrollEvent* event) = 0;
