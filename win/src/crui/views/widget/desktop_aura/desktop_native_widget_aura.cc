...
#include "crui/views/corewm/tooltip_controller.h"
#include "crui/views/drag_utils.h"
#include "crui/views/view_constants_aura.h"
#include "crui/views/views_delegate.h"
#include "crui/views/widget/desktop_aura/desktop_capture_client.h"
#include "crui/views/widget/desktop_aura/desktop_event_client.h"
#include "crui/views/widget/desktop_aura/desktop_focus_rules.h"
...
#include "crui/aura/wm/core/cursor_manager.h"
...
#include "crui/aura/wm/core/visibility_controller.h"
...
DEFINE_EXPORTED_UI_CLASS_PROPERTY_TYPE(CRUI_EXPORT,
                                       crui::views::DesktopNativeWidgetAura*)
...
class DesktopNativeWidgetTopLevelHandler : public aura::WindowObserver {
 public:
  ...
  void OnWindowDestroying(aura::Window* window) override {
    ...
    if (top_level_widget_) {
      CR_DCHECK(top_level_widget_->GetNativeView());
 ...
 public:
  ...
  aura::Window* GetDefaultParent(aura::Window* window,
    ...
    if (is_fullscreen || is_menu) {
      crui::ZOrderLevel root_z_order = crui::ZOrderLevel::kNormal;
...
DesktopNativeWidgetAura::DesktopNativeWidgetAura(
    ...
    : desktop_window_tree_host_(nullptr),
      ...
      last_drop_operation_(crui::DragDropTypes::DRAG_NONE),
...
void DesktopNativeWidgetAura::SetDesktopWindowTreeHost(
  ...
  CR_DCHECK(!desktop_window_tree_host_);
...
void DesktopNativeWidgetAura::GetWindowPlacement(
      gfx::Rect* bounds,
      crui::WindowShowState* maximized) const {
  if (content_window_)
    desktop_window_tree_host_->GetWindowPlacement(bounds, maximized);
}

bool DesktopNativeWidgetAura::SetWindowTitle(const cr::string16& title) {
  if (!content_window_)
    return false;
...
void DesktopNativeWidgetAura::InitModalType(crui::ModalType modal_type) {
...
void DesktopNativeWidgetAura::Show(crui::WindowShowState show_state,
...
void DesktopNativeWidgetAura::SetZOrderLevel(crui::ZOrderLevel order) {
  if (content_window_)
    desktop_window_tree_host_->SetZOrderLevel(order);
}

crui::ZOrderLevel DesktopNativeWidgetAura::GetZOrderLevel() const {
  if (content_window_)
...
crui::GestureRecognizer* DesktopNativeWidgetAura::GetGestureRecognizer() {
...
///void DesktopNativeWidgetAura::OnPaint(const crui::PaintContext& context) {
...
void DesktopNativeWidgetAura::OnKeyEvent(crui::KeyEvent* event) {
  if (event->is_char()) {
...
void DesktopNativeWidgetAura::OnMouseEvent(crui::MouseEvent* event) {
  CR_DCHECK(content_window_->IsVisible());
...
void DesktopNativeWidgetAura::OnScrollEvent(crui::ScrollEvent* event) {
  if (event->type() == crui::ET_SCROLL) {
    native_widget_delegate_->OnScrollEvent(event);
    if (event->handled())
      return;

    // Convert unprocessed scroll events into wheel events.
    crui::MouseWheelEvent mwe(*event->AsScrollEvent());
...
void DesktopNativeWidgetAura::OnGestureEvent(crui::GestureEvent* event) {
...
void DesktopNativeWidgetAura::OnWindowActivated(
  ...
  CR_DCHECK(content_window_ == gained_active || 
            content_window_ == lost_active);
  if (gained_active == content_window_ && restore_focus_on_activate_) {
    ...
    CR_DCHECK(!restore_focus_on_activate_);
...
void DesktopNativeWidgetAura::OnDragEntered(
    const crui::DropTargetEvent& event) {
  CR_DCHECK(drop_helper_.get() != nullptr);
  last_drop_operation_ = drop_helper_->OnDragOver(event.data(),
      event.location(), event.source_operations());
}

int DesktopNativeWidgetAura::OnDragUpdated(
    const crui::DropTargetEvent& event) {
  CR_DCHECK(drop_helper_.get() != nullptr);
  last_drop_operation_ = drop_helper_->OnDragOver(event.data(),
      event.location(), event.source_operations());
