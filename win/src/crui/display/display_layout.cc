...
void MaybeReparentTargetDisplay(
  ...
  if (iter != display_to_parent_ids_map->end()) {
    ...
    CR_DCHECK(target_display_placement_itr != placement_list->end());
  ...
  CR_DCHECK(target_display_placement);
...
void DeIntersectDisplays(int64_t primary_id,
  ...
  CR_DCHECK(sorted_displays.front()->id() == primary_id);
...
DisplayPlacement::DisplayPlacement(int64_t display_id,
  ...
  CR_DCHECK(TOP <= position);
  CR_DCHECK(LEFT >= position);
  ...
  CR_DCHECK(kMaxValidOffset >= abs(offset));
}
...
// static
bool DisplayPlacement::StringToPosition(const cr::StringPiece& string,
                                        Position* position) {
  if (string == kTop) {
    *position = TOP;
    return true;
  ...
  CR_LOG(Error) << "Invalid position value:" << string;
...
bool DisplayLayout::Validate(const DisplayIdList& list,
  ...
  if (!IsIdInList(layout.primary_id, list)) {
    CR_LOG(Error) << "The primary id: " << layout.primary_id
  ...
  for (const auto& placement : layout.placement_list) {
    ...
    if (prev_id >= (placement.display_id & 0xFF)) {
      CR_LOG(Error) << "PlacementList must be sorted by first 8 bits of"
                 << " display_id ";
      return false;
    }
    prev_id = (placement.display_id & 0xFF);
    if (placement.display_id == kInvalidDisplayId) {
      CR_LOG(Error) << "display_id is not initialized";
      return false;
    }
    if (placement.parent_display_id == kInvalidDisplayId) {
      CR_LOG(Error) << "display_parent_id is not initialized";
      return false;
    }
    if (placement.display_id == placement.parent_display_id) {
      CR_LOG(Error) << "display_id must not be same as parent_display_id";
      return false;
    }
    if (!IsIdInList(placement.display_id, list)) {
      CR_LOG(Error) << "display_id is not in the id list:" << placement.ToString();
      return false;
    }

    if (!IsIdInList(placement.parent_display_id, list)) {
      CR_LOG(Error) << "parent_display_id is not in the id list:"
                    << placement.ToString();
      return false;
    }
  ...
  if (!has_primary_as_parent)
    CR_LOG(Error) << "At least, one placement must have the primary as a parent.";
