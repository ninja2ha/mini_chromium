...
CoordinateRotation ComputeCoordinateRotationRefTop(
    const crui::gfx::Rect& ref,
    const crui::gfx::Rect& other) {
...
crui::gfx::Rect CoordinateRotateRectangle90(const crui::gfx::Rect& rect) {
  return crui::gfx::Rect(rect.y(), -rect.x() - rect.width(),
                         rect.height(), rect.width());
}

crui::gfx::Rect CoordinateRotateRectangle180(const crui::gfx::Rect& rect) {
  return crui::gfx::Rect(-rect.x() - rect.width(), -rect.y() -rect.height(),
                        rect.width(), rect.height());
}

crui::gfx::Rect CoordinateRotateRectangle270(const crui::gfx::Rect& rect) {
  return crui::gfx::Rect(-rect.y() - rect.height(), rect.x(),
                         rect.height(), rect.width());
}

crui::gfx::Rect CoordinateRotateRect(const crui::gfx::Rect& rect,
...
int ScaleOffset(int unscaled_length, float scale_factor, int unscaled_offset) {
  ...
  return crui::gfx::ToFlooredInt(scaled_length * percent);
}
...
DisplayPlacement CalculateDisplayPlacement(const DisplayInfo& parent,
                                           const DisplayInfo& current) {
  CR_DCHECK(DisplayInfosTouch(parent, current)) << "DisplayInfos must touch.";
  ...
  if (parent_end == current_end) {
    ...
    CR_DCHECK(InRange(parent_begin, current_begin, current_end));
