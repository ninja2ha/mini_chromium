...
bool IsValidTimebase(cr::TimeTicks now, cr::TimeTicks timestamp) {
  int64_t delta = (now - timestamp).InMilliseconds();
  return delta >= 0 && delta <= 60 * 1000;
}

void ValidateEventTimeClock(cr::TimeTicks* timestamp) {
  ...
  cr::TimeTicks now = EventTimeForNow();
...
///  cr::TimeTicks time_stamp = EventTimeFromNative(native_event);
///  cr::TimeDelta delta = current_time - time_stamp;
///
///  EventType type = EventTypeFromNative(native_event);
///  switch (type) {
