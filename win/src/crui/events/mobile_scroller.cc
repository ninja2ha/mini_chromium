...
#include "crui/events/mobile_scroller.h"
...
constexpr int kDefaultDurationMs = 250;

// Default friction constant in android.view.ViewConfiguration.
constexpr float kDefaultFriction = 0.015f;

// == std::log(0.78f) / std::log(0.9f)
constexpr float kDecelerationRate = 2.3582018f;
...
constexpr float kThresholdForFlingEnd = 0.1f;

const bool ApproxEquals(float a, float b) {
...
struct SplineConstants {
  ...
  SplineConstants() {
    ...
    constexpr float kP1 = kStartTension * kInflexion;
    constexpr float kP2 = 1.0f - kEndTension * (1.0f - kInflexion);
...
float ComputeDeceleration(float friction) {
  return cr::kMeanGravityFloat  // g (m/s^2)
         * 39.37f                 // inch/meter
         * 160.f                  // pixels/inch
         * friction;
}
...
cr::LazyInstance<ViscosityConstants>::Leaky g_viscosity_constants =
    LAZY_INSTANCE_INITIALIZER;

cr::LazyInstance<SplineConstants>::Leaky g_spline_constants =
    LAZY_INSTANCE_INITIALIZER;
...
bool MobileScroller::ComputeScrollOffset(cr::TimeTicks time,
...
void MobileScroller::StartScroll(float start_x,
                                 ...
                                 float dx,
                                 float dy,
                                 cr::TimeTicks start_time) {
  StartScroll(start_x, start_y, dx, dy, start_time,
              cr::TimeDelta::FromMilliseconds(kDefaultDurationMs));
}
...
void MobileScroller::StartScroll(float start_x,
  ...
  CR_DCHECK(duration > cr::TimeDelta());
  mode_ = SCROLL_MODE;
  finished_ = false;
  duration_ = duration;
...
void MobileScroller::Fling(float start_x,
                           ...
                           float max_y,
                           cr::TimeTicks start_time) {
  CR_DCHECK(velocity_x || velocity_y);
  ...
  CR_DCHECK(duration_ > cr::TimeDelta());
  duration_seconds_reciprocal_ = 1.0 / duration_.InSecondsF();
  start_time_ = start_time;
  curr_time_ = start_time_;
  curr_x_ = start_x_ = start_x;
  ...
  distance_ = static_cast<float>(total_distance * Signum(velocity));
  ...
  final_x_ = static_cast<float>(start_x + total_distance * coeff_x);
  final_x_ = Clamped(final_x_, min_x_, max_x_);

  final_y_ = static_cast<float>(start_y + total_distance * coeff_y);
  final_y_ = Clamped(final_y_, min_y_, max_y_);

  RecomputeDeltas();
}

void MobileScroller::ExtendDuration(cr::TimeDelta extend) {
  cr::TimeDelta passed = GetTimePassed();
...
cr::TimeDelta MobileScroller::GetTimePassed() const {
  return curr_time_ - start_time_;
}

cr::TimeDelta MobileScroller::GetDuration() const {
  return duration_;
}
...
bool MobileScroller::ComputeScrollOffsetInternal(cr::TimeTicks time) {
  if (finished_)
    return false;
  ...
  cr::TimeDelta time_passed = time - start_time_;
  ...
  const float u = static_cast<float>(
      time_passed.InSecondsF() * duration_seconds_reciprocal_);
  switch (mode_) {
    case UNDEFINED:
      CR_NOTREACHED() << "|StartScroll()| or |Fling()| must be called prior to "
    ...
    case FLING_MODE: {
      ...
      curr_velocity_ = static_cast<float>(
          velocity_coef * distance_ * duration_seconds_reciprocal_);
...
cr::TimeDelta MobileScroller::GetSplineFlingDuration(float velocity) const {
  ...
  ///return cr::TimeDelta::FromMicroseconds(time_seconds *
  ///                                       cr::Time::kMicrosecondsPerSecond);
  return cr::TimeDelta::FromSecondsD(time_seconds);
}
