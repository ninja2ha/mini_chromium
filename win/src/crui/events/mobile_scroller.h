...
class CRUI_EXPORT MobileScroller : public GestureCurve {
 public:
  ...
  void ExtendDuration(cr::TimeDelta extend);
  ...
  cr::TimeDelta GetTimePassed() const;

  // Returns how long the scroll event will take.
  cr::TimeDelta GetDuration() const;
 ...
 private:
  ...
  bool ComputeScrollOffsetInternal(cr::TimeTicks time);
  void RecomputeDeltas();

  double GetSplineDeceleration(float velocity) const;
  cr::TimeDelta GetSplineFlingDuration(float velocity) const;
