...
class CRUI_EXPORT MotionEvent {
 public:
  ...
  virtual cr::TimeTicks GetEventTime() const = 0;

  // Optional historical data, default implementation provides an empty history.
  virtual size_t GetHistorySize() const;
  virtual cr::TimeTicks GetHistoricalEventTime(size_t historical_index) const;
...
CRUI_EXPORT std::ostream& operator<<(
    std::ostream& stream,
