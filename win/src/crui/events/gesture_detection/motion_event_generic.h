...
struct CRUI_EXPORT PointerProperties {
  PointerProperties();
...
class CRUI_EXPORT MotionEventGeneric : public MotionEvent {
 public:
  ...
  cr::TimeTicks GetEventTime() const override;
  size_t GetHistorySize() const override;
  cr::TimeTicks GetHistoricalEventTime(
  ...
  void set_event_time(cr::TimeTicks event_time) { event_time_ = event_time; }
 ...
 private:
  ...
  Action action_;
  cr::TimeTicks event_time_;
  uint32_t unique_event_id_;
  int action_index_;
  int button_state_;
  int flags_;
  cr::StackVector<PointerProperties, kTypicalMaxPointerCount> pointers_;
