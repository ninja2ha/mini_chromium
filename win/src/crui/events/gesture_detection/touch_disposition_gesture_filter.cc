...
TouchDispositionGestureFilter::OnGesturePacket(
  ...
  if (!Tail().empty()) {
    CR_DCHECK((packet.gesture_source() == GestureEventDataPacket::TOUCH_TIMEOUT)
               || (packet.unique_touch_event_id() !=
...
void TouchDispositionGestureFilter::OnTouchEventAck(
  ...
  if (!Tail().empty() &&
    ...
    CR_DCHECK(!Head().empty());
    CR_DCHECK(Head().front().unique_touch_event_id() == unique_touch_event_id);
...
void TouchDispositionGestureFilter::SendAckedEvents() {
  ...
  while (!IsEmpty() && (!Head().empty() || sequences_.size() != 1)) {
    ...
    CR_DCHECK(sequence.front().gesture_source() !=
  ...
  CR_DCHECK(touch_packet_for_current_ack_handled);
}
...
void TouchDispositionGestureFilter::FilterAndSendPacket(
  ...
  for (size_t i = 0; i < packet.gesture_count(); ++i) {
    ...
    CR_DCHECK(gesture.details.type() >= ET_GESTURE_TYPE_START);
    CR_DCHECK(gesture.details.type() <= ET_GESTURE_TYPE_END);
    ...
    if (packet.gesture_source() == GestureEventDataPacket::TOUCH_TIMEOUT) {
      ...
      // packet.
      CR_DCHECK(1U == packet.gesture_count());
      return;
    }
...
void TouchDispositionGestureFilter::SendGesture(
  ...
  CR_DCHECK(event.unique_touch_event_id ==
  ...
  switch (event.type()) {
    case ET_GESTURE_LONG_TAP:
      ...
      break;
    case ET_GESTURE_TAP_DOWN:
      CR_DCHECK(!needs_tap_ending_event_);
    ...
    case ET_GESTURE_TAP:
      ...
      if (needs_show_press_event_) {
        SendGesture(GestureEventData(ET_GESTURE_SHOW_PRESS, event),
                    packet_being_sent);
        CR_DCHECK(!needs_show_press_event_);
      }
      needs_tap_ending_event_ = false;
      break;
    case ET_GESTURE_TAP_CANCEL:
...
void TouchDispositionGestureFilter::CancelFlingIfNecessary(
  ...
  SendGesture(CreateGesture(ET_SCROLL_FLING_CANCEL,
                            ...
                            packet_being_sent),
              packet_being_sent);
  CR_DCHECK(!needs_fling_ending_event_);
}
...
void TouchDispositionGestureFilter::EndScrollIfNecessary(
  ...
  SendGesture(CreateGesture(ET_GESTURE_SCROLL_END,
                            ...
                            packet_being_sent),
              packet_being_sent);
  CR_DCHECK(!needs_scroll_ending_event_);
}
