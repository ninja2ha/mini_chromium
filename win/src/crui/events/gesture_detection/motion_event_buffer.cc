...
#include "crui/events/gesture_detection/motion_event_buffer.h"
...
bool CanAddSample(const MotionEvent& event0, const MotionEvent& event1) {
  CR_DCHECK(event0.GetAction() == MotionEvent::Action::MOVE);
...
PointerProperties ResamplePointer(const MotionEvent& event0,
                                  ...
                                  float alpha) {
  CR_DCHECK(event0.GetPointerId(event0_pointer_index) ==
...
std::unique_ptr<MotionEventGeneric> ResampleMotionEvent(
    ...
    const MotionEvent& event1,
    cr::TimeTicks resample_time) {
  CR_DCHECK(MotionEvent::Action::MOVE == event0.GetAction());
  CR_DCHECK(event0.GetPointerCount() == event1.GetPointerCount());

  const cr::TimeTicks time0 = event0.GetEventTime();
  const cr::TimeTicks time1 = event1.GetEventTime();
  CR_DCHECK(time0 < time1);
  CR_DCHECK(time0 <= resample_time);

  const float alpha = static_cast<float>(
      (resample_time - time0).InMillisecondsF() /
      (time1 - time0).InMillisecondsF());

  std::unique_ptr<MotionEventGeneric> event;
  const size_t pointer_count = event0.GetPointerCount();
  CR_DCHECK(pointer_count == event1.GetPointerCount());
  for (size_t event0_i = 0; event0_i < pointer_count; ++event0_i) {
    int event1_i = event1.FindPointerIndexOfId(event0.GetPointerId(event0_i));
    CR_DCHECK(event1_i != -1);
...
std::unique_ptr<MotionEventGeneric> ConsumeSamplesAndTryResampling(
  ...
  const crui::MotionEvent* event0 = nullptr;
  const crui::MotionEvent* event1 = nullptr;
  if (next) {
    CR_DCHECK(resample_time < next->GetEventTime());
    ...
    const cr::TimeTicks time1 = event1->GetEventTime();
    cr::TimeTicks max_predict =
        time1 +
        std::min((event1->GetEventTime() - event0->GetEventTime()) / 2,
                 cr::TimeDelta::FromMilliseconds(kResampleMaxPredictionMs));
  ...
  const cr::TimeTicks time0 = event0->GetEventTime();
  const cr::TimeTicks time1 = event1->GetEventTime();
  cr::TimeDelta delta = time1 - time0;
  if (delta < cr::TimeDelta::FromMilliseconds(kResampleMinDeltaMs)) {
...
void MotionEventBuffer::OnMotionEvent(const MotionEvent& event) {
  CR_DCHECK(0U == event.GetHistorySize());
  if (event.GetAction() != MotionEvent::Action::MOVE) {
    last_extrapolated_event_time_ = cr::TimeTicks();
  ...
  if (!last_extrapolated_event_time_.is_null()) {
    CR_DCHECK(buffered_events_.empty());
  ...
  if (CanAddSample(*buffered_events_.front(), *clone)) {
    ...
    CR_DCHECK(buffered_events_.back()->GetEventTime() <= clone->GetEventTime());
  } else {
    FlushWithoutResampling(std::move(buffered_events_));
  }
...
void MotionEventBuffer::Flush(cr::TimeTicks frame_time) {
  if (buffered_events_.empty())
    return;
  ...
  if (resample_)
    frame_time -= cr::TimeDelta::FromMilliseconds(kResampleLatencyMs);
...
void MotionEventBuffer::FlushWithResampling(MotionEventVector events,
                                            cr::TimeTicks resample_time) {
  CR_DCHECK(!events.empty());
  cr::TimeTicks original_event_time = events.back()->GetEventTime();
