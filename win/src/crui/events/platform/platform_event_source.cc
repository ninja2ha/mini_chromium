...
#include "crui/events/platform/platform_event_dispatcher.h"
#include "crui/events/platform/platform_event_observer.h"
...
cr::LazyInstance<cr::ThreadLocalPointer<PlatformEventSource>>::Leaky
...
PlatformEventSource::PlatformEventSource()
  ...
  CR_CHECK(!lazy_tls_ptr.Pointer()->Get())
      << "Only one platform event source can be created.";
  lazy_tls_ptr.Pointer()->Set(this);
}

PlatformEventSource::~PlatformEventSource() {
  CR_CHECK(this == lazy_tls_ptr.Pointer()->Get());
...
void PlatformEventSource::OnOverriddenDispatcherRestored() {
  CR_CHECK(overridden_dispatcher_);
