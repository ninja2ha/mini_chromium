...
void SetModifierState(BYTE* keyboard_state, int flags) {
  ...
  CR_DCHECK((flags & ~(EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_ALT_DOWN |
                       EF_CAPS_LOCK_ON)) ==
            0);
}
...
const int kModifierFlagsCombinations = (1 << cr::size(modifier_flags)) - 1;

int GetModifierFlags(int combination) {
  int flags = EF_NONE;
  for (size_t i = 0; i < cr::size(modifier_flags); ++i) {
    if (combination & (1 << i))
...
PlatformKeyMap* PlatformKeyMap::GetThreadLocalPlatformKeyMap() {
  ...
  // in the same process.
  static cr::LazyInstance<cr::ThreadLocalOwnedPointer<PlatformKeyMap>>::
  ...
  if (!platform_key_map) {
    auto new_platform_key_map = cr::WrapUnique(new PlatformKeyMap);
  ...
  for (int modifier_combination = 0;
    ...
    for (int key_code = 0; key_code <= 0xFF; ++key_code) {
      ...
      int rv = ::ToUnicodeEx(key_code, 0, keyboard_state, translated_chars,
                             static_cast<int>(cr::size(translated_chars)), 
                             0, keyboard_layout_);
