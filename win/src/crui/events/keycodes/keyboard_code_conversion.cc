...
#include "crui/events/keycodes/dom_us_layout_data.h"
...
cr::char16 DomCodeToUsLayoutCharacter(DomCode dom_code, int flags) {
  DomKey dom_key;
...
bool DomCodeToUsLayoutDomKey(DomCode dom_code,
  ...
  for (const auto& it : kPrintableCodeMap) {
    if (it.dom_code == dom_code) {
      ...
      cr::char16 ch = it.character[state];
...
KeyboardCode DomCodeToUsLayoutKeyboardCode(DomCode dom_code) {
  const DomCodeToKeyboardCodeEntry* end =
      kDomCodeToKeyboardCodeMap + cr::size(kDomCodeToKeyboardCodeMap);
