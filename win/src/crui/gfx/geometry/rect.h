...
CRUI_EXPORT Rect operator+(const Rect& lhs, const Vector2d& rhs);
CRUI_EXPORT Rect operator-(const Rect& lhs, const Vector2d& rhs);
...
CRUI_EXPORT Rect IntersectRects(const Rect& a, const Rect& b);
CRUI_EXPORT Rect UnionRects(const Rect& a, const Rect& b);
CRUI_EXPORT Rect SubtractRects(const Rect& a, const Rect& b);
...
CRUI_EXPORT Rect BoundingRect(const Point& p1, const Point& p2);
...
inline Rect ScaleToEnclosingRect(const Rect& rect,
  ...
  CR_DCHECK(cr::IsValueInRangeForNumericType<int>(
...
inline Rect ScaleToEnclosingRectSafe(const Rect& rect,
  ...
  if (x_scale == 1.f && y_scale == 1.f)
    return rect;
  int x = cr::saturated_cast<int>(std::floor(rect.x() * x_scale));
  int y = cr::saturated_cast<int>(std::floor(rect.y() * y_scale));
  int w = cr::saturated_cast<int>(std::ceil(rect.width() * x_scale));
  int h = cr::saturated_cast<int>(std::ceil(rect.height() * y_scale));
  return Rect(x, y, w, h);
}
...
inline Rect ScaleToRoundedRect(const Rect& rect, float x_scale, float y_scale) {
  ...
  CR_DCHECK(
      cr::IsValueInRangeForNumericType<int>(std::round(rect.x() * x_scale)));
  CR_DCHECK(
      cr::IsValueInRangeForNumericType<int>(std::round(rect.y() * y_scale)));
  CR_DCHECK(cr::IsValueInRangeForNumericType<int>(
