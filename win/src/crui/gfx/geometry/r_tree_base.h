...
class CRUI_EXPORT RTreeBase {
 protected:
  class NodeBase;
  class RecordBase;

  typedef std::vector<const RecordBase*> Records;
  typedef std::vector<std::unique_ptr<NodeBase>> Nodes;
  ...
  class CRUI_EXPORT NodeBase {
   public:
    ...
    virtual std::unique_ptr<NodeBase> RemoveAndReturnLastChild() = 0;
  ...
  class CRUI_EXPORT RecordBase : public NodeBase {
   public:
    RecordBase(const RecordBase&) = delete;
    RecordBase& operator=(const RecordBase&) = delete;
    ...
    std::unique_ptr<NodeBase> RemoveAndReturnLastChild() override;
  ...
  class CRUI_EXPORT Node : public NodeBase {
   public:
    ...
    std::unique_ptr<Node> ConstructParent();
    ...
    std::unique_ptr<NodeBase> RemoveChild(NodeBase* child_node, Nodes* orphans);
    ...
    size_t AddChild(std::unique_ptr<NodeBase> node);
    ...
    std::unique_ptr<NodeBase> Split(size_t min_children, size_t max_children);

    size_t count() const { return children_.size(); }
    const NodeBase* child(size_t i) const { return children_[i].get(); }
    NodeBase* child(size_t i) { return children_[i].get(); }
   ...
   private:
    ...
    std::unique_ptr<NodeBase> DivideChildren(
        const Rects& low_bounds,
        const Rects& high_bounds,
  ...
  void InsertNode(std::unique_ptr<NodeBase> node, int* highest_reinsert_level);

  // Removes |node| from the tree without deleting it.
  std::unique_ptr<NodeBase> RemoveNode(NodeBase* node);
