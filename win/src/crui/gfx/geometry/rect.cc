...
static void SaturatedClampRange(int min, int max, int* origin, int* span) {
  ...
  int effective_span = cr::ClampSub(max, min);
  int span_loss = cr::ClampSub(max, min + effective_span);
  ...
  if (cr::SafeUnsignedAbs(max) < kMaxDimension) {
    // Maintain origin + span == max.
    *span = effective_span;
    *origin = max - effective_span;
  } else if (cr::SafeUnsignedAbs(min) < kMaxDimension) {
    // Maintain origin == min.
    *span = effective_span;
    *origin = min;
  } else {
...
void Rect::Inset(int left, int top, int right, int bottom) {
  ...
  // overflow as well.
  set_width(cr::ClampSub(width(), cr::ClampAdd(left, right)));
  set_height(cr::ClampSub(height(), cr::ClampAdd(top, bottom)));
}
