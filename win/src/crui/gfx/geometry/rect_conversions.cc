...
Rect ToNearestRect(const RectF& rect) {
  ...
  CR_DCHECK(std::abs(min_x - float_min_x) < 0.01f);
  CR_DCHECK(std::abs(min_y - float_min_y) < 0.01f);
  CR_DCHECK(std::abs(max_x - float_max_x) < 0.01f);
  CR_DCHECK(std::abs(max_y - float_max_y) < 0.01f);
