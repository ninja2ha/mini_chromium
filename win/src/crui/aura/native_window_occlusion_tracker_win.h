...
class CRUI_EXPORT NativeWindowOcclusionTrackerWin : public WindowObserver {
 public:
 ...
 private:
  ...
  class WindowOcclusionCalculator {
   public:
    using UpdateOcclusionStateCallback = cr::RepeatingCallback<void(
    ...
    // thread.
    static void DeleteInstanceForTesting(cr::WaitableEvent* done_event);
    
    WindowOcclusionCalculator(const WindowOcclusionCalculator&) = delete;
    WindowOcclusionCalculator& operator=(
        const WindowOcclusionCalculator&) = delete;
   ...
   private:
    ...
    cr::flat_map<HWND, Window::OcclusionState>
    ...
    cr::flat_map<DWORD, HWINEVENTHOOK> process_event_hooks_;
    ...
    cr::flat_set<DWORD> pids_for_location_change_hook_;

    // Timer to delay occlusion update.
    cr::OneShotTimer occlusion_update_timer_;
    ...
    cr::WeakPtrFactory<WindowOcclusionCalculator> weak_factory_{this};
  };
  ...
  void UpdateOcclusionState(const cr::flat_map<HWND, Window::OcclusionState>&
  ...
  const cr::RefPtr<cr::SequencedTaskRunner> update_occlusion_task_runner_;
