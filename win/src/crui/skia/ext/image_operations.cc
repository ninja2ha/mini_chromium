...
void ResizeFilter::ComputeFilters(int src_size,
  ...
  cr::StackVector<float, 64> filter_values;
  cr::StackVector<int16_t, 64> fixed_filter_values;
  ...
  for (int dest_subset_i = dest_subset_lo; dest_subset_i < dest_subset_hi;
    ...
    CR_DCHECK(!filter_values->empty()) << "We should always get a filter!";
...
SkBitmap ImageOperations::Resize(const SkBitmap& source,
  ...
  cr::TimeTicks resize_start = cr::TimeTicks::Now();

  SkIRect dest = { 0, 0, dest_width, dest_height };
  CR_DCHECK(dest.contains(dest_subset)) <<
  ...
  cr::TimeDelta delta = cr::TimeTicks::Now() - resize_start;
