...
#ifndef MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCED_TASK_RUNNER_H_
#define MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCED_TASK_RUNNER_H_
...
#include "cr_event/task/sequenced_task_runner_helpers.h"
...
class CREVENT_EXPORT SequencedTaskRunner : public TaskRunner {
 public:
  ...
  template <class T>
  void ReleaseSoon(const Location& from_here, RefPtr<T>&& object) {
    if (!object)
      return;
...
struct CREVENT_EXPORT OnTaskRunnerDeleter {
  ...
  void operator()(const T* ptr) {
    if (ptr)
      task_runner_->DeleteSoon(CR_FROM_HERE, ptr);
  }
...
#endif  // MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCED_TASK_RUNNER_H_