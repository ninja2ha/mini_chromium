...
void AssociatedThreadId::BindToCurrentThread() {
  ...
  CR_DCHECK(prev_thread_id == current_thread_id ||
            prev_thread_id == kInvalidThreadId);

  // Rebind the thread and sequence checkers to the current thread/sequence.
  CR_DETACH_FROM_THREAD(thread_checker);
  CR_DCHECK_CALLED_ON_VALID_THREAD(thread_checker);

  CR_DETACH_FROM_SEQUENCE(sequence_checker);
  CR_DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker);
}
