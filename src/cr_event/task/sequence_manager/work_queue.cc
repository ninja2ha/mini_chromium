...
WorkQueue::~WorkQueue() {
  CR_DCHECK(!work_queue_sets_) << task_queue_->GetName() << " : "
...
bool WorkQueue::GetFrontTaskEnqueueOrder(EnqueueOrder* enqueue_order) const {
  if (tasks_.empty() || BlockedByFence())
    return false;
  // Quick sanity check.
  CR_DCHECK(tasks_.front().enqueue_order() <= tasks_.back().enqueue_order())
...
void WorkQueue::Push(Task task) {
  ...
  CR_DCHECK(was_empty || tasks_.back().enqueue_order() < task.enqueue_order());
...
void WorkQueue::TaskPusher::Push(Task* task) {
  ...
  // Make sure the |enqueue_order()| is monotonically increasing.
  CR_DCHECK(work_queue_->tasks_.empty() ||
...
void WorkQueue::PushNonNestableTaskToFront(Task task) {
  CR_DCHECK(task.nestable == Nestable::kNonNestable);
  ...
  if (!was_empty) {
    // Make sure the |enqueue_order| is monotonically increasing.
    CR_DCHECK(task.enqueue_order() <= tasks_.front().enqueue_order())
        << task_queue_->GetName() << " : " << work_queue_sets_->GetName()
        << " : " << name_;
  }
...
bool WorkQueue::InsertFenceImpl(EnqueueOrder fence) {
  CR_DCHECK(fence != 0u);
  CR_DCHECK(fence >= fence_ || fence == EnqueueOrder::blocking_fence());
...
void WorkQueue::InsertFenceSilently(EnqueueOrder fence) {
  ...
  CR_DCHECK(!fence_ || fence_ == EnqueueOrder::blocking_fence());
...
bool WorkQueue::ShouldRunBefore(const WorkQueue* other_queue) const {
  CR_DCHECK(!tasks_.empty());
  CR_DCHECK(!other_queue->tasks_.empty());
