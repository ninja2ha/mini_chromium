...
#ifndef MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_TASK_QUEUE_H_
#define MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_TASK_QUEUE_H_
...
#include "cr_event/task/sequence_manager/lazy_now.h"
#include "cr_event/task/sequence_manager/tasks.h"
...
class CREVENT_EXPORT TaskQueue : public RefCountedThreadSafe<TaskQueue> {
 public:
  class Observer {
   public:
  ...
  class CREVENT_EXPORT TaskTiming {
   public:
    ...
    cr::TimeTicks start_time() const {
      CR_DCHECK(has_wall_time());
      return start_time_;
    }
    cr::TimeTicks end_time() const {
      CR_DCHECK(has_wall_time());
      return end_time_;
    }
    cr::TimeDelta wall_duration() const {
      CR_DCHECK(has_wall_time());
      return end_time_ - start_time_;
    }
    cr::ThreadTicks start_thread_time() const {
      CR_DCHECK(has_thread_time());
      return start_thread_time_;
    }
    cr::ThreadTicks end_thread_time() const {
      CR_DCHECK(has_thread_time());
      return end_thread_time_;
    }
    cr::TimeDelta thread_duration() const {
      CR_DCHECK(has_thread_time());
   ...
   protected:
    ...
    cr::TimeTicks end_time_;
    cr::ThreadTicks start_thread_time_;
    cr::ThreadTicks end_thread_time_;
  };

  // An interface that lets the owner vote on whether or not the associated
  // TaskQueue should be enabled.
  class CREVENT_EXPORT QueueEnabledVoter {
   public:
    ~QueueEnabledVoter();
   ...
   private:
    ...
    explicit QueueEnabledVoter(RefPtr<TaskQueue> task_queue);

    RefPtr<TaskQueue> const task_queue_;
    bool enabled_;
  };
  ...
  RefPtr<SingleThreadTaskRunner> CreateTaskRunner(TaskType task_type);

  // Default task runner which doesn't annotate tasks with a task type.
  const RefPtr<SingleThreadTaskRunner>& task_runner() const {
  ...
  cr::WeakPtr<TaskQueue> AsWeakPtr() {
 ...
 private:
  ...
  // a lock.
  mutable cr::internal::CheckedLock impl_lock_{
      cr::internal::UniversalPredecessor{}};
...
#endif  // MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_TASK_QUEUE_H_
