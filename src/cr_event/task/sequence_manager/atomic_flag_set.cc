...
AtomicFlagSet::~AtomicFlagSet() {
  CR_DCHECK(!alloc_list_head_);
  CR_DCHECK(!partially_free_list_head_);
}
...
void AtomicFlagSet::AtomicFlag::ReleaseAtomicFlag() {
  ...
  CR_DCHECK_CALLED_ON_VALID_THREAD(outer_->associated_thread_->thread_checker);
  SetActive(false);
  ...
  CR_DCHECK(!group_->flag_callbacks[index].is_null());
...
AtomicFlagSet::AtomicFlag AtomicFlagSet::AddFlag(RepeatingClosure callback) {
  ...
  CR_DCHECK(!group->flag_callbacks[first_unoccupied_index]);
...
AtomicFlagSet::Group::~Group() {
  CR_DCHECK(allocated_flags == 0u);
  CR_DCHECK(!partially_free_list_prev);
  CR_DCHECK(!partially_free_list_next);
}
...
int AtomicFlagSet::Group::FindFirstUnallocatedFlag() const {
  ...
  CR_DCHECK(unallocated_flags != 0u);
  int index = IndexOfFirstFlagSet(unallocated_flags);
  CR_DCHECK(index < kNumFlags);
  return index;
}
...
void AtomicFlagSet::AddToPartiallyFreeList(Group* group) {
  CR_DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  CR_DCHECK(partially_free_list_head_ != group);
  CR_DCHECK(!group->partially_free_list_prev);
  CR_DCHECK(!group->partially_free_list_next);
  if (partially_free_list_head_)
