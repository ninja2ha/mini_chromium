...
#include "cr_event/task/sequence_manager/sequence_manager_impl.h"
...
#include "cr_event/task/sequence_manager/real_time_domain.h"
#include "cr_event/task/sequence_manager/task_time_observer.h"
#include "cr_event/task/sequence_manager/thread_controller_impl.h"
#include "cr_event/task/sequence_manager/thread_controller_with_message_pump_impl.h"
#include "cr_event/task/sequence_manager/work_queue.h"
#include "cr_event/task/sequence_manager/work_queue_sets.h"
#include "cr_base/threading/thread_id_name_manager.h"
#include "cr_base/threading/thread_local.h"
#include "cr_base/time/default_tick_clock.h"
#include "cr_base/time/tick_clock.h"
...
cr::ThreadLocalPointer<internal::SequenceManagerImpl>*
...
using TimeRecordingPolicy =
    cr::sequence_manager::TaskQueue::TaskTiming::TimeRecordingPolicy;
...
SequenceManager::MetricRecordingSettings InitializeMetricRecordingSettings(
  ...
  bool records_cpu_time_for_each_task =
      cr::RandDouble() < kThreadSamplingRateForRecordingCPUTime;
...
class SequenceManagerImpl::NativeWorkHandleImpl final
 ...
 public:
  ...
  ~NativeWorkHandleImpl() final {
    ...
    // to yield.
    CR_DCHECK(
        sequence_manager_->main_thread_only().pending_native_work.size() >=
        1u);
...
SequenceManagerImpl::~SequenceManagerImpl() {
  ...
  // fine
  CR_DCHECK(!controller_->GetBoundMessagePump() ||
  ...
  if (GetMessagePump()) {
    CR_DCHECK(this == GetTLSSequenceManagerImpl()->Get());
    GetTLSSequenceManagerImpl()->Set(nullptr);
  }
}

SequenceManagerImpl::MainThreadOnly::MainThreadOnly(
    const RefPtr<AssociatedThreadId>& associated_thread,
...
void SequenceManagerImpl::CompleteInitializationOnBoundThread() {
  ...
  if (GetMessagePump()) {
    CR_DCHECK(!GetTLSSequenceManagerImpl()->Get())
...
SequenceManagerImpl::CreateTaskQueueImpl(const TaskQueue::Spec& spec) {
  ...
  CR_DCHECK(main_thread_only().time_domains.find(time_domain) !=
...
void SequenceManagerImpl::SetAddQueueTimeToTasks(bool enable) {
  cr::subtle::NoBarrier_Store(&add_queue_time_to_tasks_, enable ? 1 : 0);
}

bool SequenceManagerImpl::GetAddQueueTimeToTasks() {
  return cr::subtle::NoBarrier_Load(&add_queue_time_to_tasks_);
}
...
void SequenceManagerImpl::OnExitNestedRunLoop() {
  main_thread_only().nesting_depth--;
  CR_DCHECK(main_thread_only().nesting_depth >= 0);
...
void SequenceManagerImpl::LogTaskDebugInfo(
  ...
  switch (settings_.task_execution_logging) {
    ...
    case Settings::TaskLogging::kEnabled:
      CR_LOG(Info) << "#" << static_cast<uint64_t>(task->enqueue_order()) << " "
...
Task* SequenceManagerImpl::SelectNextTaskImpl(SelectTaskOption option) {
  ...
  while (true) {
    ...
    if (CR_UNLIKELY(work_queue->RemoveAllCanceledTasksFromFront()))
      continue;

    if (CR_UNLIKELY(work_queue->GetFrontTask()->nestable ==
    ...
    if (CR_UNLIKELY(!ShouldRunTaskOfPriority(
...
TimeDelta SequenceManagerImpl::DelayTillNextTask(
  ...
  if (auto priority =
    ...
    // more important.
    if (CR_UNLIKELY(!ShouldRunTaskOfPriority(*priority)))
...
void SequenceManagerImpl::OnTaskQueueEnabled(internal::TaskQueueImpl* queue) {
  CR_DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  CR_DCHECK(queue->IsQueueEnabled());
...
bool SequenceManagerImpl::ShouldRecordCPUTimeForTask() {
  CR_DCHECK(ThreadTicks::IsSupported() ||
...
RefPtr<TaskQueue> SequenceManagerImpl::CreateTaskQueue(
...
RefPtr<SingleThreadTaskRunner> SequenceManagerImpl::GetTaskRunner() {
...
CR_NOINLINE bool SequenceManagerImpl::Validate() {
...
///  CR_DCHECK(!main_thread_only().async_stack_crash_key);
///#if !defined(OS_NACL)
