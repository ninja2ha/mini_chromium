...
#include "cr_event/threading/thread_checker_impl.h"
...
RefPtr<SingleThreadTaskRunner> CreateNullTaskRunner() {
...
void TaskQueue::RemoveQueueEnabledVoter(bool voter_is_enabled) {
  ...
  if (voter_is_enabled) {
    --enabled_voter_count_;
    CR_DCHECK(enabled_voter_count_ >= 0);
  }

  --voter_count_;
  CR_DCHECK(voter_count_ >= 0);
...
void TaskQueue::OnQueueEnabledVoteChanged(bool enabled) {
  ...
  if (enabled) {
    ++enabled_voter_count_;
    CR_DCHECK(enabled_voter_count_ <= voter_count_);
  } else {
    --enabled_voter_count_;
    CR_DCHECK(enabled_voter_count_ >= 0);
  }
...
void TaskQueue::ShutdownTaskQueueGracefully() {
  // RefPtr guarantees us that this object isn't used.
  if (!impl_)
    return;
  ...
  impl_ptr->sequence_manager()->ShutdownTaskQueueGracefully(TakeTaskQueueImpl());
  CR_DCHECK(impl_ == nullptr);
}
...
void TaskQueue::TaskTiming::RecordTaskStart(LazyNow* now) {
  CR_DCHECK(State::NotStarted == state_);
  state_ = State::Running;

  if (has_wall_time())
    start_time_ = now->Now();
  if (has_thread_time())
    start_thread_time_ = cr::ThreadTicks::Now();
}

void TaskQueue::TaskTiming::RecordTaskEnd(LazyNow* now) {
  CR_DCHECK(state_ == State::Running || state_ == State::Finished);
  ...
  if (has_wall_time())
    end_time_ = now->Now();
  if (has_thread_time())
    end_thread_time_ = cr::ThreadTicks::Now();
}
...
RefPtr<SingleThreadTaskRunner> TaskQueue::CreateTaskRunner(
    TaskType task_type) {
  // We only need to lock if we're not on the main thread.
  cr::internal::CheckedAutoLockMaybe lock(IsOnMainThread() ? &impl_lock_
                                                             : nullptr);
  if (!impl_)
...
std::unique_ptr<internal::TaskQueueImpl> TaskQueue::TakeTaskQueueImpl() {
  cr::internal::CheckedAutoLock lock(impl_lock_);
  CR_DCHECK(impl_);
