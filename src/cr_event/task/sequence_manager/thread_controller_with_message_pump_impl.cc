...
///#include "cr_base/trace_event/base_tracing.h"
...
ThreadControllerWithMessagePumpImpl::CreateUnbound(
    const SequenceManager::Settings& settings) {
  return cr::WrapUnique(new ThreadControllerWithMessagePumpImpl(settings));
}
...
void ThreadControllerWithMessagePumpImpl::BindToCurrentThread(
  ...
  {
    cr::internal::CheckedAutoLock task_runner_lock(task_runner_lock_);
    if (task_runner_)
...
void ThreadControllerWithMessagePumpImpl::SetTimerSlack(
  ...
  CR_DCHECK(RunsTasksInCurrentSequence());
...
void ThreadControllerWithMessagePumpImpl::ScheduleWork() {
  cr::internal::CheckedLock::AssertNoLockHeldOnCurrentThread();
...
void ThreadControllerWithMessagePumpImpl::SetNextDelayedDoWork(
    LazyNow* lazy_now,
    TimeTicks run_time) {
  CR_DCHECK(lazy_now->Now() < run_time);
...
void ThreadControllerWithMessagePumpImpl::SetDefaultTaskRunner(
  ...
  cr::internal::CheckedAutoLock lock(task_runner_lock_);
...
void ThreadControllerWithMessagePumpImpl::MaybeStartHangWatchScopeEnabled() {
  ...
  ///    cr::HangWatcher::IsEnabled()) {
  ///  hang_watch_scope_.emplace(
  ///      cr::HangWatchScopeEnabled::kDefaultHangWatchTime);
  ///}
}
...
void ThreadControllerWithMessagePumpImpl::AddNestingObserver(
  ...
  CR_DCHECK(!main_thread_only().nesting_observer);
  CR_DCHECK(observer);
...
void ThreadControllerWithMessagePumpImpl::RemoveNestingObserver(
    RunLoop::NestingObserver* observer) {
  CR_DCHECK(main_thread_only().nesting_observer == observer);
...
TimeDelta ThreadControllerWithMessagePumpImpl::DoWorkImpl(
  ...
  CR_DCHECK(do_work_delay >= TimeDelta());
...
void ThreadControllerWithMessagePumpImpl::Run(bool application_tasks_allowed,
  ...
  if (application_tasks_allowed && !main_thread_only().task_execution_allowed) {
    ...
    CR_DCHECK(RunLoop::IsNestedOnCurrentThread());
