...
#ifndef MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_TASK_QUEUE_IMPL_H_
#define MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_TASK_QUEUE_IMPL_H_
...
#include "cr_event/observer_list.h"
#include "cr_event/task/pending_task.h"
#include "cr_event/task/common/checked_lock.h"
#include "cr_event/task/common/intrusive_heap.h"
#include "cr_event/task/common/operations_controller.h"
#include "cr_event/task/sequence_manager/associated_thread_id.h"
#include "cr_event/task/sequence_manager/atomic_flag_set.h"
#include "cr_event/task/sequence_manager/enqueue_order.h"
#include "cr_event/task/sequence_manager/lazily_deallocated_deque.h"
#include "cr_event/task/sequence_manager/sequenced_task_source.h"
#include "cr_event/task/sequence_manager/task_queue.h"
...
class CREVENT_EXPORT TaskQueueImpl {
 public:
  ...
  RefPtr<SingleThreadTaskRunner> CreateTaskRunner(
  ...
  cr::internal::HeapHandle heap_handle() const {
    return main_thread_only().heap_handle;
  }

  void set_heap_handle(cr::internal::HeapHandle heap_handle) {
 ...
 private:
  ...
  class GuardedTaskPoster : public RefCountedThreadSafe<GuardedTaskPoster> {
   ...
   private:
    ...
    cr::internal::OperationsController operations_controller_;
  ...
  class TaskRunner final : public SingleThreadTaskRunner {
   public:
    explicit TaskRunner(RefPtr<GuardedTaskPoster> task_poster,
   ...
   private:
    ...
    const RefPtr<GuardedTaskPoster> task_poster_;
    const RefPtr<AssociatedThreadId> associated_thread_;
  ...
  struct MainThreadOnly {
    ...
    cr::internal::HeapHandle heap_handle;
  ...
  using TaskDeque =
      LazilyDeallocatedDeque<Task, TimeTicks::Now>;
...
#endif  // MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_TASK_QUEUE_IMPL_H_
