...
#include "cr_event/task/sequence_manager/task_queue_impl.h"
...
#include "cr_event/task/common/scoped_defer_task_posting.h"
#include "cr_event/task/sequence_manager/sequence_manager_impl.h"
#include "cr_event/task/sequence_manager/time_domain.h"
#include "cr_event/task/sequence_manager/work_queue.h"
#include "cr_event/task/task_observer.h"
...
TaskQueueImpl::TaskRunner::TaskRunner(
    RefPtr<GuardedTaskPoster> task_poster,
...
TaskQueueImpl::~TaskQueueImpl() {
  ...
  // queues.
  CR_DCHECK(any_thread_.unregistered)
...
RefPtr<SingleThreadTaskRunner> TaskQueueImpl::CreateTaskRunner(
...
void TaskQueueImpl::UnregisterTaskQueue() {
  ...
  // It is possible for a task to hold a RefPtr to this, which
...
void TaskQueueImpl::MaybeLogPostTask(PostedTask* task) {
  ...
  CR_LOG(Info) << name_ << " PostTask " 
               << task->location.ToString() << " delay "
               << task->delay;
...
void TaskQueueImpl::PostDelayedTaskImpl(PostedTask task,
  ...
  // Use CHECK instead of DCHECK to crash earlier. See http://crbug.com/711167
  // for details.
  CR_CHECK(task.callback);
  CR_DCHECK(task.delay > TimeDelta());
...
void TaskQueueImpl::PushOntoDelayedIncomingQueue(Task pending_task) {
  ...
  PostImmediateTaskImpl(
      PostedTask(std::move(task_runner),
                 ...
                 CR_FROM_HERE, TimeDelta(), Nestable::kNonNestable, task_type),
...
void TaskQueueImpl::ReloadEmptyImmediateWorkQueue() {
  CR_DCHECK(main_thread_only().immediate_work_queue->Empty());
...
TaskQueue::QueuePriority TaskQueueImpl::GetQueuePriority() const {
  ...
  CR_DCHECK(set_index == delayed_work_queue()->work_queue_set_index());
...
void TaskQueueImpl::NotifyWillProcessTask(const Task& task,
  ...
  CR_DCHECK(should_notify_observers_);
...
TimeDomain* TaskQueueImpl::GetTimeDomain() const {
  CR_DCHECK(associated_thread_->IsBoundToCurrentThread() ||
...
void TaskQueueImpl::InsertFenceAt(TimeTicks time) {
  CR_DCHECK(delayed_fence_allowed_)
...
void TaskQueueImpl::RequeueDeferredNonNestableTask(
  ...
  CR_DCHECK(task.task.nestable == Nestable::kNonNestable);
...
void TaskQueueImpl::SetObserver(TaskQueue::Observer* observer) {
  if (observer) {
    CR_DCHECK(!main_thread_only().task_queue_observer)
        << "Can't assign two different observers to "
           "cr::sequence_manager:TaskQueue";
  }
...
void TaskQueueImpl::SetOnTaskStartedHandler(
  ...
  CR_DCHECK(should_notify_observers_ || handler.is_null());
