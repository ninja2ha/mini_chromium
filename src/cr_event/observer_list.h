...
#include "cr_event/internal/observer_list_internal.h"
...
//     ...
//
//     class Observer : public cr::CheckedObserver {
//      public:
...
class ObserverList {
 public:
  ...
  class Iter {
   public:
    ...
    explicit Iter(const ObserverList* list)
      ...
      CR_DCHECK(list);
      CR_DCHECK(allow_reentrancy || list_.IsOnlyRemainingNode());
   ...
   private:
    ...
    ObserverType* GetCurrent() const {
      CR_DCHECK(list_);
      CR_DCHECK(index_ < clamped_max_index());
  ...
  explicit ObserverList(ObserverListPolicy policy = ObserverListPolicy::ALL)
    ...
    CR_DETACH_FROM_SEQUENCE(iteration_sequence_checker_);
  }
  ...
  ~ObserverList() {
    ...
    if (!live_iterators_.empty())
      CR_DCHECK_CALLED_ON_VALID_SEQUENCE(iteration_sequence_checker_);

    while (!live_iterators_.empty())
      live_iterators_.head()->value()->Invalidate();
    if (check_empty) {
      Compact();
      CR_DCHECK(observers_.empty()) << GetObserversCreationStackString();
    }
  }
 ...
 private:
  ...
  cr::LinkedList<internal::WeakLinkNode<ObserverList>> live_iterators_;

  size_t observers_count_{0};

  const ObserverListPolicy policy_;

  cr::SequenceChecker iteration_sequence_checker_;
};
