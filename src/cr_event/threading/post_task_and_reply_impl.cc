...
#include "cr_event/threading/post_task_and_reply_impl.h"
...
class PostTaskAndReplyRelay {
 public:
  ...
  PostTaskAndReplyRelay(const Location& from_here,
                        OnceClosure task,
                        OnceClosure reply,
                        RefPtr<SequencedTaskRunner> reply_task_runner)
  ...
  ~PostTaskAndReplyRelay() {
    ...
    if (!reply_task_runner_) {
      CR_DCHECK(task_.is_null() == reply_.is_null());
      return;
    }
    ...
    if (!reply_task_runner_->RunsTasksInCurrentSequence()) {
      ...
      CR_DVLOG(1) << "~PostTaskAndReplyRelay leaked ptr:" 
                  << relay_to_delete.get();
 ...
 private:
  ...
  RefPtr<SequencedTaskRunner> reply_task_runner_;
};
...
bool PostTaskAndReplyImpl::PostTaskAndReply(const Location& from_here,
                                            OnceClosure task,
                                            OnceClosure reply) {
  CR_DCHECK(task) << from_here.ToString();
  CR_DCHECK(reply) << from_here.ToString();
  ...
  CR_CHECK(has_sequenced_context || !post_task_success);
