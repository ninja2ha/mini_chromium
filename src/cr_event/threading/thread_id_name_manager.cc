...
ThreadLocalStorage::Slot& GetThreadNameTLS() {
  static cr::NoDestructor<cr::ThreadLocalStorage::Slot> thread_name_tls;
...
void ThreadIdNameManager::AddObserver(Observer* obs) {
  AutoLock locked(lock_);
  CR_DCHECK(!cr::contains(observers_, obs));
...
void ThreadIdNameManager::SetName(const std::string& name) {
  ...
  CR_DVLOG(1) << "leaked=> SetCurrentThreadName(" 
              << (void*)(leaked_str->c_str()) << ")";
}
