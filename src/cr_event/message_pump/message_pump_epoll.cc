...
#include "cr_base_runtime/message_pump/message_pump_epoll.h"
...
class MessagePumpEpoll::Interest : public RefCounted<Interest> {
 public:
  ...
  void WatchForControllerDestruction() {
    CR_DCHECK(nested_controller_destruction_watchers_ >= 0);
    if (nested_controller_destruction_watchers_ == 0) {
      CR_DCHECK(!controller_->was_destroyed_);
      ...
      CR_DCHECK(controller_->was_destroyed_ == &was_controller_destroyed_);
    }
...
MessagePumpEpoll::MessagePumpEpoll() {
  ...
  CR_PCHECK(epoll_.is_valid());

  wake_event_.reset(eventfd(0, EFD_NONBLOCK));
  CR_PCHECK(wake_event_.is_valid());
...
void MessagePumpEpoll::Run(Delegate* delegate) {
  ...
  for (;;) {
    ...
    CR_DCHECK(!next_work_info.delayed_run_time.is_null());
...
void MessagePumpEpoll::RecordPeriodicMetrics() {
  ...
  ///                          (int)entries_.size());
  ///next_metrics_time_ += cr::Minutes(1);
}

void MessagePumpEpoll::Quit() {
  CR_DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  CR_DCHECK(run_state_) << "Quit() called outside of Run()";
...
///#endif
  ...
  CR_DCHECK(FindPollEntry(entry.fd) == pollfds_.end());
  struct pollfd poll_entry;
  ...
  if (!entry.stopped) {
    if (events == 0) {
      ...
      if (!(entry.registered_events & EPOLLONESHOT)) {
        ...
        CR_CHECK(poll_entry != pollfds_.end());
        poll_entry->events = 0;
      }
      return;
    }
...
void MessagePumpEpoll::UnregisterInterest(
    const RefPtr<Interest>& interest) {
  CR_DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
bool MessagePumpEpoll::GetEventsPoll(int epoll_timeout,
  ...
  int retval = poll(&pollfds_[0], cr::checked_cast<nfds_t>(pollfds_.size()),
                    epoll_timeout);
  if (retval < 0) {
    CR_DPCHECK(errno == EINTR);
    return false;
...
void MessagePumpEpoll::OnEpollEvent(EpollEventEntry& entry, uint32_t events) {
  ...
  CR_DCHECK(readable || writable || disconnected);
...
MessagePumpEpoll::EpollEventEntry::~EpollEventEntry() {
  if (active_event) {
    CR_DCHECK(this == active_event->data.ptr);
...
MessagePumpEpoll::FdWatchController::~FdWatchController() {
  CR_CHECK(StopWatchingFileDescriptor());
  if (was_destroyed_) {
    CR_DCHECK(!*was_destroyed_);
    *was_destroyed_ = true;
  }
}
...
const RefPtr<MessagePumpEpoll::Interest>&
