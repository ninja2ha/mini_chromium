...
#include "cr_event/message_pump/win/message_pump_win.h"
...
#include "cr_base/numerics/ranges.h"
...
DWORD GetSleepTimeoutMs(TimeTicks next_task_time,
  ...
  // work.
  CR_DCHECK(!next_task_time.is_null());
  ...
  return SaturatedCast<DWORD>(timeout_ms);
}
...
void MessagePumpWin::Run(Delegate* delegate) {
  CR_DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
...
void MessagePumpForUI::ScheduleWork() {
  ...
  const BOOL ret = ::PostMessageW(message_window_.hwnd(), kMsgHaveWork, 0, 0);
  if (ret)
...
void MessagePumpForUI::DoRunLoop() {
  ...
  for (;;) {
    ...
    CR_DCHECK(!in_native_loop_);
    CR_DCHECK(!installed_native_timer_);
    if (run_state_->should_quit)
      break;
...
void MessagePumpForUI::WaitForWork(Delegate::NextWorkInfo next_work_info) {
  ...
  for (DWORD delay = GetSleepTimeoutMs(next_work_info.delayed_run_time,
    ...
    cr::debug::Alias(&delay);
    cr::debug::Alias(&wait_flags);
    ...
    CR_DCHECK(WAIT_FAILED != result) << GetLastError();
  }
}
...
void MessagePumpForUI::ScheduleNativeTimer(
  ...
  CR_DCHECK(!next_work_info.is_immediate());
  CR_DCHECK(in_native_loop_);
  ...
  UINT delay_msec = StrictCast<UINT>(GetSleepTimeoutMs(
...
bool MessagePumpForUI::ProcessNextWindowsMessage() {
  ...
  bool more_work_is_plausible = false;
  {
...

    {
      ...
      ///    });
      has_msg = ::PeekMessageW(&msg, nullptr, 0, 0, PM_REMOVE) != FALSE;
    }
  }
  if (has_msg)
...
bool MessagePumpForUI::ProcessPumpReplacementMessage() {
  ...
  bool have_message = false;
  {
    ...
    have_message = ::PeekMessageW(&msg, nullptr, 0, 0,
...
void MessagePumpForIO::WaitForWork(Delegate::NextWorkInfo next_work_info) {
  ...
  // recursion problems.
  CR_DCHECK(!run_state_->is_nested) << "Cannot nest an IO message loop!";

  DWORD timeout = GetSleepTimeoutMs(next_work_info.delayed_run_time,
                                    next_work_info.recent_now);

  // Tell the optimizer to retain these values to simplify analyzing hangs.
  cr::debug::Alias(&timeout);
