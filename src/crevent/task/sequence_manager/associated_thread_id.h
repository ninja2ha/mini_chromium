...
#ifndef MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_ASSOCIATED_THREAD_ID_H_
#define MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_ASSOCIATED_THREAD_ID_H_
...
// the various methods.
class CREVENT_EXPORT AssociatedThreadId
    : public cr::RefCountedThreadSafe<AssociatedThreadId> {
 public:
  AssociatedThreadId();

  // TODO(eseckler): Replace thread_checker with sequence_checker everywhere.
  CR_THREAD_CHECKER(thread_checker);
  CR_SEQUENCE_CHECKER(sequence_checker);

  static RefPtr<AssociatedThreadId> CreateUnbound() {
    return MakeRefCounted<AssociatedThreadId>();
  }

  static RefPtr<AssociatedThreadId> CreateBound() {
 ...
 private:
  friend class cr::RefCountedThreadSafe<AssociatedThreadId>;
  ~AssociatedThreadId();
...
#endif  // MINI_CHROMIUM_SRC_CREVENT_TASK_SEQUENCE_MANAGER_ASSOCIATED_THREAD_ID_H_
