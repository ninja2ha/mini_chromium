...
void TaskQueueSelector::SetQueuePriority(internal::TaskQueueImpl* queue,
  ...
  CR_DCHECK(priority < TaskQueue::kQueuePriorityCount);
  CR_DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  if (queue->IsQueueEnabled()) {
  ...
  CR_DCHECK(priority == queue->GetQueuePriority());
}
...
void TaskQueueSelector::AddQueueImpl(internal::TaskQueueImpl* queue,
                                     TaskQueue::QueuePriority priority) {
#if CR_DCHECK_IS_ON()
  CR_DCHECK(!CheckContainsQueueForTest(queue));
#endif
...
void TaskQueueSelector::WorkQueueSetBecameEmpty(size_t set_index) {
  ...
  CR_DCHECK(non_empty_set_counts_[set_index] >= 0);
...
void TaskQueueSelector::WorkQueueSetBecameNonEmpty(size_t set_index) {
  non_empty_set_counts_[set_index]++;
  CR_DCHECK(non_empty_set_counts_[set_index] <= kMaxNonEmptySetCount);
...
bool TaskQueueSelector::CheckContainsQueueForTest(
  ...
  CR_DCHECK(contains_delayed_work_queue == contains_immediate_work_queue);
...
void TaskQueueSelector::ActivePriorityTracker::SetActive(
    TaskQueue::QueuePriority priority,
    bool is_active) {
  CR_DCHECK(priority < TaskQueue::QueuePriority::kQueuePriorityCount);
  CR_DCHECK(IsActive(priority) != is_active);
  if (is_active) {
...
TaskQueueSelector::ActivePriorityTracker::HighestActivePriority() const {
  CR_DCHECK(active_priorities_ != 0u)
