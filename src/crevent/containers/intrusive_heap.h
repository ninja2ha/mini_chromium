...
#ifndef MINI_CHROMIUM_SRC_CREVENT_CONTAINERS_INTRUSIVE_HEAP_H_
#define MINI_CHROMIUM_SRC_CREVENT_CONTAINERS_INTRUSIVE_HEAP_H_
...
// in place.
class CREVENT_EXPORT HeapHandle {
 public:
...
class CREVENT_EXPORT InternalHeapHandleStorage {
 public:
...
CREVENT_EXPORT inline size_t ParentIndex(size_t i) {
  CR_DCHECK(0u != i);
  return (i - 1) / 2;
}

CREVENT_EXPORT inline size_t LeftIndex(size_t i) {
  return 2 * i + 1;
}
...
CREVENT_EXPORT inline void CheckInvalidOrEqualTo(HeapHandle handle, 
                                                size_t index) {
  if (handle.IsValid())
...
void IntrusiveHeap<T, Compare, HeapHandleAccessor>::ClearHeapHandle(
  ...
  CR_DCHECK(!GetHeapHandle(i).IsValid());
}
...
void IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHole(
  ...
  CR_DCHECK(new_hole_pos != old_hole_pos);
  CR_DCHECK(new_hole_pos < size());
  CR_DCHECK(old_hole_pos <= size());
...
IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHoleDownAndFill(
  ...
  CR_DCHECK(hole_pos < n);
  CR_DCHECK(!GetHeapHandle(hole_pos).IsValid());
...
#endif  // MINI_CHROMIUM_SRC_CREVENT_CONTAINERS_INTRUSIVE_HEAP_H_
