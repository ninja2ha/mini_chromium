...
RunLoop::Delegate::~Delegate() {
  ...
  CR_DCHECK(active_run_loops_.empty());
  ...
  if (bound_) {
    CR_DCHECK(this == GetTlsDelegate().Get());
...
void RunLoop::Run(const Location& location) {
  ...
  if (run_timeout) {
    ...
    origin_task_runner_->PostDelayedTask(
        CR_FROM_HERE, cancelable_timeout.callback(), run_timeout->timeout);
  }

  CR_DCHECK(this == delegate_->active_run_loops_.top());
...
void RunLoop::QuitWhenIdle() {
  ...
  if (!origin_task_runner_->RunsTasksInCurrentSequence()) {
    origin_task_runner_->PostTask(
        CR_FROM_HERE, BindOnce(&RunLoop::QuitWhenIdle, Unretained(this)));
    return;
  }
...
void RunLoop::QuitCurrentDeprecated() {
  CR_DCHECK(IsRunningOnCurrentThread());
  Delegate* delegate = GetTlsDelegate().Get();
  CR_DCHECK(delegate->active_run_loops_.top()->allow_quit_current_deprecated_)
...
RunLoop::ScopedDisallowRunning::~ScopedDisallowRunning() {
  CR_DCHECK(current_delegate_ == GetTlsDelegate().Get());
  if (current_delegate_)
...
#if CR_DCHECK_IS_ON()
  CR_DCHECK(delegate_->allow_running_for_testing_)
...
void RunLoop::AfterRun() {
  ...
  auto& active_run_loops = delegate_->active_run_loops_;
  CR_DCHECK(active_run_loops.top() == this);
