...
#include "cr_url/url_file.h"
#include "cr_url/url_util_internal.h"
...
SchemeRegistry* GetSchemeRegistryWithoutLocking() {
  static cr::NoDestructor<SchemeRegistry> registry;
  return registry.get();
}
...
bool DoIsInSchemes(const CHAR* spec,
  ...
  for (const SchemeWithType& scheme_with_type : schemes) {
    if (cr::LowerCaseEqualsASCII(typename CharToStringPiece<CHAR>::Piece(
...
void DoSchemeModificationPreamble() {
  ...
  CR_DCHECK(!g_scheme_registries_used.load())
  ...
  CR_DCHECK(!scheme_registries_locked)
...
void DoAddScheme(const char* new_scheme, std::vector<std::string>* schemes) {
  ...
  CR_DCHECK(schemes);
  CR_DCHECK(strlen(new_scheme) > 0);
  CR_DCHECK(cr::ToLowerASCII(new_scheme) == new_scheme);
  CR_DCHECK(std::find(schemes->begin(), schemes->end(), new_scheme) ==
                      schemes->end());
...
void DoAddSchemeWithType(const char* new_scheme,
  ...
  CR_DCHECK(strlen(new_scheme) > 0);
  CR_DCHECK(cr::ToLowerASCII(new_scheme) == new_scheme);
  CR_DCHECK(std::find_if(schemes->begin(), schemes->end(),
...
bool DomainIs(cr::StringPiece canonical_host,
  ...
  if (cr::StringPiece(host_first_pos, canonical_domain.length()) !=
      canonical_domain) {
    return false;
  }
...
bool HostIsIPAddress(cr::StringPiece host) {
  /* STACK_UNINITIALIZED */ crurl::RawCanonOutputT<char, 128> ignored_output;
  crurl::CanonHostInfo host_info;
  crurl::CanonicalizeIPAddress(host.data(), 
                               // unsafe_cast!!
                               Component(0, static_cast<int>(host.length())),
