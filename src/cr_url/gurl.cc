...
#include "cr_url/url_canon_stdstring.h"
...
GURL::GURL(GURL&& other) noexcept
  ...
  other.parsed_ = crurl::Parsed();
}

GURL::GURL(cr::StringPiece url_string) {
  InitCanonical(url_string, true);
}

GURL::GURL(cr::StringPiece16 url_string) {
  InitCanonical(url_string, true);
}

GURL::GURL(const std::string& url_string, RetainWhiteSpaceSelector) {
  InitCanonical(cr::StringPiece(url_string), false);
}
...
void GURL::InitCanonical(cr::BasicStringPiece<CharT> input_spec,
                         bool trim_path_end) {
  ...
  CR_DCHECK(!is_valid_ || !spec_.empty());
}
...
GURL GURL::Resolve(cr::StringPiece relative) const {
  ...
  if (!crurl::ResolveRelative(spec_.data(), static_cast<int>(spec_.length()),
...
GURL GURL::Resolve(cr::StringPiece16 relative) const {
  // Not allowed for invalid URLs.
  if (!is_valid_)
    return GURL();
...
// Note: code duplicated above (it's inconvenient to use a template here).
GURL GURL::ReplaceComponents(
    const crurl::Replacements<char16_t>& replacements) const {
  GURL result;
...
bool GURL::IsAboutBlank() const {
  return IsAboutUrl(crurl::kAboutBlankPath);
}

bool GURL::IsAboutSrcdoc() const {
  return IsAboutUrl(crurl::kAboutSrcdocPath);
}
...
bool GURL::SchemeIsHTTPOrHTTPS() const {
  return SchemeIs(crurl::kHttpScheme) || SchemeIs(crurl::kHttpsScheme);
}

bool GURL::SchemeIsWSOrWSS() const {
  return SchemeIs(crurl::kWsScheme) || SchemeIs(crurl::kWssScheme);
}
...
bool GURL::SchemeIsCryptographic(cr::StringPiece lower_ascii_scheme) {
  CR_DCHECK(cr::IsStringASCII(lower_ascii_scheme));
  CR_DCHECK(cr::ToLowerASCII(lower_ascii_scheme) == lower_ascii_scheme);

  return lower_ascii_scheme == crurl::kHttpsScheme ||
         lower_ascii_scheme == crurl::kWssScheme;
}
...
cr::StringPiece GURL::PathForRequestPiece() const {
  ...
  return cr::StringPiece(&spec_[parsed_.path.begin], path_len);
}
...
cr::StringPiece GURL::HostNoBracketsPiece() const {
...
std::string GURL::GetContent() const {
  ...
  if (!SchemeIs(crurl::kJavaScriptScheme) && parsed_.ref.len >= 0)
    content.erase(content.size() - parsed_.ref.len - 1);
  return content;
}

bool GURL::HostIsIPAddress() const {
  return is_valid_ && crurl::HostIsIPAddress(host_piece());
}
...
bool GURL::DomainIs(cr::StringPiece canonical_domain) const {
  if (!is_valid_)
    return false;
  ...
  return crurl::DomainIs(host_piece(), canonical_domain);
}

bool GURL::EqualsIgnoringRef(const GURL& other) const {
  int ref_position = parsed_.CountCharactersBefore(crurl::Parsed::REF, true);
  int ref_position_other =
      other.parsed_.CountCharactersBefore(crurl::Parsed::REF, true);
  return cr::StringPiece(spec_).substr(0, ref_position) ==
         cr::StringPiece(other.spec_).substr(0, ref_position_other);
}
...
// static
bool GURL::IsAboutPath(cr::StringPiece actual_path,
                       cr::StringPiece allowed_path) {
  if (!cr::StartsWith(actual_path, allowed_path))
    return false;

  if (actual_path.size() == allowed_path.size()) {
    CR_DCHECK(actual_path == allowed_path);
    return true;
  }

  if ((actual_path.size() == allowed_path.size() + 1) &&
      actual_path.back() == '/') {
    CR_DCHECK(actual_path == (std::string(allowed_path) + '/'));
    return true;
  }
