...
Origin Origin::Create(const GURL& url) {
  ...
  if (url.SchemeIsFileSystem()) {
    ...
    CR_DCHECK(!tuple.IsValid() || url.IsStandard() ||
              cr::Contains(GetLocalSchemes(), url.scheme_piece()) ||
...
// static
cr::Optional<Origin> Origin::UnsafelyCreateTupleOriginWithoutNormalization(
...
// static
cr::Optional<Origin> Origin::UnsafelyCreateOpaqueOriginWithoutNormalization(
    cr::StringPiece precursor_scheme,
    cr::StringPiece precursor_host,
...
cr::Optional<cr::UnguessableToken> Origin::GetNonceForSerialization()
    const {
  // TODO(nasko): Consider not making a copy here, but return a reference to
  // the nonce.
  return nonce_ ? cr::MakeOptional(nonce_->token()) : cr::nullopt;
}
...
bool Origin::CanBeDerivedFrom(const GURL& url) const {
  ...
  if (cr::Contains(crurl::GetNoAccessSchemes(), url.scheme()) &&
...
bool Origin::DomainIs(cr::StringPiece canonical_domain) const {
  return !opaque() && crurl::DomainIs(tuple_.host(), canonical_domain);
}
...
std::string Origin::GetDebugString(bool include_nonce) const {
  ...
  if (!opaque()) {
    ...
    if (scheme() == kFileScheme)
      cr::StrAppend(&out, {" [internally: ", tuple_.Serialize(), "]"});
    return out;
  }
  ...
  // understand.
  std::string out = cr::StrCat({Serialize(), " [internally:"});
  if (include_nonce) {
    out += " (";
  ...
  if (!tuple_.IsValid())
    cr::StrAppend(&out, {" anonymous]"});
  else
    cr::StrAppend(&out, {" derived from ", tuple_.Serialize(), "]"});
  return out;
}

Origin::Origin(SchemeHostPort tuple) : tuple_(std::move(tuple)) {
  CR_DCHECK(!opaque());
  CR_DCHECK(tuple_.IsValid());
}
...
std::ostream& operator<<(std::ostream& out, const crurl::Origin::Nonce& nonce) {
...
Origin::Nonce::Nonce(const cr::UnguessableToken& token) : token_(token) {
  CR_CHECK(!token_.is_empty());
}

const cr::UnguessableToken& Origin::Nonce::token() const {
  ...
  if (token_.is_empty())
    token_ = cr::UnguessableToken::Create();
  return token_;
}

const cr::UnguessableToken& Origin::Nonce::raw_token() const {
  return token_;
}
...
///  other.token_ = cr::UnguessableToken();  // Reset |other|.
///}
