...
#include "cr_base/token/unguessable_token.h"

#include "cr_url/url_export.h"
#include "cr_url/scheme_host_port.h"
...
class CRURL_EXPORT Origin {
 public:
  ...
  static cr::Optional<Origin> UnsafelyCreateTupleOriginWithoutNormalization(
  ...
  const std::string& scheme() const {
    return !opaque() ? tuple_.scheme() : cr::EmptyString();
  }
  const std::string& host() const {
    return !opaque() ? tuple_.host() : cr::EmptyString();
  }
  ...
  bool DomainIs(cr::StringPiece canonical_domain) const;
 ...
 private:
  ...
  // includes the nonce.
  friend class crnet::SchemefulSite;
  friend class OriginTest;
  friend CRURL_EXPORT std::ostream& operator<<(std::ostream& out, 
                                               const Origin& origin);

  // Origin::Nonce is a wrapper around cr::UnguessableToken that generates
  ...
  class CRURL_EXPORT Nonce {
   ...
   private:
    ...
    mutable cr::UnguessableToken token_;
  };
  ...
  static cr::Optional<Origin> UnsafelyCreateOpaqueOriginWithoutNormalization(
  ...
  cr::Optional<Nonce> nonce_;
};

// Pretty-printers for logging. These expose the internal state of the nonce.
CRURL_EXPORT std::ostream& operator<<(std::ostream& out, const Origin& origin);
CRURL_EXPORT std::ostream& operator<<(std::ostream& out, 
                                      const Origin::Nonce& origin);

CRURL_EXPORT bool IsSameOriginWith(const GURL& a, const GURL& b);
