...
bool IsValidInput(const cr::StringPiece& scheme,
  ...
  if (cr::Contains(GetNoAccessSchemes(), scheme))
    return false;
  ...
  if (!is_standard) {
    ...
    if (cr::Contains(GetLocalSchemes(), scheme) && host.empty() && port == 0)
      return true;
  ...
  switch (scheme_type) {
    ...
    case SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION:
      ...
      // canonicalized.
      CR_DCHECK(policy == SchemeHostPort::CHECK_CANONICALIZATION ||
...
SchemeHostPort::SchemeHostPort(std::string scheme,
  ...
  host_ = std::move(host);
  port_ = port;
  CR_DCHECK(IsValid()) << "Scheme: " << scheme_ << " Host: " << host_
                       << " Port: " << port;
}
...
bool SchemeHostPort::IsValid() const {
  ...
  CR_DCHECK(!scheme_.empty() || host_.empty());
  CR_DCHECK(!scheme_.empty() || port_ == 0);
...
GURL SchemeHostPort::GetURL() const {
  ...
  CR_DCHECK(!parsed.path.is_valid());
  // unsafe_cast
  parsed.path = Component(static_cast<int>(serialized.length()), 1);
  serialized.append("/");
...
std::string SchemeHostPort::SerializeInternal(crurl::Parsed* parsed) const {
  std::string result;
  if (!IsValid())
    return result;
  ...
  if (!scheme_.empty()) {
    // unsafe_cast
    parsed->scheme = Component(0, static_cast<int>(scheme_.length()));
  ...
  if (!host_.empty()) {
    // unsafe_cast
    parsed->host = Component(static_cast<int>(result.length()), 
                             static_cast<int>(host_.length()));
    result.append(host_);
  }
  ...
  if (port_ != default_port) {
    result.push_back(':');
    std::string port(cr::NumberToString(port_));
    // unsafe_cast
    parsed->port = Component(static_cast<int>(result.length()), 
                             static_cast<int>(port.length()));
