...
#include "cr_winrt/third_party/minhook/src/minhook.h"
...
bool Hooker::CreateHook(void* target, 
                        void* detour, 
                        void** original, 
                        bool enable) {
  ...
  if (enable && status == MH_OK) 
    return this->EnableHook(target);
  return status == MH_OK;
}

bool Hooker::CreateContextHook(void* target, 
  ...
  unsigned char shell_code[] = {
    ...
    0x50,       // push eax - 0x0A
    0xB8, 0x00, 0x00, 0x00, 0x00, // mov eax, ... // 0x0B
    0xFF, 0xD0, // call eax - 0x10
    0x58,       // pop eax  - 0x12
    0x58,       // pop eax  - 0x13
    0x5B,       // pop ebx  - 0x14
    0x59,       // pop ecx  - 0x15
  ...
  *(uintptr_t*)(shell_code + 0x0B + 0x1) = (uintptr_t)callback;
...
bool Hooker::EnableHook(void* target) {
  MH_STATUS status = ::MH_EnableHook(target);
  CR_DLOG_IF(Error, status != MH_OK) 
      << "nhook EnableHook with an error:" << status;
  return status == MH_OK;
}

bool Hooker::EnableAllHook() {
  MH_STATUS status = ::MH_EnableHook(MH_ALL_HOOKS);
  CR_DLOG_IF(Error, status != MH_OK) 
      << "nhook EnableAllHook with an error:" << status;
  return status == MH_OK;
}

bool Hooker::DisableHook(void* target) {
  MH_STATUS status = ::MH_DisableHook(target);
  CR_DLOG_IF(Error, status != MH_OK) 
      << "nhook DisableHook with an error:" << status;
  return status == MH_OK;
}

bool Hooker::DisableAllHook() {
  MH_STATUS status = ::MH_DisableHook(MH_ALL_HOOKS);
  CR_DLOG_IF(Error, status != MH_OK) 
      << "nhook DisableAllHook with an error:" << status;
  return status == MH_OK;
}
