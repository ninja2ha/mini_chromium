...
#if defined(MINI_CHROMIUM_OS_WIN)
    ::OutputDebugStringW(MultiByteToWide(str_newline, CP_UTF8).c_str());
#endif
  }
...
Win32ErrorLogMessage::~Win32ErrorLogMessage() {
  ...
  // We're about to crash (CR_CHECK). Put |err_| on the stack (by placing it in 
  // a field) and use Alias in hopes that it makes it into crash dumps.
