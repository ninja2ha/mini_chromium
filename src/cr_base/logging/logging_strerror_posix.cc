...
void SafeStrError(SystemErrorCode error_code, char *buf, size_t len) {
  if (buf == nullptr) {
    return;
  }
  ...
  // static.
  wrap_posix_strerror_r(&strerror_r, error_code, buf, len);
}
...
std::string StrError(SystemErrorCode error_code) {
  ...
  SafeStrError(error_code, buf, sizeof(buf));
