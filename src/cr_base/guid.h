...
// NOTE: use GUID::GenerateRandomV4() instead.
CRBASE_EXPORT std::string GenerateGUID();

// NOTE: use GUID::ParseCaseInsensitive() and GUID::is_valid() instead.
CRBASE_EXPORT bool IsValidGUID(StringPiece input);
CRBASE_EXPORT bool IsValidGUID(StringPiece16 input);

// NOTE: use GUID::ParseLowercase() and GUID::is_valid() instead.
CRBASE_EXPORT bool IsValidGUIDOutputString(StringPiece input);

CRBASE_EXPORT std::string DataToGUIDString(const uint64_t bytes[2]);

class CRBASE_EXPORT GUID {
 public:
  ...
  // should not treat it as such. When the internal type of cr::GUID changes,
 ...
 private:
  ...
  // during the migration to cr::GUID.
  //
...
struct CRBASE_EXPORT GUIDHash {
  size_t operator()(const GUID& guid) const {
    ...
    // (Ninja2ha): ??? WHY
    return StringPieceHash()(guid.AsLowercaseString());
  }
};

// Stream operator so GUID objects can be used in logging statements.
CRBASE_EXPORT std::ostream& operator<<(std::ostream& out, const GUID& guid);
