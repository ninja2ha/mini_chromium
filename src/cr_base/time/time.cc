...
#include "cr_base/third_party/nspr/prtime.h"
...
constexpr Optional<ParsedDecimal> ConsumeDurationNumber(
  ...
  if (number_string.empty() || number_string.front() != '.')
    return int_part_empty ? nullopt : MakeOptional(res);
  ...
  for (; !number_string.empty(); number_string.remove_prefix(1)) {
    ...
    if (d < 0 || d >= 10)
      break;
    CR_DCHECK(res.frac_part < res.frac_scale);
  ...
  return int_part_empty && res.frac_scale == 1 ? nullopt : MakeOptional(res);
}
...
Optional<TimeDelta> ConsumeDurationUnit(StringPiece& unit_string) {
  const std::initializer_list<std::pair<const char*, TimeDelta>> str_delta_list = {
  ...
  for (const auto& str_delta : str_delta_list) {
...
Optional<TimeDelta> TimeDelta::FromString(StringPiece duration_string) {
  ...
  while (!duration_string.empty()) {
    ...
    if (number.frac_part != 0) {
      double frack_part = static_cast<double>(sign) *
          static_cast<double>(number.frac_part);
      delta += (frack_part / static_cast<double>(number.frac_scale)) * unit;
    }
  }
  return delta;
}
...
int TimeDelta::InDaysFloored() const {
  if (!is_inf()) {
    const int result = static_cast<int>(delta_ / Time::kMicrosecondsPerDay);
...
Time Time::Midnight(bool is_local) const {
  ...
  CR_DCHECK(result);  // This function must not fail.
  return out_time;
}
...
bool Time::FromStringInternal(const char* time_string,
                              bool is_local,
                              Time* parsed_time) {
  CR_DCHECK(time_string);
  CR_DCHECK(parsed_time);
...
std::string Time::ToLogString() {
  Time::Exploded exploded;
  LocalExplode(&exploded);
  return StringPrintf("%04d-%02d-%02d %02d:%02d:%02d.%03d",
                      exploded.year,
                      exploded.month,
...
std::string Time::ToLogUTCString() {
  Time::Exploded exploded;
  UTCExplode(&exploded);
  return StringPrintf("%04d-%02d-%02d %02d:%02d:%02d.%03d UTC",
                      exploded.year,
                      exploded.month,
                      exploded.day_of_month,
                      exploded.hour,
