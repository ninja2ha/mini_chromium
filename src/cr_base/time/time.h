...
class CRBASE_EXPORT TimeDelta {
 public:
  ...
  // TODO(crbug.com/989694): Avoid cr::TimeDelta factory functions
  ...
  //  "-inf"          -> TimeDelta::Min()
  // Returns |cr::nullopt| when parsing fails. Numbers larger than 2^63-1
  ...
  // cr::Clamp{Ceil,Floor,Round}() or cr::saturated_cast() (for truncation).
  ...
  constexpr double operator/(TimeDelta a) const {
    ...
    // too easy).
    CR_CHECK(!is_zero() || !a.is_zero());
    CR_CHECK(!is_inf() || !a.is_inf());
...
constexpr TimeDelta TimeDelta::operator+(TimeDelta other) const {
  if (!other.is_inf())
    return TimeDelta(int64_t{cr::ClampAdd(delta_, other.delta_)});

  // Additions involving two infinities are only valid if signs match.
  CR_CHECK(!is_inf() || (delta_ == other.delta_));
  return other;
}

constexpr TimeDelta TimeDelta::operator-(TimeDelta other) const {
  if (!other.is_inf())
    return TimeDelta(int64_t{cr::ClampSub(delta_, other.delta_)});

  // Subtractions involving two infinities are only valid if signs differ.
  CR_CHECK(delta_ != other.delta_);
...
class TimeBase {
 public:
  ...
  // For legacy serialization only. When serializing to `cr::Value`, prefer
...
// UTCExplode() and LocalExplode(). See method comments for further details.
  ...
  // that cr::Time::LocalExplode and cr::Time::FromLocalExploded only
  ...
  // 00:00:00 UTC).
  //
  // For serialization, when handling `cr::Value`, prefer the helpers in
  // //base/util/values/values_util.h instead. Otherwise, use these methods for
  // opaque serialization and deserialization, e.g.
  //
  //   // Serialization:
  //   cr::Time last_updated = ...;
  //   SaveToDatabase(last_updated.ToDeltaSinceWindowsEpoch().InMicroseconds());
  //
  //   // Deserialization:
  //   cr::Time last_updated = cr::Time::FromDeltaSinceWindowsEpoch(
  //       cr::TimeDelta::FromMicroseconds(LoadFromDatabase()));
  //
  ...
  // comments on cr::Time::Exploded for more information.
  static bool FromUTCExploded(const Exploded& exploded,
                              Time* time) CR_WARN_UNUSED_RESULT {
  ...
  static bool FromString(const char* time_string,
                         Time* parsed_time) CR_WARN_UNUSED_RESULT {
  ...
  // on cr::Time::Exploded for more information.
  ...
  //
  // DEPRECATED - Do not use in new code. When deserializing from `cr::Value`,
 ...
 private:
  ...
  static bool ExplodedMostlyEquals(const Exploded& lhs,
                                   const Exploded& rhs) CR_WARN_UNUSED_RESULT;
...
constexpr TimeDelta TimeDelta::FromSecondsD(double secs) {
  return TimeDelta(
      SaturatedCast<int64_t>(secs * Time::kMicrosecondsPerSecond));
}

// static
constexpr TimeDelta TimeDelta::FromSeconds(int64_t secs) {
  return TimeDelta(int64_t{cr::ClampMul(secs, Time::kMicrosecondsPerSecond)});
}

// static
constexpr TimeDelta TimeDelta::FromMillisecondsD(double ms) {
  return TimeDelta(
      SaturatedCast<int64_t>(ms * Time::kMicrosecondsPerMillisecond));
}

// static
constexpr TimeDelta TimeDelta::FromMilliseconds(int64_t ms) {
  return TimeDelta(
      int64_t{cr::ClampMul(ms, Time::kMicrosecondsPerMillisecond)});
}

// static
constexpr TimeDelta TimeDelta::FromMicrosecondsD(double us) {
  return TimeDelta(SaturatedCast<int64_t>(us));
}
...
constexpr TimeDelta TimeDelta::FromNanosecondsD(double ns) {
  return TimeDelta(
      SaturatedCast<int64_t>(ns / Time::kNanosecondsPerMicrosecond));
}
...
constexpr int TimeDelta::InHours() const {
  ...
  return SaturatedCast<int>(delta_ / Time::kMicrosecondsPerHour);
}

constexpr int TimeDelta::InMinutes() const {
  // saturated_cast<> is necessary since very large (but still less than
  // min/max) deltas would result in overflow.
  return SaturatedCast<int>(delta_ / Time::kMicrosecondsPerMinute);
}
...
constexpr int64_t TimeDelta::InNanoseconds() const {
  return cr::ClampMul(delta_, Time::kNanosecondsPerMicrosecond);
}
...
class CRBASE_EXPORT TimeTicks : public time_internal::TimeBase<TimeTicks> {
 public:
  ...
  static bool IsHighResolution() CR_WARN_UNUSED_RESULT;
  ...
  static bool IsConsistentAcrossProcesses() CR_WARN_UNUSED_RESULT;
