...
#include "cr_base/strings/internal/string_split_internal.h"
...
bool AppendStringKeyValue(StringPiece input,
  ...
  if (end_key_pos == std::string::npos) {
    CR_DVLOG(1) << "cannot find delimiter in: " << input;
  ...
  if (begin_value_pos == StringPiece::npos) {
    CR_DVLOG(1) << "cannot parse value from input: " << input;
...
std::vector<WStringPiece> SplitStringPieceUsingSubstr(
    ...
    WhitespaceHandling whitespace,
    SplitResult result_type) {
  std::vector<WStringPiece> result;
