...
#ifndef MINI_CHROMIUM_SRC_CRBASE_STRINGS_STRING_UTIL_INTERNAL_H_
#define MINI_CHROMIUM_SRC_CRBASE_STRINGS_STRING_UTIL_INTERNAL_H_
...
inline bool CompareParameter(const ReplacementOffset& elem1,
...
template <class CharT>
SubstringMatcher<CharT> MakeSubstringMatcher(
    BasicStringPiece<CharT> find_this) {
...
struct CharacterMatcher {
  ...
  size_t MatchSize() { return 1; }
};

// Type deduction helper for CharacterMatcher.
template <class CharT>
CharacterMatcher<CharT> MakeCharacterMatcher(
    BasicStringPiece<CharT> find_any_of_these) {
...
// (cr::Span or std::initializer_list) of strings/StringPieces (std::string,
...
static std::basic_string<CharT> JoinStringT(list_type parts,
  ...
  CR_DCHECK(total_size == result.size());
    ...
    if ('$' == *i) {
      if (i + 1 != format_string.end()) {
        ...
        if ('$' == *i) {
          ...
          if (*i < '1' || *i > '9') {
            CR_DLOG(Error) << "Invalid placeholder: $" << *i;
            continue;
          }
          uintptr_t index = *i - '1';
          if (offsets) {
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_STRINGS_STRING_UTIL_INTERNAL_H_