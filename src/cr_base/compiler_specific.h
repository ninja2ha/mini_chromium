...
#define CR_MSVC_POP_WARNING(n) __pragma(warning(pop))

#else

#define CR_MSVC_PUSH_DISABLE_WARNING(n)
#define CR_MSVC_POP_WARNING(n)
...
#else
#error Please add support for your compipler in cr_base/compiler_specific.h
#endif
...
//
template<typename T> CR_ALWAYS_INLINE void CR_IGNORE_RESULT(const T&x) {}
