...
#ifndef MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_CIRCULAR_DEQUE_H_
#define MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_CIRCULAR_DEQUE_H_
...
#include "cr_base/containers/internal/vector_buffer.h"
#include "cr_base/internal/template_util.h"

// cr::CircularDeque is similar to std::deque. Unlike std::deque, the
...
// Constructors:
//   CircularDeque();
//   CircularDeque(size_t count);
//   CircularDeque(size_t count, const T& value);
//   CircularDeque(InputIterator first, InputIterator last);
//   CircularDeque(const CircularDeque&);
//   CircularDeque(CircularDeque&&);
//   CircularDeque(std::initializer_list<value_type>);
//
// Assignment functions:
//   CircularDeque& operator=(const CircularDeque&);
//   CircularDeque& operator=(CircularDeque&&);
//   CircularDeque& operator=(std::initializer_list<T>);
...
template <typename T>
class CircularDequeConstIterator {
 public:
  ...
  CircularDequeConstIterator() : parent_deque_(nullptr), index_(0) {
#if CR_DCHECK_IS_ON()
  ...
  CircularDequeConstIterator& operator++() {
    Increment();
    return *this;
  }
  CircularDequeConstIterator operator++(int) {
    CircularDequeConstIterator ret = *this;
    Increment();
    return ret;
  }
  CircularDequeConstIterator& operator--() {
  ...
  friend CircularDequeConstIterator operator+(
      const CircularDequeConstIterator& iter,
      difference_type offset) {
    CircularDequeConstIterator ret = iter;
    ret.Add(offset);
    return ret;
  }
  CircularDequeConstIterator& operator+=(difference_type offset) {
    Add(offset);
    return *this;
  }
  friend CircularDequeConstIterator operator-(
  ...
  friend std::ptrdiff_t operator-(const CircularDequeConstIterator& lhs,
                                  const CircularDequeConstIterator& rhs) {
    lhs.CheckComparable(rhs);
    return lhs.OffsetFromBegin() - rhs.OffsetFromBegin();
  }

  // Comparisons.
  friend bool operator==(const CircularDequeConstIterator& lhs,
                         const CircularDequeConstIterator& rhs) {
    lhs.CheckComparable(rhs);
 ...
 protected:
  friend class CircularDeque<T>;

  CircularDequeConstIterator(const CircularDeque<T>* parent, size_t index)
  ...
  void CheckUnstableUsage() const {
    CR_DCHECK(parent_deque_);
    // Since CircularDeque doesn't guarantee stability, any attempt to
    // dereference this iterator after a mutation (i.e. the generation doesn't
    // match the original) in the container is illegal.
    CR_DCHECK(created_generation_ == parent_deque_->generation_)
        << "CircularDeque iterator dereferenced after mutation.";
  }
  void CheckComparable(const CircularDequeConstIterator& other) const {
    CR_DCHECK(parent_deque_ == other.parent_deque_);
    // Since CircularDeque doesn't guarantee stability, two iterators that
    ...
    CR_DCHECK(created_generation_ == other.created_generation_);
  }
#else
  inline void CheckUnstableUsage() const {}
  inline void CheckComparable(const CircularDequeConstIterator&) const {}
#endif  // DCHECK_IS_ON()

  const CircularDeque<T>* parent_deque_;
  size_t index_;
...
template <typename T>
class CircularDequeIterator : public CircularDequeConstIterator<T> {
  using base = CircularDequeConstIterator<T>;
 ...
 public:
  ...
  CircularDequeIterator() : CircularDequeConstIterator<T>() {}
  ...
  // Random access mutation.
  friend CircularDequeIterator operator+(const CircularDequeIterator& iter,
                                         difference_type offset) {
    CircularDequeIterator ret = iter;
    ret.Add(offset);
    return ret;
  }
  CircularDequeIterator& operator+=(difference_type offset) {
    base::Add(offset);
    return *this;
  }
  friend CircularDequeIterator operator-(const CircularDequeIterator& iter,
  ...
  CircularDequeIterator operator++(int) {
    CircularDequeIterator ret = *this;
    base::Increment();
    return ret;
  }
  CircularDequeIterator& operator--() {
 ...
 private:
  CircularDequeIterator(const CircularDeque<T>* parent, size_t index)
      : CircularDequeConstIterator<T>(parent, index) {}
};
...
class CircularDeque {
 ...
 public:
  ...
  using iterator = internal::CircularDequeIterator<T>;
  using const_iterator = internal::CircularDequeConstIterator<T>;
  ...
  constexpr CircularDeque() = default;

  // Constructs with |count| copies of |value| or default constructed version.
  CircularDeque(size_type count) { resize(count); }
  CircularDeque(size_type count, const T& value) { resize(count, value); }
  ...
  // Copy/move.
  CircularDeque(const CircularDeque& other) : buffer_(other.size() + 1) {
  ...
  CircularDeque(std::initializer_list<value_type> init) { assign(init); }

  ~CircularDeque() { DestructRange(begin_, end_); }
  ...
  CircularDeque& operator=(std::initializer_list<value_type> ilist) {
    reserve(ilist.size());
  ...
  value_type& at(size_type i) {
    return const_cast<value_type&>(cr::as_const(*this).at(i));
  }

  value_type& operator[](size_type i) {
    return const_cast<value_type&>(cr::as_const(*this)[i]);
  }
  ...
  template <class InputIterator>
  typename std::enable_if<::cr::internal::is_iterator<InputIterator>::value,
                          void>::type
  ...
  friend void swap(CircularDeque& lhs, CircularDeque& rhs) { lhs.swap(rhs); }

 private:
  friend internal::CircularDequeIterator<T>;
  friend internal::CircularDequeConstIterator<T>;
  ...
  void CheckValidIndex(size_t i) const {
    if (begin_ <= end_)
      CR_DCHECK(i >= begin_ && i < end_);
    else
      CR_DCHECK((i >= begin_ && i < buffer_.capacity()) || i < end_);
  }
  ...
  void ValidateIterator(const const_iterator& i) const {
    CR_DCHECK(i.parent_deque_ == this);
...
// Implementations of cr::Erase[If] (see base/stl_util.h).
template <class T, class Value>
size_t Erase(CircularDeque<T>& container, const Value& value) {
...
size_t EraseIf(CircularDeque<T>& container, Predicate pred) {
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_CIRCULAR_DEQUE_H_
