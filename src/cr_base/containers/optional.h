...
struct OptionalStorageBase {
  ...
  template <class... Args>
  constexpr explicit OptionalStorageBase(internal::in_place_t, Args&&... args)
...
template <typename T,
          bool = internal::is_trivially_copy_constructible<T>::value,
...
using IsConvertibleFromOptional =
    internal::disjunction<std::is_constructible<T, Optional<U>&>,
...
using IsAssignableFromOptional =
    internal::disjunction<IsConvertibleFromOptional<T, U>,
...
#ifdef MINI_CHROMIUM_COMPILER_MSVC
...
class OPTIONAL_DECLSPEC_EMPTY_BASES Optional
 ...
 private:
  ...
  static_assert(
      !std::is_same<internal::remove_cvref_t<T>, internal::in_place_t>::value,
      "instantiation of cr::Optional with in_place_t is ill-formed");
  static_assert(!std::is_same<internal::remove_cvref_t<T>, nullopt_t>::value,
                "instantiation of cr::Optional with nullopt_t is ill-formed");
  static_assert(
      !std::is_reference<T>::value,
      "instantiation of cr::Optional with a reference type is ill-formed");
  // See: https://timsong-cpp.github.io/cppwp/n4659/optional#optional-3
  static_assert(std::is_destructible<T>::value,
                "instantiation of cr::Optional with a non-destructible type "
                "is ill-formed");
  ...
  static_assert(
      ...
      "instantiation of cr::Optional with an array type is ill-formed");
...
#undef OPTIONAL_DECLSPEC_EMPTY_BASES
  ...
  constexpr explicit Optional(internal::in_place_t,
      ...
      : internal::OptionalBase<T>(
            internal::in_place, il, std::forward<Args>(args)...) {}
  ...
  template <typename U = value_type,
            std::enable_if_t<
                std::is_constructible<T, U&&>::value &&
                    !std::is_same<internal::remove_cvref_t<U>, 
  ...
  template <typename U>
  std::enable_if_t<!std::is_same<internal::remove_cvref_t<U>, 
                                 Optional<T>>::value &&
                       std::is_constructible<T, U>::value &&
  ...
  constexpr const T* operator->() const {
    CR_CHECK(storage_.is_populated_);
  ...
  void swap(Optional& other) {
    ...
    CR_CHECK(storage_.is_populated_ && other.storage_.is_populated_);
    using std::swap;
    swap(**this, *other);
  }
...
constexpr Optional<T> MakeOptional(std::initializer_list<U> il,
                                   Args&&... args) {
