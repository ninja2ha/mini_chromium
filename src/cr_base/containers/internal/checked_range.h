...
#ifndef MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_CHECKED_RANGE_H_
#define MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_CHECKED_RANGE_H_
...
// expected that cr::data(container) and cr::size(container) are valid
...
class CheckedContiguousRange {
 public:
  using element_type = std::remove_pointer_t<decltype(
      cr::data(std::declval<ContiguousContainer&>()))>;
  ...
  constexpr pointer data() const noexcept {
    return container_ ? cr::data(*container_) : nullptr;
  }

  constexpr const_pointer cdata() const noexcept { return data(); }

  constexpr size_type size() const noexcept {
    return container_ ? cr::size(*container_) : 0;
  }

  constexpr bool empty() const noexcept {
    return container_ ? cr::empty(*container_) : true;
  }
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_CHECKED_RANGE_H_