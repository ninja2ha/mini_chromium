...
#ifndef MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_CHECKED_ITERATORS_H_
#define MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_CHECKED_ITERATORS_H_
...
class CheckedContiguousIterator {
 public:
  ...
  constexpr CheckedContiguousIterator(const T* start, T* current, const T* end)
    ...
    CR_CHECK(start <= current);
    CR_CHECK(current <= end);
  }
  ...
  constexpr CheckedContiguousIterator(const CheckedContiguousIterator<U>& other)
    ...
    // CR_CHECKs would be redundant, since we expect |other| to maintain its own
    // invariant. However, CR_DCHECKs never hurt anybody. Presumably.
    CR_DCHECK(other.start_ <= other.current_);
    CR_DCHECK(other.current_ <= other.end_);
  }
  ...
  constexpr CheckedContiguousIterator& operator+=(difference_type rhs) {
    if (rhs > 0) {
      CR_CHECK(rhs <= (end_ - current_));
    } else {
      CR_CHECK(-rhs <= (current_ - start_));
    }
    current_ += rhs;
    return *this;
  }
  ...
  static bool IsRangeMoveSafe(const CheckedContiguousIterator& from_begin,
    ...
    if (from_end < from_begin)
      return false;
    const auto from_begin_uintptr = AsUIntPtr(from_begin.current_);
    const auto from_end_uintptr = AsUIntPtr(from_end.current_);
    const auto to_begin_uintptr = AsUIntPtr(to.current_);
    const auto to_end_uintptr =
        AsUIntPtr((to + std::distance(from_begin, from_end)).current_);
 ...
 private:
  constexpr void CheckComparable(const CheckedContiguousIterator& other) const {
    CR_CHECK(start_ == other.start_);
    CR_CHECK(end_ == other.end_);
  }
...
struct __is_cpp17_contiguous_iterator<
    ::cr::internal::CheckedContiguousIterator<T>> : true_type {};

template <typename T>
struct pointer_traits<::cr::internal::CheckedContiguousIterator<T>> {
  using pointer = ::cr::internal::CheckedContiguousIterator<T>;
  using element_type = T;
  using difference_type = ptrdiff_t;

  template <typename U>
  using rebind = ::cr::internal::CheckedContiguousIterator<U>;
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_CHECKED_ITERATORS_H_