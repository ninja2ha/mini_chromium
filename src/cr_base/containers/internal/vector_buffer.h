...
#ifndef MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_VECTOR_BUFFER_H_
#define MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_VECTOR_BUFFER_H_
...
class VectorBuffer {
 public:
  ...
  T& operator[](size_t i) {
    // TODO(crbug.com/817982): Some call sites (at least CircularDeque.h) are
  ...
  template <typename T2 = T,
            typename std::enable_if<
                internal::is_trivially_copyable<T2>::value,
                int>::type = 0>
 ...
 private:
  static bool RangesOverlap(const T* from_begin,
                            const T* from_end,
                            const T* to) {
    const auto from_begin_uintptr = AsUIntPtr(from_begin);
    const auto from_end_uintptr = AsUIntPtr(from_end);
    const auto to_uintptr = AsUIntPtr(to);
    return !(
        to >= from_end ||
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_CONTAINERS_INTERNAL_VECTOR_BUFFER_H_