...
#ifndef MINI_CHROMIUM_SRC_CRBASE_NUMERIC_BITS_H_
#define MINI_CHROMIUM_SRC_CRBASE_NUMERIC_BITS_H_
...
CR_ALWAYS_INLINE
  ...
  unsigned long index;
  return CR_LIKELY(_BitScanReverse(&index, static_cast<uint32_t>(x)))
             ? (31 - index - (32 - bits))
             : bits;
}
...
// MSVC only supplies _BitScanReverse64 when building for a 64-bit target.
  ...
  return CR_LIKELY(_BitScanReverse64(&index, static_cast<uint64_t>(x)))
             ? (63 - index)
             : 64;
#else
  uint32_t left = static_cast<uint32_t>(x >> 32);
  if (CR_LIKELY(_BitScanReverse(&index, left)))
    return 31 - index;

  uint32_t right = static_cast<uint32_t>(x);
  if (CR_LIKELY(_BitScanReverse(&index, right)))
    return 63 - index;
...
CR_ALWAYS_INLINE
  ...
  static_assert(bits > 0, "invalid instantiation");
  unsigned long index;
  return CR_LIKELY(_BitScanForward(&index, static_cast<uint32_t>(x))) 
      ? index : bits;
}
...
#if defined(MINI_CHROMIUM_ARCH_CPU_64_BITS)
  return CR_LIKELY(_BitScanForward64(&index, static_cast<uint64_t>(x))) 
      ? index : 64;
#else
  uint32_t right = static_cast<uint32_t>(x);
  if (CR_LIKELY(_BitScanForward(&index, right)))
    return index;

  uint32_t left = static_cast<uint32_t>(x >> 32);
  if (CR_LIKELY(_BitScanForward(&index, left)))
    return 32 + index;
...
CR_ALWAYS_INLINE
  ...
  return CR_LIKELY(value) ? bits == 64
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_NUMERIC_BITS_H_