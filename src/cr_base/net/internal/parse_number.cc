...
bool StringToNumber(StringPiece input, int32_t* output) {
  // This assumes ints are 32-bits (will fail compile if that ever changes).
  return StringToInt(input, output);
}

bool StringToNumber(StringPiece input, uint32_t* output) {
  // This assumes ints are 32-bits (will fail compile if that ever changes).
  return StringToUint(input, output);
}

bool StringToNumber(StringPiece input, int64_t* output) {
  return StringToInt64(input, output);
}

bool StringToNumber(StringPiece input, uint64_t* output) {
...
template <typename T>
bool ParseIntHelper(StringPiece input,
...
bool ParseInt32(StringPiece input,
                ParseIntFormat format,
                int32_t* output,
                ParseIntError* optional_error) {
...
bool ParseInt64(StringPiece input,
                ParseIntFormat format,
                int64_t* output,
                ParseIntError* optional_error) {
  return ParseIntHelper(input, format, output, optional_error);
...
bool ParseUint32(StringPiece input,
                 ParseIntFormat format,
                 uint32_t* output,
                 ParseIntError* optional_error) {
  CR_CHECK(format == ParseIntFormat::NON_NEGATIVE ||
           format == ParseIntFormat::STRICT_NON_NEGATIVE);
  return ParseIntHelper(input, format, output, optional_error);
}

bool ParseUint64(StringPiece input,
                 ParseIntFormat format,
                 uint64_t* output,
                 ParseIntError* optional_error) {
  CR_CHECK(format == ParseIntFormat::NON_NEGATIVE ||
