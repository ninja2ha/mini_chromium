...
#include "cr_base/net/internal/parse_number.h"
...
bool IsPubliclyRoutableIPv4(const IPAddressBytes& ip_address) {
  ...
  CR_DCHECK(IPAddress::kIPv4AddressSize == ip_address.size());
  struct {
...
bool IsPubliclyRoutableIPv6(const IPAddressBytes& ip_address) {
  CR_DCHECK(IPAddress::kIPv6AddressSize == ip_address.size());
  struct {
...
bool ParseIPLiteralToBytes(StringPiece ip_literal,
  ...
  if (ip_literal.find(':') != cr::StringPiece::npos) {
    ...
    return internal::IPv6AddressToNumber(host_brackets, bytes->data());
  }
  ...
  int num_components;
  return internal::IPv4AddressToNumber(
      ip_literal, bytes->data(), &num_components);
}
...
bool IPAddressBytes::operator==(const IPAddressBytes& other) const {
  return std::equal(
      other.begin(), other.end(), this->begin(), this->end());
}
...
size_t IPAddressBytes::EstimateMemoryUsage() const {
  return sizeof(bytes_) + bytes_.max_size();
}

// static
cr::Optional<IPAddress> IPAddress::FromIPLiteral(StringPiece ip_literal) {
  IPAddress address;
  if (!address.AssignFromIPLiteral(ip_literal)) {
    return cr::nullopt;
  }
  CR_DCHECK(address.IsValid());
  return address;
}
...
bool IPAddress::AssignFromIPLiteral(StringPiece ip_literal) {
...
std::string IPAddress::ToString() const {
  ...
  if (IsIPv4()) {
    internal::AppendIPv4Address(ip_address_.data(), &str);
  } else if (IsIPv6()) {
    internal::AppendIPv6Address(ip_address_.data(), &str);
  }
  return str;
}

size_t IPAddress::EstimateMemoryUsage() const {
  return sizeof(ip_address_) + ip_address_.size();
}
...
IPAddress ConvertIPv4MappedIPv6ToIPv4(const IPAddress& address) {
  CR_CHECK(address.IsIPv4MappedIPv6());
...
bool IPAddressMatchesPrefix(const IPAddress& ip_address,
  ...
  // or IPv6.
  CR_CHECK(ip_address.IsValid());
  CR_CHECK(ip_prefix.IsValid());

  CR_CHECK(prefix_length_in_bits <= ip_prefix.size() * 8);
...
bool ParseCIDRBlock(StringPiece cidr_literal,
  ...
  std::vector<StringPiece> parts = SplitStringPiece(
      cidr_literal, "/", TRIM_WHITESPACE, SPLIT_WANT_ALL);
  if (parts.size() != 2)
    return false;
...
bool ParseURLHostnameToAddress(StringPiece hostname,
                               IPAddress* ip_address) {
  if (hostname.size() >= 2 && hostname.front() == '[' &&
      hostname.back() == ']') {
    // Strip the square brackets that surround IPv6 literals.
    auto ip_literal =
        StringPiece(hostname).substr(1, hostname.size() - 2);
...
IPAddress ConvertIPv4ToIPv4EmbeddedIPv6(const IPAddress& ipv4_address,
  ...
  CR_DCHECK(ipv4_address.IsIPv4());
  CR_DCHECK(ipv6_address.IsIPv6());
