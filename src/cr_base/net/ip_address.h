...
// * VERSION: 120.0.6099.338

#ifndef MINI_CHROMIUM_SRC_CRBASE_NET_IP_ADDRESS_H_
#define MINI_CHROMIUM_SRC_CRBASE_NET_IP_ADDRESS_H_
...
class CRBASE_EXPORT IPAddressBytes {
 public:
  IPAddressBytes();
...
class CRBASE_EXPORT IPAddress {
 public:
  ...
  static cr::Optional<IPAddress> FromIPLiteral(StringPiece ip_literal);
  ...
  // network byte order.
  explicit IPAddress(cr::Span<const uint8_t> address)
  ...
  bool AssignFromIPLiteral(StringPiece ip_literal) CR_WARN_UNUSED_RESULT;
...
CRBASE_EXPORT std::string IPAddressToStringWithPort(const IPAddress& address,
                                                    uint16_t port);

// Returns the address as a sequence of bytes in network-byte-order.
CRBASE_EXPORT std::string IPAddressToPackedString(const IPAddress& address);

// Converts an IPv4 address to an IPv4-mapped IPv6 address.
// For example 192.168.0.1 would be converted to ::ffff:192.168.0.1.
CRBASE_EXPORT IPAddress ConvertIPv4ToIPv4MappedIPv6(const IPAddress& address);

// Converts an IPv4-mapped IPv6 address to IPv4 address. Should only be called
// on IPv4-mapped IPv6 addresses.
CRBASE_EXPORT IPAddress ConvertIPv4MappedIPv6ToIPv4(const IPAddress& address);
...
// (IPv6) addresses.
CRBASE_EXPORT bool IPAddressMatchesPrefix(const IPAddress& ip_address,
...
//    a:b:c::/46
//    ::1/128
CRBASE_EXPORT bool ParseCIDRBlock(StringPiece cidr_literal,
...
CRBASE_EXPORT bool ParseURLHostnameToAddress(
    StringPiece hostname,
    IPAddress* ip_address) CR_WARN_UNUSED_RESULT;

// Returns number of matching initial bits between the addresses |a1| and |a2|.
CRBASE_EXPORT size_t CommonPrefixLength(const IPAddress& a1, 
                                        const IPAddress& a2);

// Computes the number of leading 1-bits in |mask|.
CRBASE_EXPORT size_t MaskPrefixLength(const IPAddress& mask);
...
CRBASE_EXPORT Dns64PrefixLength
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_NET_IP_ADDRESS_H_