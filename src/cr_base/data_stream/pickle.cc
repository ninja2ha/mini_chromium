...
#include "cr_base/data_stream/pickle.h"
...
bool PickleIterator::ReadLong(long* result) {
  ...
  *result = cr::CheckedCast<long>(result_int64);
  return true;
}
...
bool PickleIterator::ReadData(Span<const uint8_t>* data) {
  ...
  *data = cr::AsConstBytes(MakeSpan(ptr, length));
  return true;
}
...
Pickle::Pickle()
    : header_(nullptr),
      ...
      write_offset_(0) {
  static_assert(cr::bits::IsPowerOfTwo(Pickle::kPayloadUnit),
...
Pickle::Pickle(int header_size)
    : header_(nullptr),
      ...
      capacity_after_header_(0),
      write_offset_(0) {
  CR_DCHECK(static_cast<size_t>(header_size) >= sizeof(Header));
  CR_DCHECK(header_size <= kPayloadUnit);
  Resize(kPayloadUnit);
  header_->payload_size = 0;
}
...
bool Pickle::WriteAttachment(RefPtr<Attachment> attachment) {
  return false;
}

bool Pickle::ReadAttachment(cr::PickleIterator* iter,
...
void Pickle::Resize(size_t new_capacity) {
  CR_CHECK(capacity_after_header_ != kCapacityReadOnly);
  capacity_after_header_ = cr::bits::AlignUp(new_capacity, kPayloadUnit);
