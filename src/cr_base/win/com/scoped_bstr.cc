...
#include "cr_base/win/com/scoped_bstr.h"
...
BSTR AllocBstrOrDie(WStringPiece non_bstr) {
  BSTR result = ::SysAllocStringLen(non_bstr.data(),
                                    CheckedCast<UINT>(non_bstr.length()));
  
  if (!result) {
    cr::debug::ImmediateCrashWithReason(
        "OutOfMemory_com1",
        (non_bstr.length() + 1) * sizeof(wchar_t));
  }
  return result;
}

BSTR AllocBstrBytesOrDie(size_t bytes) {
  BSTR result = ::SysAllocStringByteLen(nullptr, CheckedCast<UINT>(bytes));
  if (!result) {
    cr::debug::ImmediateCrashWithReason(
        "OutOfMemory_com2",
...
void ScopedBstr::SetByteLen(size_t bytes) {
  ...
  data[-1] = CheckedCast<uint32_t>(bytes);
}
