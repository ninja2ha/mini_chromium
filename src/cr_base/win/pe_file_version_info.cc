...
#include "cr_base/win/pe_file_version_info.h"
...
#include "cr_base/win/resource_util.h"
...
PeFileVersionInfo::~PeFileVersionInfo() = default;

// static
std::unique_ptr<PeFileVersionInfo>
PeFileVersionInfo::CreateFromModule(HMODULE module) {
  void* data;
  ...
  return cr::WrapUnique(
      new PeFileVersionInfo(data, translate->language, translate->code_page));
}

// static
std::unique_ptr<PeFileVersionInfo>
PeFileVersionInfo::CreateFromFile(const cr::FilePath& file_path) {
  ...
  return cr::WrapUnique(new PeFileVersionInfo(
      std::move(data), translate->language, translate->code_page));
}

std::u16string PeFileVersionInfo::company_name() {
  return GetStringValue(u"CompanyName");
}

std::u16string PeFileVersionInfo::company_short_name() {
  return GetStringValue(u"CompanyShortName");
}

std::u16string PeFileVersionInfo::internal_name() {
  return GetStringValue(u"InternalName");
}

std::u16string PeFileVersionInfo::product_name() {
  return GetStringValue(u"ProductName");
}

std::u16string PeFileVersionInfo::product_short_name() {
  return GetStringValue(u"ProductShortName");
}

std::u16string PeFileVersionInfo::product_version() {
  return GetStringValue(u"ProductVersion");
}

std::u16string PeFileVersionInfo::file_description() {
  return GetStringValue(u"FileDescription");
}

std::u16string PeFileVersionInfo::file_version() {
  return GetStringValue(u"FileVersion");
}

std::u16string PeFileVersionInfo::original_filename() {
  return GetStringValue(u"OriginalFilename");
}

std::u16string PeFileVersionInfo::special_build() {
  return GetStringValue(u"SpecialBuild");
}

bool PeFileVersionInfo::GetValue(const char16_t* name,
  ...
  for (const auto& lang_codepage : lang_codepages) {
    ...
    _snwprintf_s(sub_block, MAX_PATH, MAX_PATH,
                 ...
                 lang_codepage.code_page, cr::AsConstWide(name));
...
std::u16string PeFileVersionInfo::GetStringValue(const char16_t* name) const {
  std::u16string str;
  GetValue(name, &str);
  return str;
}

cr::Version PeFileVersionInfo::GetFileVersion() const {
...
PeFileVersionInfo::PeFileVersionInfo(std::vector<uint8_t>&& data,
                                     WORD language,
                                     WORD code_page)
...
PeFileVersionInfo::PeFileVersionInfo(void* data,
                                     WORD language,
                                     WORD code_page)
    : data_(data),
      language_(language),
