...
#include "cr_base/win/registry.h"
...
LONG RegKey::CreateWithDisposition(HKEY rootkey,
  ...
  LONG result =
      ::RegCreateKeyExW(rootkey, subkey, 0, nullptr, REG_OPTION_NON_VOLATILE,
...
LONG RegKey::CreateKey(const wchar_t* name, REGSAM access) {
  ...
  LONG result = ::RegCreateKeyExW(
      key_, name, 0, nullptr, REG_OPTION_NON_VOLATILE, access, nullptr, 
      &subkey, nullptr);
  if (result == ERROR_SUCCESS) {
    Close();
    key_ = subkey;
...
LONG RegKey::GetValueNameAt(int index, std::wstring* name) const {
  wchar_t buf[256];
  DWORD bufsize = static_cast<DWORD>(size(buf));
  LONG r = ::RegEnumValueW(key_, index, buf, &bufsize, nullptr, nullptr, 
                           nullptr, nullptr);
  if (r == ERROR_SUCCESS)
...
LONG RegKey::DeleteEmptyKey(const wchar_t* name) {
  ...
  DWORD count = 0;
  result =
      ::RegQueryInfoKeyW(target_key, nullptr, nullptr, nullptr, nullptr, 
                         nullptr, nullptr, &count, nullptr, nullptr, nullptr, 
                         nullptr);
...
LONG RegKey::RegDelRecurse(HKEY root_key, const wchar_t* name, REGSAM access) {
  ...
  while (result == ERROR_SUCCESS) {
    ...
    result =
        ::RegEnumKeyExW(target_key, 0, WriteInto(&key_name, kMaxKeyNameLength),
