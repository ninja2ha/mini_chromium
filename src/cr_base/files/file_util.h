...
#include "cr_base/posix/file_descriptor_posix.h"
...
// |outfile|.
CRBASE_EXPORT bool CopyFileContents(File& infile, File& outfile);
...
CRBASE_EXPORT bool ReadStreamToString(FILE* stream, std::string* contents);

// As ReadFileToStringWithMaxSize, but reading from an open stream after seeking
// to its start (if supported by the stream).
CRBASE_EXPORT bool ReadStreamToStringWithMaxSize(FILE* stream,
                                                 size_t max_size,
                                                 std::string* contents);
...
// EWOULDBLOCK.
CRBASE_EXPORT bool ReadFileToStringNonBlocking(const cr::FilePath& file,
                                               std::string* ret);
...
CRBASE_EXPORT bool ExecutableExistsInPath(
    Environment* env,
...
CRBASE_EXPORT ScopedFILE CreateAndOpenTemporaryStream(FilePath* path);

// Similar to CreateAndOpenTemporaryStream, but the file is created in |dir|.
CRBASE_EXPORT ScopedFILE CreateAndOpenTemporaryStreamInDir(const FilePath& dir,
                                                           FilePath* path);
...
CRBASE_EXPORT File FILEToFile(FILE* file_stream);
...
CRBASE_EXPORT bool WriteFileDescriptor(const int fd, 
                                       const char* data, 
                                       int size);
...
CRBASE_EXPORT bool CopyFileContentsWithSendfile(File& infile,
                                                File& outfile,
                                                bool& retry_slow);
