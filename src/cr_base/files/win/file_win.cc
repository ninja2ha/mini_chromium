...
bool File::SetTimes(Time last_access_time, Time last_modified_time) {
  ...
  last_access_time.ToFileTime(&last_access_filetime);

  FILETIME last_modified_filetime;
  last_modified_time.ToFileTime(&last_modified_filetime);
...
File::Error File::OSErrorToFileError(DWORD last_error) {
  switch (last_error) {
    ...
    default:
      ...
      CR_DLOG(Warning) << "PlatformFile.UnknownErrors.Windows: " << last_error;
      // This function should only be called for errors.
      CR_DCHECK(static_cast<DWORD>(ERROR_SUCCESS) != last_error);
