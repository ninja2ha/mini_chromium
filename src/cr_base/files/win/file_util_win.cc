...
void RecordPostOperationState(const FilePath& path,
  ...
  std::string histogram_name =
      cr::StrCat({"Windows.PostOperationState.", operation});
  CR_DVLOG(1) << histogram_name << ": " << (int)metric;
...
void RecordFilesystemError(StringPiece operation, DWORD error) {
  std::string histogram_name =
      cr::StrCat({"Windows.FilesystemError.", operation});
  CR_DVLOG(1) << histogram_name << ": " << error;
...
File FILEToFile(FILE* file_stream) {
  ...
  CR_DCHECK(file_handle != reinterpret_cast<intptr_t>(INVALID_HANDLE_VALUE));
...
int GetMaximumPathComponentLength(const FilePath& path) {
  ...
  if (!GetVolumePathNameW(path.NormalizePathSeparators().value().c_str(),
                          volume_path, 
                          static_cast<DWORD>(size(volume_path)))) {
    return -1;
  }
