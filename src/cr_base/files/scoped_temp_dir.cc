...
#include "cr_base/files/scoped_temp_dir.h"
...
ScopedTempDir& ScopedTempDir::operator=(ScopedTempDir&& other) {
  if (!path_.empty() && !Delete())
    CR_DLOG(Warning) << "Could not delete temp dir in operator=().";
  path_ = other.Take();
  return *this;
}
...
const FilePath& ScopedTempDir::GetPath() const {
  CR_DCHECK(!path_.empty()) << "Did you call CreateUniqueTempDir* before?";
  return path_;
}
