...
bool FilePath::AppendRelativePath(const FilePath& child,
  ...
  if (IsNetwork() && parent_components.size() > 1) {
    if (*parent_comp++ != *child_comp++ ||
        !cr::EqualsCaseInsensitiveASCII(*parent_comp++, *child_comp++)) {
      return false;
    }
  }
...
FilePath FilePath::InsertBeforeExtension(StringPieceType suffix) const {
  ...
  return FilePath(
      cr::StrCat({RemoveExtension().value(), suffix, Extension()}));
}
...
std::string FilePath::MaybeAsASCII() const {
  return cr::IsStringASCII(path_) ? WideToASCII(path_) : std::string();
}
...
std::string FilePath::AsUTF8Unsafe() const {
#if defined(MINI_CHROMIUM_SYSTEM_NATIVE_UTF8)
  return value();
#else
...
int FilePath::CompareIgnoreCase(StringPieceType string1,
  ...
  // default.
  win::CharUpperWFunction char_upper_proc = nullptr;
  if (!win::IsUser32AndGdi32Available()) {
    char_upper_proc = win::GetKernelBaseCharUpperW();
    CR_CHECK(char_upper_proc);
  } else {
    char_upper_proc = ::CharUpperW;
  }
  ...
  for ( ; i1 != string1end && i2 != string2end; ++i1, ++i2) {
    wchar_t c1 =
        (wchar_t)LOWORD(char_upper_proc((LPWSTR)(DWORD_PTR)MAKELONG(*i1, 0)));
    wchar_t c2 =
        (wchar_t)LOWORD(char_upper_proc((LPWSTR)(DWORD_PTR)MAKELONG(*i2, 0)));
    if (c1 < c2)
      return -1;
    if (c1 > c2)
      return 1;
