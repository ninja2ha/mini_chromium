...
// Copied from file_util_linux.cc
#include <errno.h>
...
bool VerifySpecificPathControlledByUser(const FilePath& path,
  ...
  if (File::Lstat(path.value().c_str(), &stat_info) != 0) {
    CR_DPLOG(Error) << "Failed to get information on path "
                    << path.value();
    return false;
  }

  if (S_ISLNK(stat_info.st_mode)) {
    CR_DLOG(Error) << "Path " << path.value() << " is a symbolic link.";
    return false;
  }

  if (stat_info.st_uid != owner_uid) {
    CR_DLOG(Error) << "Path " << path.value() << " is owned by the wrong user.";
    return false;
  }

  if ((stat_info.st_mode & S_IWGRP) &&
      !Contains(group_gids, stat_info.st_gid)) {
    CR_DLOG(Error) << "Path " << path.value()
                   << " is writable by an unprivileged group.";
    return false;
  }

  if (stat_info.st_mode & S_IWOTH) {
    CR_DLOG(Error) << "Path " << path.value() << " is writable by any user.";
    return false;
  }
...
bool AdvanceEnumeratorWithStat(FileEnumerator* traversal,
  ...
  CR_DCHECK(out_next_path);
  CR_DCHECK(out_next_stat);
...
bool DoCopyDirectory(const FilePath& from_path,
  ...
  CR_DCHECK(to_path.value().find('*') == std::string::npos);
  CR_DCHECK(from_path.value().find('*') == std::string::npos);
  ...
  if (File::Stat(from_path.value().c_str(), &from_stat) < 0) {
    CR_DPLOG(Error) << "CopyDirectory() couldn't stat source directory: "
                    << from_path.value();
    return false;
  }
  ...
  CR_DCHECK(recursive || S_ISDIR(from_stat.st_mode));
  ...
  do {
    ...
    if (S_ISDIR(from_stat.st_mode)) {
      ...
      CR_DPLOG(Error) << "CopyDirectory() couldn't create directory: "
                      << target_path.value();
      return false;
    }

    if (!S_ISREG(from_stat.st_mode)) {
      CR_DLOG(Warning) << "CopyDirectory() skipping non-regular file: "
                       << current.value();
      continue;
    }
    ...
    if (!infile.IsValid()) {
      CR_DPLOG(Error) << "CopyDirectory() couldn't open file: " 
                      << current.value();
      return false;
    }
    ...
    if (File::Fstat(infile.GetPlatformFile(), &stat_at_use) < 0) {
      CR_DPLOG(Error) << "CopyDirectory() couldn't stat file: " 
                      << current.value();
      return false;
    }

    if (!S_ISREG(stat_at_use.st_mode)) {
      CR_DPLOG(Warning) << "CopyDirectory() skipping non-regular file: "
                        << current.value();
      continue;
    }
    ...
    // use the cr::File constructor. On Chrome OS, cr::File uses a different
    // set of permissions than it does on other POSIX platforms.
    int mode = 0600;

    File outfile(open(target_path.value().c_str(), open_flags, mode));
    if (!outfile.IsValid()) {
      CR_DPLOG(Error) << "CopyDirectory() couldn't create file: "
                      << target_path.value();
      return false;
    }

    if (!CopyFileContents(infile, outfile)) {
      CR_DLOG(Error) << "CopyDirectory() couldn't copy file: " 
                     << current.value();
      return false;
    }
...
bool DoDeleteFile(const FilePath& path, bool recursive) {
  ...
  bool success = true;
  cr::Stack<std::string> directories;
...
bool CreateSymbolicLink(const FilePath& target_path,
  ...
  CR_DCHECK(!symlink_path.empty());
  CR_DCHECK(!target_path.empty());
...
bool ReadSymbolicLink(const FilePath& symlink_path, FilePath* target_path) {
  CR_DCHECK(!symlink_path.empty());
  CR_DCHECK(target_path);
  char buf[PATH_MAX];
  ssize_t count =
      ::readlink(symlink_path.value().c_str(), buf, cr::size(buf));
...
bool SetPosixFilePermissions(const FilePath& path,
                             int mode) {
  ///ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
  CR_DCHECK((mode & ~FILE_PERMISSION_MASK) == 0);
...
bool ExecutableExistsInPath(Environment* env,
  ...
  if (!env->GetVar("PATH", &path)) {
    CR_LOG(Error) << "No $PATH variable. Assuming no " << executable << ".";
    return false;
  }
...
FilePath FormatTemporaryFileName(FilePath::StringPieceType identifier) {
  StringPiece prefix = "org.mini_chromium.base";
...
static bool CreateTemporaryDirInDirImpl(const FilePath& base_dir,
  ...
  CR_DCHECK(EndsWith(name_tmpl.value(), "XXXXXX"))
...
// file.
bool ReadFileToStringNonBlocking(const FilePath& file, std::string* ret) {
  CR_DCHECK(ret);
  ret->clear();

  cr::ScopedFD fd(HANDLE_EINTR(
...
bool AllocateFileRegion(File* file, int64_t offset, size_t size) {
  ...
  if (original_file_len < 0) {
    CR_DPLOG(Error) << "fstat " << file->GetPlatformFile();
    return false;
  }
  ...
  if (!file->SetLength(std::max(original_file_len, new_file_len))) {
    CR_DPLOG(Error) << "ftruncate " << file->GetPlatformFile();
    return false;
  }
  ...
  if (File::Fstat(file->GetPlatformFile(), &statbuf) == 0 &&
      statbuf.st_blksize > 0 && cr::bits::IsPowerOfTwo(statbuf.st_blksize)) {
    block_size = statbuf.st_blksize;
  }

  // Write starting at the next block boundary after the old file length.
  const int64_t extension_start =
      cr::bits::AlignUp(original_file_len, block_size);
...
bool AppendToFile(const FilePath& filename, const char* data, int size) {
  ...
  if (fd < 0) {
    CR_DPLOG(Warning) << "Unable to create file " << filename.value();
    return false;
  }

  // This call will either write all of the data or return false.
  if (!WriteFileDescriptor(fd, data, size)) {
    CR_DPLOG(Warning) << "Error while writing to file " << filename.value();
    ret = false;
  }

  if (IGNORE_EINTR(close(fd)) < 0) {
    CR_DPLOG(Warning) << "Error while closing file " << filename.value();
    return false;
  }
...
bool VerifyPathControlledByUser(const FilePath& base,
  ...
  if (base != path && !base.IsParent(path)) {
     CR_DLOG(Error) << "|base| must be a subdirectory of |path|.  base = \""
  ...
  for (ib = base_components.begin(), ip = path_components.begin();
    ...
    CR_DCHECK(ip != path_components.end());
    CR_DCHECK(*ip == *ib);
  }
...
#if defined(MINI_CHROMIUM_OS_LINUX)
bool IsPathExecutable(const FilePath& path) {
  bool result = false;
  ...
  if (fd.is_valid()) {
    ...
    CR_CHECK(sysconf_result >= 0);
    size_t pagesize = static_cast<size_t>(sysconf_result);
    CR_CHECK(sizeof(pagesize) >= sizeof(sysconf_result));
