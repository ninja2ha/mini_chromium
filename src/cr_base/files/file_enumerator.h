...
// Example:
//
//   cr::FileEnumerator e(my_dir, false, cr::FileEnumerator::FILES,
//                        FILE_PATH_LITERAL("*.txt"));
//   for (cr::FilePath name = e.Next(); !name.empty(); name = e.Next())
//     ...
class CRBASE_EXPORT FileEnumerator {
 public:
  ...
  class CRBASE_EXPORT FileInfo {
   public:
    ...
    inline const WIN32_FIND_DATAW& find_data() const {
      return *cr::win::ToWinType(&find_data_); 
    }
#elif defined(MINI_CHROMIUM_OS_POSIX)
 ...
 private:
  ...
  cr::Stack<FilePath> pending_paths_;
};
