...
#ifndef MINI_CHROMIUM_SRC_CRBASE_THREADING_SEQUENCE_TOKEN_H_
#define MINI_CHROMIUM_SRC_CRBASE_THREADING_SEQUENCE_TOKEN_H_
...
class CRBASE_EXPORT SequenceToken {
 public:
  ...
  intptr_t ToInternalValue() const;
 ...
 private:
  explicit SequenceToken(intptr_t token)
      : token_(token) {}

  static constexpr intptr_t kInvalidSequenceToken = -1;
  intptr_t token_ = kInvalidSequenceToken;
};
...
class CRBASE_EXPORT TaskToken {
 ...
 private:
  ...
  explicit TaskToken(intptr_t token) : token_(token) {}
  ...
  static constexpr intptr_t kInvalidTaskToken = -1;
  intptr_t token_ = kInvalidTaskToken;
};

// Instantiate this in the scope where a single task runs.
class CRBASE_EXPORT ScopedSetSequenceTokenForCurrentThread {
 public:
  ScopedSetSequenceTokenForCurrentThread(
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_THREADING_SEQUENCE_TOKEN_H_