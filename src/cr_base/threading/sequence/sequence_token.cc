...
intptr_t SequenceToken::ToInternalValue() const {
  return token_;
}
...
ScopedSetSequenceTokenForCurrentThread::ScopedSetSequenceTokenForCurrentThread(
  ...
  CR_DCHECK(!GetTlsCurrentSequenceToken().Get());
  CR_DCHECK(!GetTlsCurrentTaskToken().Get());
...
ScopedSetSequenceTokenForCurrentThread::
    ~ScopedSetSequenceTokenForCurrentThread() {
  CR_DCHECK(GetTlsCurrentSequenceToken().Get() == &sequence_token_);
  CR_DCHECK(GetTlsCurrentTaskToken().Get() == &task_token_);
