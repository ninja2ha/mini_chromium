...
void PlatformThread::SetCurrentThreadPriorityImpl(ThreadPriority priority) {
  PlatformThreadHandle::Handle thread_handle =
      PlatformThreadHandle::Current().platform_handle();
...
ThreadPriority PlatformThread::GetCurrentThreadPriority() {
  ...
  const int priority =
      ::GetThreadPriority(PlatformThreadHandle::Current().platform_handle());
  ...
  switch (priority) {
    case kWin7BackgroundThreadModePriority:
      CR_DCHECK(win::GetVersion() == win::Version::WIN7);
      return ThreadPriority::BACKGROUND;
    ...
    case THREAD_PRIORITY_ERROR_RETURN:
      CR_DPCHECK(false) << "::GetThreadPriority error";
  }

  CR_NOTREACHED() << "::GetThreadPriority returned " << priority << ".";
  return ThreadPriority::NORMAL;
}
