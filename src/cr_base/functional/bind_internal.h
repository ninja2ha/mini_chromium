...
#include "cr_base/functional/callback_internal.h"

// See cr_base/functional/callback.h for user documentation.
//
//
// CONCEPTS:
...
template <typename T>
struct NeedsRefPtrButGetsRawPtr
    : internal::conjunction<std::is_pointer<T>,
...
class RetainedRefWrapper {
 public:
  ...
  explicit RetainedRefWrapper(RefPtr<T> o) : ptr_(std::move(o)) {}
...
template <typename Callable>
struct IsCallableObject<Callable, 
                        internal::void_t<decltype(&Callable::operator())>>
    : std::true_type {};
...
struct HasRefCountedTypeAsRawPtr
    : internal::disjunction<NeedsRefPtrButGetsRawPtr<Ts>...> {};
...
#if defined(MINI_CHROMIUM_OS_WIN) && !defined(MINI_CHROMIUM_ARCH_CPU_64_BITS)
...
struct FunctorTraits<R(__fastcall*)(Args...)> {
  ...
  static R Invoke(R(__fastcall* function)(Args...), RunArgs&&... args) {
    return function(std::forward<RunArgs>(args)...);
        ...
        // !defined(MINI_CHROMIUM_ARCH_CPU_64_BITS)
...
BanUnconstructedRefCountedReceiver(const Receiver& receiver, Unused&&...) {
  ...
  // In the example below, cr::BindOnce() makes the implicit first reference
  ...
  // another reference.
  //   Foo::Foo() {
  //     cr::PostTask(CR_FROM_HERE, cr::BindOnce(&Foo::Bar, this));
  //   }
  //
  //   RefPtr<Foo> oo = new Foo();
  //
  ...
  //     auto foo = cr::WrapRefCounted(new Foo());
  //     cr::PostTask(CR_FROM_HERE, cr::BindOnce(&Foo::Bar, foo));
  //     return foo;
  //   }
  //
  //   Foo::Foo() {}
  //
  //   cr::RefPtr<Foo> oo = Foo::Create();
  CR_DCHECK(receiver->HasAtLeastOneRef())
      << "cr::Bind{Once,Repeating}() refuses to create the first reference "
...
struct MakeBindStateTypeImpl<true, Functor, Receiver, BoundArgs...> {
 private:
  ...
  static_assert(
      ...
      " and has no lifetime concerns, use cr::Unretained() and document why"
      " it's safe.");
  static_assert(!HasRefCountedTypeAsRawPtr<std::decay_t<BoundArgs>...>::value,
                "A parameter is a refcounted type and needs scoped_refptr.");
 ...
 public:
  using Type = BindState<
      ...
      std::conditional_t<std::is_pointer<DecayedReceiver>::value,
                         RefPtr<std::remove_pointer_t<DecayedReceiver>>,
                         DecayedReceiver>,
...
struct AssertConstructible {
 private:
  ...
  static_assert(
      ...
      "cr::BindRepeating() argument is a move-only type. Use cr::Passed() "
...
decltype(auto) BindImpl(Functor&& functor, Args&&... args) {
  ...
  PolymorphicInvoke invoke_func = 
      GetInvokeFunc<Invoker>(internal::bool_constant<kIsOnce>());
...
// method, cr::Bind cancels the method invocation if the receiver is tested as
// false.
// E.g. Foo::bar() is not called:
//   struct Foo : cr::SupportsWeakPtr<Foo> {
//     void bar() {}
//   };
//
//   WeakPtr<Foo> oo = nullptr;
//   cr::BindOnce(&Foo::bar, oo).Run();
template <typename T>
