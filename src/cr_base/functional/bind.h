...
#include "cr_base/functional/bind_internal.h"
...
//
// Overview:
// cr::BindOnce() and cr::BindRepeating() are helpers for creating
// cr::OnceCallback and cr::RepeatingCallback objects respectively.
//
// For a runnable object of n-arity, the cr::Bind*() family allows partial
// application of the first m arguments. The remaining n - m arguments must be
// passed when invoking the callback with Run().
//
//   // The first argument is bound at callback creation; the remaining
//   // two must be passed when calling Run() on the callback object.
//   cr::OnceCallback<long(int, long)> cb = cr::BindOnce(
...
//
//   class C : public cr::RefCounted<C> { void F(); };
//   auto instance = cr::MakeRefCounted<C>();
//   auto cb = cr::BindOnce(&C::F, instance);
//   std::move(cb).Run();  // Identical to instance->F()
//
// cr::Bind is currently a type alias for cr::BindRepeating(). In the
// future, we expect to flip this to default to cr::BindOnce().
//
...
inline OnceCallback<internal::MakeUnboundRunType<Functor, Args...>> BindOnce(
  ...
  static_assert(!internal::IsOnceCallback<std::decay_t<Functor>>() ||
                ...
                " I.e.: cr::BindOnce(std::move(callback)).");
  static_assert(
      internal::conjunction<
          internal::AssertBindArgIsNotBasePassed<std::decay_t<Args>>...>::value,
      "Use std::move() instead of cr::Passed() with cr::BindOnce()");
...
// Special cases for binding to a cr::Callback without extra bound arguments.
// We CR_CHECK() the validity of callback to guard against null pointers
...
//
//    RefPtr<RefCountedBytes> bytes = ...;
//    OnceClosure callback = BindOnce(&foo, cr::RetainedRef(bytes));
...
template <typename T>
inline internal::RetainedRefWrapper<T> RetainedRef(RefPtr<T> o) {
...
// argument will CR_CHECK() because the first invocation would have already
...
//   cb.Run();  // This CR_CHECK()s since Foo() already been used once.
//
...
//
//   // Prints "2" on |ml|.
//   ml->PostTask(CR_FROM_HERE, BindOnce(IgnoreResult(&DoSomething), 2);
template <typename T>
