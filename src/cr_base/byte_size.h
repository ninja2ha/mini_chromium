...
// * VERSION: 151.0.7905.1

#ifndef MINI_CHROMIUM_SRC_CRBASE_BYTE_SIZE_H_
#define MINI_CHROMIUM_SRC_CRBASE_BYTE_SIZE_H_

#include <cstdint>
#include <iosfwd>

#include "cr_base/base_export.h"
#include "cr_base/numerics/checked_math.h"
...
class ByteSizeBase {
 public:
  ...
  // Comparators.
  constexpr friend bool operator==(ByteSizeBase a, ByteSizeBase b) {
 ...
 protected:
  ...
  constexpr explicit ByteSizeBase(T bytes)
      : bytes_(CheckedCast<int64_t>(bytes)) {}

  // Conversion helper functions.
  template <typename ReturnType>
  constexpr ReturnType InBytesImpl() const {
    return cr::CheckedCast<ReturnType>(bytes_);
  }
  template <typename ReturnType>
  ...
  template <typename ResultType, 
            typename = std::enable_if_t<
                std::is_base_of_v<ByteSizeBase, ResultType>> >
...
class CRBASE_EXPORT ByteSize : public internal::ByteSizeBase {
 public:
  ...
  constexpr explicit ByteSize(T bytes)
      ...
      : ByteSizeBase(CheckedCast<uint64_t>(bytes)) {}
 ...
 private:
  ...
  friend class internal::ByteSizeBase;

  constexpr explicit ByteSize(CheckedNumeric<int64_t> checked_bytes)
      : ByteSizeBase(CheckedCast<uint64_t>(checked_bytes.ValueOrDie())) {}
};

// A signed number of bytes, in the range [min(int64_t)...max(int64_t)].
class CRBASE_EXPORT ByteSizeDelta : public internal::ByteSizeBase {
 public:
  ...
  constexpr ByteSize AsByteSize() const {
    return ByteSize(CheckedCast<uint64_t>(InBytes()));
  }
...
constexpr ByteSize KiBU(T kib) {
  return ByteSize(CheckedCast<uint64_t>(kib * 1024.0));
}

template <typename T, 
          std::enable_if_t<std::is_signed_v<T>>* = nullopt>
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSizeDelta KiBS(T kib) {
  return ByteSizeDelta(CheckedCast<int64_t>(kib * 1024.0));
}
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSize MiBU(T mib) {
  return ByteSize(CheckedCast<uint64_t>(mib * 1024.0 * 1024.0));
}

template <typename T, 
          std::enable_if_t<std::is_signed_v<T>>* = nullptr>
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSizeDelta MiBS(T mib) {
  return ByteSizeDelta(CheckedCast<int64_t>(mib * 1024.0 * 1024.0));
}
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSize GiBU(T gib) {
  return ByteSize(CheckedCast<uint64_t>(gib * 1024.0 * 1024.0 * 1024.0));
}
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSizeDelta GiBS(T gib) {
  return ByteSizeDelta(CheckedCast<int64_t>(gib * 1024.0 * 1024.0 * 1024.0));
}
...
constexpr ByteSize TiBU(T tib) {
  return ByteSize(
      CheckedCast<uint64_t>(tib * 1024.0 * 1024.0 * 1024.0 * 1024.0));
}
...
constexpr ByteSizeDelta TiBS(T tib) {
  return ByteSizeDelta(
      CheckedCast<int64_t>(tib * 1024.0 * 1024.0 * 1024.0 * 1024.0));
}
...
constexpr ByteSize PiBU(T pib) {
  return ByteSize(
      CheckedCast<uint64_t>(pib * 1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0));
}
...
constexpr ByteSizeDelta PiBS(T pib) {
  return ByteSizeDelta(
      CheckedCast<int64_t>(pib * 1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0));
}
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSize EiBU(T eib) {
  return ByteSize(CheckedCast<uint64_t>(eib * 1024.0 * 1024.0 * 1024.0 *
...
template <typename T, 
          std::enable_if_t<std::is_floating_point_v<T>>* = nullptr>
constexpr ByteSizeDelta EiBS(T eib) {
  return ByteSizeDelta(CheckedCast<int64_t>(eib * 1024.0 * 1024.0 * 1024.0 *
                                            1024.0 * 1024.0 * 1024.0));
}

// Stream operators for logging and testing.
CRBASE_EXPORT std::ostream& operator<<(std::ostream& os, ByteSize size);
CRBASE_EXPORT std::ostream& operator<<(std::ostream& os, ByteSizeDelta delta);
...
constexpr ByteSizeDelta ByteSize::AsByteSizeDelta() const {
  return ByteSizeDelta(CheckedCast<int64_t>(InBytes()));
}
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_BYTE_SIZE_H_