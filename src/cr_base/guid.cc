...
std::string GetCanonicalGUIDInternal(StringPieceType input, bool strict) {
  ...
  for (size_t i = 0; i < input.length(); ++i) {
    ...
    if (IsHyphenPosition(i)) {
      ...
      if (strict ? !IsLowerHexDigit(current) : !IsAsciiHexDigit(current))
        return std::string();
...
std::string DataToGUIDString(const uint64_t bytes[2]) {
  return StringPrintf(
...
GUID GUID::GenerateRandomV4() {
  ...
  GUID guid;
  guid.lowercase_ = DataToGUIDString(sixteen_bytes);
  return guid;
}
