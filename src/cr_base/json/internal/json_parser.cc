...
JSONParser::JSONParser(int options, size_t max_depth)
    : options_(options),
      ...
      error_line_(0),
      error_column_(0) {
  CR_CHECK(max_depth <= kAbsoluteMaxDepth);
}
...
Optional<Value> JSONParser::Parse(StringPiece input) {
  ...
  if (!cr::IsValueInRangeForNumericType<int32_t>(input.length())) {
...
void JSONParser::StringBuilder::Append(uint32_t point) {
  CR_DCHECK(IsValidCodepoint(point));

  if (point < kExtendedASCIIStart && !string_) {
    CR_DCHECK(static_cast<char>(point) == pos_[length_]);
    ++length_;
  } else {
    Convert();
    if (CR_UNLIKELY(point == kUnicodeReplacementPoint)) {
...
const char* JSONParser::pos() {
  CR_CHECK(static_cast<size_t>(index_) <= input_.length());
...
bool JSONParser::DecodeUTF16(uint32_t* out_code_point) {
  ...
  if (CBU16_IS_SURROGATE(code_unit16_high)) {
    ...
    *out_code_point = code_point;
    ...
    // Not a surrogate.
    CR_DCHECK(CBU16_IS_SINGLE(code_unit16_high));
