...
JSONWriter::JSONWriter(int options, std::string* json, size_t max_depth)
    : omit_binary_values_((options & OPTIONS_OMIT_BINARY_VALUES) != 0),
      ...
      stack_depth_(0) {
  CR_DCHECK(json);
  CR_CHECK(max_depth <= internal::kAbsoluteMaxDepth);
}
...
bool JSONWriter::BuildJSONString(const Value& node, size_t depth) {
  ...
  switch (node.type()) {
    ...
    case Value::Type::STRING:
      internal::EscapeJSONString(node.GetString(), true, json_string_);
      return true;
    ...
    case Value::Type::DICTIONARY: {
      ...
      for (const auto& pair : node.DictItems()) {
        ...
        internal::EscapeJSONString(key, true, json_string_);
        json_string_->push_back(':');
        if (pretty_print_)
          json_string_->push_back(' ');
    ...
    case Value::Type::BINARY:
      ...
      CR_DLOG_IF(Error, !omit_binary_values_) << "Cannot serialize binary value.";
