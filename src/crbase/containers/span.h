...
#include "crbase/containers/internal/checked_iterators.h"
#include "crbase/containers/internal/contiguous_iterator.h"
...
class Span : public internal::ExtentStorage<Extent> {
 ...
 public:
  ...
  constexpr Span(It first, size_t count) noexcept
    ...
    CR_CHECK(Extent == dynamic_extent || Extent == count);
  }
  ...
  constexpr Span<T, Count> first() const noexcept {
    ...
    CR_CHECK(Extent != dynamic_extent || Count <= size());
    return {data(), Count};
  }
  ...
  subspan() const noexcept {
    ...
    CR_CHECK(Extent != dynamic_extent || Offset <= size());
    CR_CHECK(Extent != dynamic_extent || Count == dynamic_extent ||
             Count <= size() - Offset);
  ...
  constexpr Span<T, dynamic_extent> subspan(size_t offset,
    ...
    CR_CHECK(count == dynamic_extent || count <= size() - offset);
    return {data() + offset, count != dynamic_extent ? count : size() - offset};
  }
  ...
  constexpr T& front() const noexcept {
    ...
    CR_CHECK(Extent != dynamic_extent || !empty());
    return *data();
  }
