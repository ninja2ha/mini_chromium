...
#ifndef MINI_CHROMIUM_SRC_CRBASE_HELPER_SCOPED_GENERIC_H_
#define MINI_CHROMIUM_SRC_CRBASE_HELPER_SCOPED_GENERIC_H_
...
class ScopedGeneric {
 ...
 public:
  ...
  virtual ~ScopedGeneric() {
    CR_CHECK(!receiving_) << "ScopedGeneric destroyed with active receiver";
    FreeIfNecessary();
  }
  ...
  class Receiver {
   public:
    explicit Receiver(ScopedGeneric& parent) : scoped_generic_(&parent) {
      CR_CHECK(!scoped_generic_->receiving_)
    ...
    Receiver(Receiver&& move) {
      CR_CHECK(!used_) << "moving into already-used Receiver";
      CR_CHECK(!move.used_) << "moving from already-used Receiver";
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_HELPER_SCOPED_GENERIC_H_
