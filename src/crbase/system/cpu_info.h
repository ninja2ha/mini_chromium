...
#ifndef MINI_CHROMIUM_SRC_CRBASE_SYSTEM_CPU_INFO_H_
#define MINI_CHROMIUM_SRC_CRBASE_SYSTEM_CPU_INFO_H_
...
CRBASE_EXPORT X86ModelInfo ComputeX86FamilyAndModel(const std::string& vendor,
                                                    int signature);
...
class CRBASE_EXPORT CPUInfo final {
 public:
  CPUInfo();
  CPUInfo(CPUInfo&&);
  CPUInfo(const CPUInfo&) = delete;

  // Get a preallocated instance of CPUInfo.
  ...
  // to disable a feature by attacking this data, cr::ProtectedMemory is used
  // to protected it from write accesses.
  // Note that cr::ProtectedMemory falls back to unprotected memory if the
  // target OS is not supported.
  static const CPUInfo& GetInstanceNoAllocation();
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_SYSTEM_CPU_INFO_H_