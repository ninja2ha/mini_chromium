...
bool ParseVersionNumbers(StringPiece version_str,
  ...
  for (auto it = numbers.begin(); it != numbers.end(); ++it) {
    ...
    if (it == numbers.begin() && cr::StringPiece(NumberToString(num)) != *it)
      return false;
...
Version::Version(WStringPiece version_str) {
  std::string ver = cr::WideToASCII(version_str);
