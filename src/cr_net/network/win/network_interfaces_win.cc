...
#include "cr_net/network/win/network_interfaces_win.h"

#include <algorithm>
#include <memory>

#include "cr_base/files/file_path.h"
///#include "cr_base/lazy_instance.h"
...
//#include "cr_base/threading/scoped_blocking_call.h"
///#include "cr_base/threading/scoped_thread_priority.h"
#include "cr_base/win/scoped_handle.h"
///#include "cr_net/base/escape.h"
#include "cr_net/base/ip_endpoint.h"
#include "cr_net/base/net_errors.h"
#include "cr_url/gurl.h"

#ifndef LOAD_LIBRARY_SEARCH_SYSTEM32
#define LOAD_LIBRARY_SEARCH_SYSTEM32 (0x1000)
#endif
...
WlanApi& WlanApi::GetInstance() {
  static cr::NoDestructor<WlanApi> inst;
  return *inst;
}
...
WlanApi::WlanApi() : initialized(false) {
  ...
  HMODULE module =
      ::LoadLibraryExW(L"wlanapi.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
  if (!module)
    return;
    ...
    for (IP_ADAPTER_UNICAST_ADDRESS* address = adapter->FirstUnicastAddress;
      ...
      if (family == AF_INET || family == AF_INET6) {
        ...
        if (endpoint.FromSockAddr(address->Address.lpSockaddr,
          ...
          uint32_t prefix_length = address->OnLinkPrefixLength;
          ...
          networks->push_back(NetworkInterface(
              adapter->AdapterName,
              cr::SysWideToNativeMB(adapter->FriendlyName), index,
...
bool GetNetworkList(NetworkInterfaceList* networks, int policy) {
  ...
  IP_ADAPTER_ADDRESSES* adapters = nullptr;
  {
    ...
    if (result == ERROR_NO_DATA) {
      ...
      CR_LOG(Error) << "GetAdaptersAddresses failed: " << result;
      return false;
    }
  }
