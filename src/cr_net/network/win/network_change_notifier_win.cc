...
#include "cr_event/task/task_traits.h"
#include "cr_event/task/sequenced_task_runner.h"
#include "cr_event/task/single_thread_task_runner.h"
#include "cr_event/task_runner_util.h"
#include "cr_event/threading/thread.h"
...
class NetworkChangeNotifierWin::DnsConfigServiceThread : public cr::Thread {
public:
  DnsConfigServiceThread(const DnsConfigServiceThread&) = delete;
  DnsConfigServiceThread& operator=(const DnsConfigServiceThread&) = delete;

  DnsConfigServiceThread() : cr::Thread("DnsConfigService") {}
...
NetworkChangeNotifierWin::NetworkChangeCalculatorParamsWin() {
  ...
  params.ip_address_offline_delay_ = cr::TimeDelta::FromMilliseconds(1500);
  params.ip_address_online_delay_ = cr::TimeDelta::FromMilliseconds(1500);
  params.connection_type_offline_delay_ =
      cr::TimeDelta::FromMilliseconds(1500);
  params.connection_type_online_delay_ = cr::TimeDelta::FromMilliseconds(500);
  return params;
}
...
NetworkChangeNotifierWin::RecomputeCurrentConnectionType() {
  ...
  if (0 != WSALookupServiceBeginW(&query_set, LUP_RETURN_ALL,
                                  &ws_handle)) {
    CR_LOG(Error) << "WSALookupServiceBegin failed with: " << WSAGetLastError();
  ...
  if (result == 0) {
    ...
    if (result == WSAEFAULT) {
      ...
      CR_LOG(Warning) << "WSALookupServiceNext() failed with:" << result;
    }
  }

  result = WSALookupServiceEnd(ws_handle);
  CR_LOG_IF(Error, result != 0)
...
void NetworkChangeNotifierWin::RecomputeCurrentConnectionTypeOnBlockingSequence(
  ...
  cr::PostTaskAndReplyWithResult(
      dns_config_service_thread_->task_runner().get(), CR_FROM_HERE,
      cr::BindOnce(&NetworkChangeNotifierWin::RecomputeCurrentConnectionType),
      std::move(reply_callback));
}

NetworkChangeNotifier::ConnectionType
NetworkChangeNotifierWin::GetCurrentConnectionType() const {
  cr::AutoLock auto_lock(last_computed_connection_type_lock_);
...
void NetworkChangeNotifierWin::OnObjectSignaled(HANDLE object) {
  CR_DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  CR_DCHECK(is_watching_);
  is_watching_ = false;

  // Start watching for the next address change.
  WatchForAddressChange();

  RecomputeCurrentConnectionTypeOnBlockingSequence(cr::BindOnce(
...
void NetworkChangeNotifierWin::NotifyObservers(ConnectionType connection_type) {
  ...
  // delay again.
  offline_polls_ = 0;
  timer_.Start(CR_FROM_HERE, cr::TimeDelta::FromSeconds(1), this,
...
void NetworkChangeNotifierWin::WatchForAddressChange() {
  ...
  if (!WatchForAddressChangeInternal()) {
    ...
    cr::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        CR_FROM_HERE,
        cr::BindOnce(&NetworkChangeNotifierWin::WatchForAddressChange,
                     weak_factory_.GetWeakPtr()),
        cr::TimeDelta::FromMilliseconds(
  ...
  if (sequential_failures_ > 0) {
    RecomputeCurrentConnectionTypeOnBlockingSequence(
        cr::BindOnce(&NetworkChangeNotifierWin::NotifyObservers,
                     weak_factory_.GetWeakPtr()));
  }
...
bool NetworkChangeNotifierWin::WatchForAddressChangeInternal() {
  ...
  if (!dns_config_service_thread_->IsRunning()) {
    dns_config_service_thread_->StartWithOptions(
        cr::Thread::Options(cr::MessagePumpType::IO, 0));
  }
  ...
  addr_watcher_.StartWatchingOnce(addr_overlapped_.hEvent, this, CR_FROM_HERE);
  return true;
}
