...
#include "cr_net/network/network_change_notifier_factory.h"
#include "cr_net/network/network_interfaces.h"
///#include "cr_net/base/url_util.h"
...
///#include "cr_net/url_request/url_request.h"
#include "cr_url/gurl.h"

#if defined(MINI_CHROMIUM_OS_WIN)
#include "cr_net/network/win/network_change_notifier_win.h"
// TODO(crbug.com/1052397): Revisit the macro expression once build flag switch
// of lacros-chrome is complete.
#elif defined(MINI_CHROMIUM_OS_LINUX)
#include "cr_net/network/linux/network_change_notifier_linux.h"
#endif
...
cr::Lock& NetworkChangeNotifierCreationLock() {
  static cr::NoDestructor<cr::Lock> instance;
  return *instance;
}
...
class NetworkChangeNotifier::NetworkChangeCalculator
 ...
 public:
  ...
  void Init() {
    ...
    CR_DCHECK(g_network_change_notifier);
  ...
  ///  cr::TimeDelta delay = last_announced_connection_type_ == CONNECTION_NONE
  ///      ? params_.ip_address_offline_delay_ : params_.ip_address_online_delay_;
  ///  // Cancels any previous timer.
  ///  timer_.Start(CR_FROM_HERE, delay, this, &NetworkChangeCalculator::Notify);
  ///}
...
///
///  void OnSystemDnsConfigChanged(cr::Optional<DnsConfig> config) override {
...
void NetworkChangeNotifier::ClearGlobalPointer() {
  if (!cleared_global_pointer_) {
    ...
    CR_DCHECK(this == g_network_change_notifier);
...
void NetworkChangeNotifier::SetFactory(
  ...
  CR_CHECK(!g_network_change_notifier_factory);
...
std::unique_ptr<NetworkChangeNotifier> NetworkChangeNotifier::CreateIfNeeded(
  ...
  cr::AutoLock auto_lock(NetworkChangeNotifierCreationLock());
...
void NetworkChangeNotifier::GetConnectedNetworks(NetworkList* network_list) {
  CR_DCHECK(AreNetworkHandlesSupported());
  if (g_network_change_notifier) {
...
const char* NetworkChangeNotifier::ConnectionTypeToString(
  ...
  static_assert(cr::size(kConnectionTypeNames) ==
...
NetworkChangeNotifier::ConnectionTypeFromInterfaceList(
  ...
  for (size_t i = 0; i < interfaces.size(); ++i) {
    ...
    if (cr::ToLowerASCII(interfaces[i].friendly_name).find("vmnet") !=
        std::string::npos) {
      continue;
    }
    if (first) {
...
NetworkChangeNotifier::NetworkChangeNotifier(
    ...
    : ///ip_address_observer_list_(
      ///    new cr::ObserverListThreadSafe<IPAddressObserver>(
      ///        cr::ObserverListPolicy::EXISTING_ONLY)),
      ///connection_type_observer_list_(
      ///    new cr::ObserverListThreadSafe<ConnectionTypeObserver>(
      ///        cr::ObserverListPolicy::EXISTING_ONLY)),
      ///resolver_state_observer_list_(
      ///    new cr::ObserverListThreadSafe<DNSObserver>(
      ///        cr::ObserverListPolicy::EXISTING_ONLY)),
      network_change_observer_list_(
          new cr::ObserverListThreadSafe<NetworkChangeObserver>(
              cr::ObserverListPolicy::EXISTING_ONLY)),
      max_bandwidth_observer_list_(
          new cr::ObserverListThreadSafe<MaxBandwidthObserver>(
              cr::ObserverListPolicy::EXISTING_ONLY)),
      network_observer_list_(new cr::ObserverListThreadSafe<NetworkObserver>(
          cr::ObserverListPolicy::EXISTING_ONLY)),
      connection_cost_observer_list_(
          new cr::ObserverListThreadSafe<ConnectionCostObserver>(
              cr::ObserverListPolicy::EXISTING_ONLY)),
  ...
  ///  static cr::NoDestructor<SystemDnsConfigChangeNotifier> singleton{};
...
///  CR_DCHECK(system_dns_config_notifier_);
...
void NetworkChangeNotifier::NotifyObserversOfNetworkChangeImpl(
  ...
  network_change_observer_list_->Notify(
      CR_FROM_HERE, &NetworkChangeObserver::OnNetworkChanged, type);
}

///void NetworkChangeNotifier::NotifyObserversOfDNSChangeImpl() {
///  resolver_state_observer_list_->Notify(CR_FROM_HERE, &DNSObserver::OnDNSChanged);
///}
...
void NetworkChangeNotifier::NotifyObserversOfMaxBandwidthChangeImpl(
  ...
  max_bandwidth_observer_list_->Notify(
      CR_FROM_HERE, &MaxBandwidthObserver::OnMaxBandwidthChanged,
      max_bandwidth_mbps, type);
}
  ...
  switch (type) {
    case CONNECTED:
      network_observer_list_->Notify(
          CR_FROM_HERE, &NetworkObserver::OnNetworkConnected, network);
      break;
    case DISCONNECTED:
      network_observer_list_->Notify(
          CR_FROM_HERE, &NetworkObserver::OnNetworkDisconnected, network);
      break;
    case SOON_TO_DISCONNECT:
      network_observer_list_->Notify(
          CR_FROM_HERE, &NetworkObserver::OnNetworkSoonToDisconnect, network);
      break;
    case MADE_DEFAULT:
      network_observer_list_->Notify(
          CR_FROM_HERE, &NetworkObserver::OnNetworkMadeDefault, network);
      break;
  }
}
...
void NetworkChangeNotifier::NotifyObserversOfConnectionCostChangeImpl(
  ...
  connection_cost_observer_list_->Notify(
      CR_FROM_HERE, &ConnectionCostObserver::OnConnectionCostChanged, cost);
}
