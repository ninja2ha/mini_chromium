...
class StunMessage {
 public:
  ...
  bool Read(cr::NetByteBufferReader* buf);

  // Writes this object into a STUN packet. The return value indicates whether
  // this was successful.
  bool Write(cr::NetByteBufferWriter* buf) const;
...
class StunAttribute {
 public:
  ...
  virtual bool Read(cr::NetByteBufferReader* buf) = 0;

  // Writes the body (not the type or length) to the given buffer.  Return
  // value is true if successful.
  virtual bool Write(cr::NetByteBufferWriter* buf) const = 0;
 ...
 protected:
  ...
  void WritePadding(cr::NetByteBufferWriter* buf) const;
  void ConsumePadding(cr::NetByteBufferReader* buf) const;
...
std::unique_ptr<StunAttribute> CopyStunAttribute(
    ...
    cr::NetByteBufferWriter* tmp_buffer_ptr = 0);
