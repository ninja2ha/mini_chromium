...
#include "cr_net/http/http_stream_parser.h"
...
#include "cr_net/http/http_chunked_decoder.h"
///#include "cr_net/http/http_log_util.h"
#include "cr_net/http/http_request_headers.h"
#include "cr_net/http/http_request_info.h"
...
// // data() points to the beginning of the buffer.
//
class HttpStreamParser::SeekableIOBuffer : public cr::VectorIOBuffer {
 public:
  explicit SeekableIOBuffer(size_t capacity)
      : cr::VectorIOBuffer(capacity), full_span_(span()) {}
  ...
  void SetOffset(int bytes) {
    CR_CHECK(bytes <= bytes_written_);
    bytes_consumed_ = bytes;
    SetSpan(full_span_.subspan(cr::checked_cast<size_t>(bytes_consumed_)));
  }
  ...
  void DidAppend(int bytes) {
    CR_CHECK(bytes >= 0);
    CR_CHECK((bytes_written_ + bytes) >= 0);
    CR_CHECK((bytes_written_ + bytes) <= capacity());
  ...
  int capacity() const { return cr::checked_cast<int>(full_span_.size()); }
 ...
 private:
  ...
  const cr::Span<uint8_t> full_span_;
...
HttpStreamParser::HttpStreamParser(StreamSocket* stream_socket,
                                   ...
                                   cr::GrowableIOBuffer* read_buffer/*,
    ...
    : io_state_(STATE_NONE),
      ...
      upload_error_(OK) {
  io_callback_ = cr::BindRepeating(&HttpStreamParser::OnIOComplete,
...
int HttpStreamParser::SendRequest(
  ...
  CR_DCHECK(STATE_NONE == io_state_);
  ...
  CR_DVLOG(1) << __func__ << "() request_line = \"" << request_line << "\""
  ...
  if (ShouldMergeRequestHeadersAndBody(request, request_->upload_data_stream)) {
    ...
    cr::RefPtr<cr::IOBuffer> merged_request_headers_and_body =
        cr::MakeRefCounted<cr::VectorIOBuffer>(merged_size);
    // We'll repurpose |request_headers_| to store the merged headers and
    // body.
    request_headers_ = cr::MakeRefCounted<cr::DrainableIOBuffer>(
    ...
    while (todo) {
      ...
      CR_DCHECK(consumed > 0);
      request_headers_->DidConsume(consumed);
      todo -= consumed;
    }
    CR_DCHECK(request_->upload_data_stream->IsEOF());
  ...
  if (!did_merge) {
    ...
    cr::RefPtr<cr::StringIOBuffer> headers_io_buf =
        cr::MakeRefCounted<cr::StringIOBuffer>(request);
    request_headers_ = cr::MakeRefCounted<cr::DrainableIOBuffer>(
...
int HttpStreamParser::ConfirmHandshake(CompletionOnceCallback callback) {
  int ret = stream_socket_->ConfirmHandshake(
      cr::BindOnce(&HttpStreamParser::RunConfirmHandshakeCallback,
                   weak_ptr_factory_.GetWeakPtr()));
...
int HttpStreamParser::ReadResponseHeaders(CompletionOnceCallback callback) {
  CR_DCHECK(io_state_ == STATE_NONE || io_state_ == STATE_DONE);
  CR_DCHECK(callback_.is_null());
  CR_DCHECK(!callback.is_null());
  CR_DCHECK(0 == read_buf_unused_offset_);
  CR_DCHECK(SendRequestBuffersEmpty());
...
int HttpStreamParser::ReadResponseBody(cr::IOBuffer* buf,
                                       int buf_len,
                                       CompletionOnceCallback callback) {
  CR_DCHECK(io_state_ == STATE_NONE || io_state_ == STATE_DONE);
  CR_DCHECK(callback_.is_null());
  CR_DCHECK(!callback.is_null());
  CR_DCHECK(buf_len <= kMaxBufSize);
  CR_DCHECK(SendRequestBuffersEmpty());
...
int HttpStreamParser::DoLoop(int result) {
  do {
    ...
    CR_DCHECK(STATE_DONE != io_state_);
    CR_DCHECK(STATE_NONE != io_state_);
...
int HttpStreamParser::DoSendBody() {
  ...
  return request_->upload_data_stream->Read(
      ...
      cr::BindOnce(&HttpStreamParser::OnIOComplete,
                   weak_ptr_factory_.GetWeakPtr()));
}
...
int HttpStreamParser::DoSendRequestReadBodyComplete(int result) {
  ...
  if (request_->upload_data_stream->is_chunked()) {
    ...
    const cr::StringPiece payload(request_body_read_buf_->data(), result);
  ...
  if (result == 0) {  // Reached the end.
    ...
    CR_DCHECK(request_->upload_data_stream->IsEOF());
    CR_DCHECK(!request_->upload_data_stream->is_chunked());
    // Finished sending the request.
    io_state_ = STATE_SEND_REQUEST_COMPLETE;
  } else if (result > 0) {
...
int HttpStreamParser::DoReadBodyComplete(int result) {
  ...
  if (result <= 0 || IsResponseBodyComplete()) {
    ...
    CR_CHECK((save_amount + additional_save_amount) <= kMaxBufSize);
    ...
    if (save_amount) {
      ...
      memcpy(read_buf_->everything().data(), user_read_buf_->data() + result,
             save_amount);
    }
...
int HttpStreamParser::HandleReadHeaderResult(int result) {
  ...
  CR_DCHECK(read_buf_->offset() <= read_buf_->capacity());
  CR_DCHECK(result > 0);
  ...
  if (end_of_header_offset == -1) {
    ...
    if (response_body_length_ == 0) {
      ...
      if (extra_bytes) {
        CR_CHECK(extra_bytes > 0);
        memmove(read_buf_->everything().data(),
                read_buf_->everything().data() + end_of_header_offset,
                extra_bytes);
      }
    ...
    // (non-1xx).
    if (response_->headers->response_code() / 100 != 1) {
      CR_DCHECK(non_informational_response_start_time_.is_null());
...
int HttpStreamParser::FindAndParseResponseHeaders(int new_bytes) {
  ...
  if (response_header_start_offset_ == std::string::npos) {
    response_header_start_offset_ = HttpUtil::LocateStartOfStatusLine(
        reinterpret_cast<const char*>(read_buf_->everything().data()), 
        read_buf_->offset());
  }
  ...
  if (response_header_start_offset_ != std::string::npos) {
    ...
    size_t lower_bound =
        (cr::ClampedNumeric<size_t>(read_buf_->offset()) - new_bytes - 3)
            .RawValue();
  ...
  if (rv < 0)
    return rv;
  return cr::checked_cast<int>(end_offset);
}

int HttpStreamParser::ParseResponseHeaders(size_t end_offset) {
  cr::RefPtr<HttpResponseHeaders> headers;
  CR_DCHECK(0 == read_buf_unused_offset_);

  const char* read_buf_start = reinterpret_cast<const char*>(
      read_buf_->everything().data());

  if (response_header_start_offset_ != std::string::npos) {
    received_bytes_ += cr::checked_cast<int>(end_offset);
    headers = HttpResponseHeaders::TryToCreate(
        cr::StringPiece(read_buf_start, end_offset));
    if (!headers)
      return crnet::ERR_INVALID_HTTP_RESPONSE;
  } else {
    ...
    cr::StringPiece scheme = request_->url.scheme_piece();
    if (crurl::DefaultPortForScheme(scheme.data(), 
      ...
      if (read_buf_->offset() < 3 || scheme != "http" ||
          !cr::LowerCaseEqualsASCII(cr::StringPiece(read_buf_start, 3), "icy")) {
  ...
  CR_DVLOG(1) << __func__ << "() content_length = \""
...
int HttpStreamParser::EncodeChunk(const cr::StringPiece& payload,
                                  char* output,
                                  size_t output_size) {
  ...
  char* cursor = output;
  // Add the header.
  const int num_chars = cr::snprintf(output, output_size,
                                     "%X\r\n",
  ...
  return cr::checked_cast<int>(cursor - output);
}
