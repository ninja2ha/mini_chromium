...
std::string HttpByteRange::GetHeaderValue() const {
  ...
  if (IsSuffixByteRange())
    return cr::StringPrintf("bytes=-%" PRId64, suffix_length());

  CR_DCHECK(HasFirstBytePosition());

  if (!HasLastBytePosition())
    return cr::StringPrintf("bytes=%" PRId64 "-", first_byte_position());

  return cr::StringPrintf("bytes=%" PRId64 "-%" PRId64,
