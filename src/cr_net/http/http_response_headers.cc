...
#include "cr_base/strings/format_macros.h"
...
#include "cr_net/http/http_byte_range.h"
///#include "cr_net/http/http_log_util.h"
#include "cr_net/http/http_util.h"
///#include "cr_net/log/net_log_capture_mode.h"
///#include "cr_net/log/net_log_values.h"
...
bool ShouldUpdateHeader(cr::StringPiece name) {
  for (size_t i = 0; i < cr::size(kNonUpdatedHeaders); ++i) {
    if (cr::LowerCaseEqualsASCII(name, kNonUpdatedHeaders[i]))
      return false;
  }
  for (size_t i = 0; i < cr::size(kNonUpdatedHeaderPrefixes); ++i) {
    if (cr::StartsWith(name, kNonUpdatedHeaderPrefixes[i],
...
bool HasEmbeddedNulls(cr::StringPiece str) {
  for (char c : str) {
    if (c == '\0')
      return true;
  }
...
void CheckDoesNotHaveEmbeddedNulls(cr::StringPiece str) {
...
cr::RefPtr<HttpResponseHeaders> HttpResponseHeaders::TryToCreate(
    cr::StringPiece headers) {
...
void HttpResponseHeaders::Update(const HttpResponseHeaders& new_headers) {
  ...
  for (size_t i = 0; i < new_headers.parsed_.size(); ++i) {
    ...
    CR_DCHECK(!new_parsed[i].is_continuation());
    ...
    auto name =
        cr::MakeStringPiece(new_parsed[i].name_begin, new_parsed[i].name_end);
    if (ShouldUpdateHeader(name)) {
      std::string name_lower = cr::ToLowerASCII(name);
...
void HttpResponseHeaders::MergeWithHeaders(const std::string& raw_headers,
  ...
  for (size_t i = 0; i < parsed_.size(); ++i) {
    CR_DCHECK(!parsed_[i].is_continuation());
    ...
    std::string name = cr::ToLowerASCII(
        cr::MakeStringPiece(parsed_[i].name_begin, parsed_[i].name_end));
...
void HttpResponseHeaders::RemoveHeader(cr::StringPiece name) {
  // Copy up to the null byte.  This just copies the status line.
  std::string new_raw_headers(raw_headers_.c_str());
  new_raw_headers.push_back('\0');

  HeaderSet to_remove;
  to_remove.insert(cr::ToLowerASCII(name));
...
void HttpResponseHeaders::RemoveHeaders(
  ...
  for (const auto& header_name : header_names) {
    to_remove.insert(cr::ToLowerASCII(header_name));
  }
  MergeWithHeaders(new_raw_headers, to_remove);
}
...
void HttpResponseHeaders::RemoveHeaderLine(const std::string& name,
  ...
  std::string name_lowercase = cr::ToLowerASCII(name);
  ...
  while (EnumerateHeaderLines(&iter, &old_header_name, &old_header_value)) {
    std::string old_header_name_lowercase = cr::ToLowerASCII(old_header_name);
...
void HttpResponseHeaders::AddHeader(cr::StringPiece name,
                                    cr::StringPiece value) {
  CR_DCHECK(HttpUtil::IsValidHeaderName(name));
  CR_DCHECK(HttpUtil::IsValidHeaderValue(value));
...
void HttpResponseHeaders::SetHeader(cr::StringPiece name,
                                    cr::StringPiece value) {
  RemoveHeader(name);
...
void HttpResponseHeaders::UpdateWithNewRange(const HttpByteRange& byte_range,
  ...
  CR_DCHECK(byte_range.IsValid());
  CR_DCHECK(byte_range.HasFirstBytePosition());
  CR_DCHECK(byte_range.HasLastBytePosition());
  ...
  AddHeader(kRangeHeader,
            cr::StringPrintf("bytes %" PRId64 "-%" PRId64 "/%" PRId64, start,
                             end, resource_size));
  AddHeader(kLengthHeader, cr::StringPrintf("%" PRId64, range_len));
}
...
void HttpResponseHeaders::Parse(const std::string& raw_input) {
  ...
  if (line_end == raw_input.end()) {
    ...
    CR_DCHECK('\0' == raw_headers_[raw_headers_.size() - 2]);
    CR_DCHECK('\0' == raw_headers_[raw_headers_.size() - 1]);
    return;
  }
...
bool HttpResponseHeaders::GetNormalizedHeader(cr::StringPiece name,
                                              std::string* value) const {
  // If you hit this assertion, please use EnumerateHeader instead!
  CR_DCHECK(!HttpUtil::IsNonCoalescingHeader(name));
...
bool HttpResponseHeaders::HasHeaderValue(cr::StringPiece name,
                                         cr::StringPiece value) const {
  ...
  while (EnumerateHeader(&iter, name, &temp)) {
    if (cr::EqualsCaseInsensitiveASCII(value, temp))
      return true;
  }
  return false;
}

bool HttpResponseHeaders::HasHeader(cr::StringPiece name) const {
...
HttpVersion HttpResponseHeaders::ParseVersion(
  ...
  if (!cr::StartsWith(cr::MakeStringPiece(line_begin, line_end), "http",
                      cr::CompareCase::INSENSITIVE_ASCII)) {
    CR_DVLOG(1) << "missing status line";
    return HttpVersion();
  }
  ...
  if (p >= line_end || *p != '/') {
    CR_DVLOG(1) << "missing version";
    return HttpVersion();
  }

  std::string::const_iterator dot = std::find(p, line_end, '.');
  if (dot == line_end) {
    CR_DVLOG(1) << "malformed version";
    return HttpVersion();
  }
  ...
  if (!(cr::IsAsciiDigit(*p) && cr::IsAsciiDigit(*dot))) {
    CR_DVLOG(1) << "malformed version number";
    return HttpVersion();
  }
...
void HttpResponseHeaders::ParseStatusLine(
  ...
  if (parsed_http_version != http_version_) {
    CR_DVLOG(1) << "assuming HTTP/" << http_version_.major_value() << "."
  ...
  if (p == line_end) {
    CR_DVLOG(1) << "missing response status; assuming 200 OK";
  ...
  while (p < line_end && cr::IsAsciiDigit(*p))
    ++p;

  if (p == code) {
    CR_DVLOG(1) << "missing response status number; assuming 200";
  ...
  cr::StringToInt(cr::MakeStringPiece(code, p), &response_code_);
...
size_t HttpResponseHeaders::FindHeader(size_t from,
                                       cr::StringPiece search) const {
  for (size_t i = from; i < parsed_.size(); ++i) {
    if (parsed_[i].is_continuation())
      continue;
    auto name =
        cr::MakeStringPiece(parsed_[i].name_begin, parsed_[i].name_end);
    if (cr::EqualsCaseInsensitiveASCII(search, name))
      return i;
  }
...
bool HttpResponseHeaders::GetCacheControlDirective(cr::StringPiece directive,
                                                   TimeDelta* result) const {
  cr::StringPiece name("cache-control");
  std::string value;
  ...
  while (EnumerateHeader(&iter, name, &value)) {
    if (value.size() > directive_size + 1 &&
        cr::StartsWith(value, directive,
                       cr::CompareCase::INSENSITIVE_ASCII) &&
        value[directive_size] == '=') {
      int64_t seconds;
      cr::StringToInt64(cr::MakeStringPiece(
...
void HttpResponseHeaders::AddHeader(std::string::const_iterator name_begin,
  ...
  if (values_begin == values_end ||
      HttpUtil::IsNonCoalescingHeader(
          cr::MakeStringPiece(name_begin, name_end))) {
...
void HttpResponseHeaders::AddNonCacheableHeaders(HeaderSet* result) const {
  ...
  while (EnumerateHeader(&iter, kCacheControl, &value)) {
    ...
    while (item != end) {
      ...
      if (item_end > item) {
        result->insert(
            cr::ToLowerASCII(cr::StringPiece(&*item, item_end - item)));
      }
...
void HttpResponseHeaders::AddSecurityStateHeaders(HeaderSet* result) {
  for (size_t i = 0; i < cr::size(kSecurityStateHeaders); ++i)
...
bool HttpResponseHeaders::IsRedirect(std::string* location) const {
  ...
  if (location) {
    auto location_strpiece =
        cr::MakeStringPiece(parsed_[i].value_begin, parsed_[i].value_end);
...
HttpResponseHeaders::GetFreshnessLifetimes(const Time& response_time) const {
  ...
  if (must_revalidate || !GetStaleWhileRevalidateValue(&lifetimes.staleness)) {
    CR_DCHECK(TimeDelta() == lifetimes.staleness);
  }
  ...
  if (GetExpiresValue(&expires_value)) {
    ...
    CR_DCHECK(TimeDelta() == lifetimes.freshness);
    return lifetimes;
  }
  ...
  for (const char* header : kConnectionHeaders) {
    ...
    while (EnumerateHeader(&iterator, header, &token)) {
      for (const KeepAliveToken& keep_alive_token : kKeepAliveTokens) {
        if (cr::LowerCaseEqualsASCII(token, keep_alive_token.token))
...
int64_t HttpResponseHeaders::GetInt64HeaderValue(
  ...
  int64_t result;
  bool ok = cr::StringToInt64(content_length_val, &result);
  if (!ok || result < 0)
    return -1;
...
bool HttpResponseHeaders::IsCookieResponseHeader(cr::StringPiece name) {
  for (const char* cookie_header : kCookieResponseHeaders) {
    if (cr::EqualsCaseInsensitiveASCII(cookie_header, name))
      return true;
  }
  return false;
}
