...
int HttpChunkedDecoder::ScanForChunkRemaining(const char* buf, int buf_len) {
  CR_DCHECK(0 == chunk_remaining_);
  CR_DCHECK(buf_len > 0);

  int bytes_consumed = 0;

  size_t index_of_lf = cr::StringPiece(buf, buf_len).find('\n');
  if (index_of_lf != cr::StringPiece::npos) {
    ...
    if (reached_last_chunk_) {
      if (buf_len > 0)
        CR_DVLOG(1) << "ignoring http trailer";
      else
        reached_eof_ = true;
    } else if (chunk_terminator_remaining_) {
      if (buf_len > 0) {
        CR_DLOG(Error) << "chunk data not terminated properly";
        return ERR_INVALID_CHUNKED_ENCODING;
      }
      chunk_terminator_remaining_ = false;
    } else if (buf_len > 0) {
      // Ignore any chunk-extensions.
      size_t index_of_semicolon = cr::StringPiece(buf, buf_len).find(';');
      if (index_of_semicolon != cr::StringPiece::npos)
        buf_len = static_cast<int>(index_of_semicolon);

      if (!ParseChunkSize(buf, buf_len, &chunk_remaining_)) {
        CR_DLOG(Error) << "Failed parsing HEX from: " <<
      ...
      CR_DLOG(Error) << "missing chunk-size";
      return ERR_INVALID_CHUNKED_ENCODING;
    }
    line_buf_.clear();
  } else {
    ...
    if (line_buf_.length() + buf_len > kMaxLineBufLen) {
      CR_DLOG(Error) << "Chunked line length too long";
      return ERR_INVALID_CHUNKED_ENCODING;
    }
...
bool HttpChunkedDecoder::ParseChunkSize(const char* start,
  ...
  cr::StringPiece chunk_size(start, len);
  ...
  bool ok = cr::HexStringToInt64(chunk_size, &parsed_number);
