...
#ifndef MINI_CHROMIUM_SRC_CRNET_HTTP_HTTP_RESPONSE_HEADERS_H_
#define MINI_CHROMIUM_SRC_CRNET_HTTP_HTTP_RESPONSE_HEADERS_H_
...
#include "cr_net/http/http_version.h"
...
class CRNET_EXPORT HttpResponseHeaders
    : public cr::RefCountedThreadSafe<HttpResponseHeaders> {
 public:
  // Persist options.
  ...
  static cr::RefPtr<HttpResponseHeaders> TryToCreate(
  ...
  void RemoveHeader(cr::StringPiece name);
  ...
  void AddHeader(cr::StringPiece name, cr::StringPiece value);
  ...
  // be SetHeader().
  void SetHeader(cr::StringPiece name, cr::StringPiece value);
  ...
  bool GetNormalizedHeader(cr::StringPiece name, std::string* value) const;
  ...
  bool HasHeaderValue(cr::StringPiece name, cr::StringPiece value) const;

  // Returns true if the response contains the specified header.
  // The name is compared case insensitively.
  bool HasHeader(cr::StringPiece name) const;
  ...
  ValidationType RequiresValidation(const cr::Time& request_time,
                                    const cr::Time& response_time,
                                    const cr::Time& current_time) const;
  ...
  FreshnessLifetimes GetFreshnessLifetimes(
      const cr::Time& response_time) const;
  ...
  cr::TimeDelta GetCurrentAge(const cr::Time& request_time,
                              const cr::Time& response_time,
                              const cr::Time& current_time) const;
  ...
  bool GetMaxAgeValue(cr::TimeDelta* value) const;
  bool GetAgeValue(cr::TimeDelta* value) const;
  bool GetDateValue(cr::Time* value) const;
  bool GetLastModifiedValue(cr::Time* value) const;
  bool GetExpiresValue(cr::Time* value) const;
  bool GetStaleWhileRevalidateValue(cr::TimeDelta* value) const;
  ...
  bool GetTimeValuedHeader(const std::string& name, cr::Time* result) const;
  ...
  static bool IsCookieResponseHeader(cr::StringPiece name);

 private:
  friend class cr::RefCountedThreadSafe<HttpResponseHeaders>;
  ...
  size_t FindHeader(size_t from, cr::StringPiece name) const;
  ...
  // and return true.
  bool GetCacheControlDirective(cr::StringPiece directive,
                                cr::TimeDelta* result) const;
...
using ResponseHeadersCallback =
    cr::RepeatingCallback<void(cr::RefPtr<const HttpResponseHeaders>)>;

}  // namespace crnet

#endif  // MINI_CHROMIUM_SRC_CRNET_HTTP_HTTP_RESPONSE_HEADERS_H_