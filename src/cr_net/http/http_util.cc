...
class AcceptLanguageBuilder {
 public:
  ...
  void AddLanguageCode(const std::string& language) {
    ...
    CR_DCHECK(std::string::npos == language.find_first_of("; "));
    CR_DCHECK(cr::IsStringASCII(language));
    if (seen_.find(language) == seen_.end()) {
      if (str_.empty()) {
        cr::StringAppendF(&str_, "%s", language.c_str());
      } else {
        cr::StringAppendF(&str_, ",%s", language.c_str());
      }
...
std::string GetBaseLanguageCode(const std::string& language_code) {
  const std::vector<std::string> tokens = cr::SplitString(
      language_code, "-", cr::TRIM_WHITESPACE, cr::SPLIT_WANT_ALL);
...
// static
std::string HttpUtil::SpecForRequest(const crurl::GURL& url) {
...
void HttpUtil::ParseContentType(const std::string& content_type_str,
  ...
  while (offset < content_type_str.size()) {
    CR_DCHECK(';' == content_type_str[offset]);
    ...
    auto param_name =
        cr::MakeStringPiece(content_type_str.begin() + param_name_start,
    ...
    if (!type_has_charset &&
        cr::LowerCaseEqualsASCII(param_name, "charset")) {
    ...
    if (boundary && !type_has_boundary &&
        cr::LowerCaseEqualsASCII(param_name, "boundary")) {
  ...
  bool eq = !mime_type->empty() &&
            cr::LowerCaseEqualsASCII(
                cr::MakeStringPiece(begin + type_val, begin + type_end),
                mime_type->data());
  if (!eq) {
    *mime_type = cr::ToLowerASCII(
        cr::MakeStringPiece(begin + type_val, begin + type_end));
  }
  if ((!eq && *had_charset) || type_has_charset) {
    *had_charset = true;
    *charset = cr::ToLowerASCII(charset_value);
  }
}
...
bool HttpUtil::ParseRangeHeader(const std::string& ranges_specifier,
  ...
  cr::StringPiece bytes_unit =
      cr::StringPiece(ranges_specifier).substr(0, equal_char_offset);

  // "bytes" unit identifier is not found.
  bytes_unit = TrimLWS(bytes_unit);
  if (!cr::LowerCaseEqualsASCII(bytes_unit, "bytes")) {
    return false;
  }
  ...
  while (byte_range_set_iterator.GetNext()) {
    cr::StringPiece value = byte_range_set_iterator.value_piece();
    size_t minus_char_offset = value.find('-');
    // If '-' character is not found, reports failure.
    if (minus_char_offset == std::string::npos)
      return false;

    cr::StringPiece first_byte_pos = value.substr(0, minus_char_offset);
    ...
    if (!first_byte_pos.empty()) {
      ...
      if (!cr::StringToInt64(first_byte_pos, &first_byte_position))
        return false;
      range.set_first_byte_position(first_byte_position);
    }

    cr::StringPiece last_byte_pos = value.substr(minus_char_offset + 1);
    ...
    if (!last_byte_pos.empty()) {
      ...
      if (!cr::StringToInt64(last_byte_pos, &last_byte_position))
        return false;
...
bool HttpUtil::ParseContentRangeHeaderFor206(
    cr::StringPiece content_range_spec,
  ...
  *first_byte_position = *last_byte_position = *instance_length = -1;
  ...
  if (space_position == cr::StringPiece::npos)
    return false;
  ...
  if (minus_position == cr::StringPiece::npos)
    return false;
  size_t slash_position = content_range_spec.find('/', minus_position + 1);
  if (slash_position == cr::StringPiece::npos)
    return false;
...
bool HttpUtil::ParseRetryAfterHeader(const std::string& retry_after_string,
  ...
  if (interval < cr::TimeDelta::FromSeconds(0))
    return false;
...
// static
bool HttpUtil::IsMethodSafe(cr::StringPiece method) {
  return method == "GET" || method == "HEAD" || method == "OPTIONS" ||
         method == "TRACE";
}

// static
bool HttpUtil::IsMethodIdempotent(cr::StringPiece method) {
  return IsMethodSafe(method) || method == "PUT" || method == "DELETE";
}

// static
bool HttpUtil::IsSafeHeader(cr::StringPiece name) {
  if (cr::StartsWith(name, "proxy-", cr::CompareCase::INSENSITIVE_ASCII) ||
      cr::StartsWith(name, "sec-", cr::CompareCase::INSENSITIVE_ASCII))
    return false;

  for (const char* field : kForbiddenHeaderFields) {
    if (cr::LowerCaseEqualsASCII(name, field))
      return false;
  }
  return true;
}
...
// static
bool HttpUtil::IsValidHeaderName(cr::StringPiece name) {
  // Check whether the header name is RFC 2616-compliant.
  return HttpUtil::IsToken(name);
}

// static
bool HttpUtil::IsValidHeaderValue(cr::StringPiece value) {
...
// static
bool HttpUtil::IsNonCoalescingHeader(cr::StringPiece name) {
  ...
  for (const char* header : kNonCoalescingHeaders) {
    if (cr::LowerCaseEqualsASCII(name, header)) {
      return true;
    }
  }
  return false;
...
bool HttpUtil::IsLWS(char c) {
  const cr::StringPiece kWhiteSpaceCharacters(HTTP_LWS);
  return kWhiteSpaceCharacters.find(c) != cr::StringPiece::npos;
}
...
// static
cr::StringPiece HttpUtil::TrimLWS(const cr::StringPiece& string) {
  const char* begin = string.data();
  const char* end = string.data() + string.size();
  TrimLWSImplementation(&begin, &end);
  return cr::StringPiece(begin, end - begin);
}
...
bool HttpUtil::IsToken(cr::StringPiece string) {
  if (string.empty())
    return false;
  for (char c : string) {
    if (!IsTokenChar(c))
...
bool HttpUtil::IsParmName(cr::StringPiece str) {
  if (str.empty())
    return false;
  for (char c : str) {
...
bool UnquoteImpl(cr::StringPiece str, bool strict_quotes, std::string* out) {
  if (str.empty())
    return false;
...
// static
std::string HttpUtil::Unquote(cr::StringPiece str) {
  std::string result;
...
// static
bool HttpUtil::StrictUnquote(cr::StringPiece str, std::string* out) {
  return UnquoteImpl(str, true, out);
}

// static
std::string HttpUtil::Quote(cr::StringPiece str) {
  std::string escaped;
...
size_t HttpUtil::LocateStartOfStatusLine(const char* buf, size_t buf_len) {
  ...
  if (buf_len >= http_len) {
    ...
    for (size_t i = 0; i <= i_max; ++i) {
      if (cr::LowerCaseEqualsASCII(cr::StringPiece(buf + i, http_len),
                                   "http"))
        return i;
    }
  }
...
static bool IsLineSegmentContinuable(cr::StringPiece line) {
  if (line.empty())
    return false;

  size_t colon = line.find(':');
  if (colon == cr::StringPiece::npos)
    return false;

  cr::StringPiece name = line.substr(0, colon);
...
static size_t FindStatusLineEnd(cr::StringPiece str) {
  size_t i = str.find_first_of("\r\n");
  if (i == cr::StringPiece::npos)
    return str.size();
  return i;
}

// Helper used by AssembleRawHeaders, to skip past leading LWS.
static cr::StringPiece RemoveLeadingNonLWS(cr::StringPiece str) {
  for (size_t i = 0; i < str.size(); i++) {
    if (!HttpUtil::IsLWS(str[i]))
      return str.substr(i);
  }
  return cr::StringPiece();  // Remove everything.
}

std::string HttpUtil::AssembleRawHeaders(cr::StringPiece input) {
  ...
  cr::CStringTokenizer lines(input.data(), input.data() + input.size(),
                               "\r\n");
...
std::string HttpUtil::ConvertHeadersBackToHTTPResponse(const std::string& str) {
  ...
  cr::StringTokenizer tokenizer(str, std::string(1, '\0'));
  while (tokenizer.GetNext()) {
    cr::StrAppend(&disassembled_headers, {tokenizer.token_piece(), "\r\n"});
  }
...
std::string HttpUtil::ExpandLanguageList(const std::string& language_prefs) {
  const std::vector<std::string> languages = cr::SplitString(
      language_prefs, ",", cr::TRIM_WHITESPACE, cr::SPLIT_WANT_ALL);
...
std::string HttpUtil::GenerateAcceptLanguageHeader(
  ...
  cr::StringTokenizer t(raw_language_list, ",");
  ...
  while (t.GetNext()) {
    ...
    if (qvalue10 == 10) {
      ...
      CR_DCHECK(qvalue10 < 10U);
      cr::StringAppendF(&lang_list_with_q, ",%s;q=0.%d", language.c_str(),
                          qvalue10);
    }
...
bool HttpUtil::HasStrongValidators(HttpVersion version,
  ...
  if (!etag_header.empty()) {
    ...
    TrimLWS(&i, &j);
    if (!cr::LowerCaseEqualsASCII(cr::MakeStringPiece(i, j), "w"))
      return true;
  }

  cr::Time last_modified;
  if (!cr::Time::FromString(last_modified_header.c_str(), &last_modified))
    return false;

  cr::Time date;
  if (!cr::Time::FromString(date_header.c_str(), &date))
    return false;
...
bool HttpUtil::HeadersIterator::GetNext() {
  while (lines_.GetNext()) {
    ...
    CR_DCHECK(name_begin_ < name_end_);
    if (!IsToken(cr::MakeStringPiece(name_begin_, name_end_)))
      continue;  // skip malformed header
...
bool HttpUtil::HeadersIterator::AdvanceTo(const char* name) {
  CR_DCHECK(name != nullptr);
  CR_DCHECK(0 == cr::ToLowerASCII(name).compare(name))
      << "the header name must be in all lower case";

  while (GetNext()) {
    if (cr::LowerCaseEqualsASCII(
            cr::MakeStringPiece(name_begin_, name_end_), name)) {
      return true;
    }
  }
...
HttpUtil::ValuesIterator::ValuesIterator(
  ...
  if (!ignore_empty_values_)
    values_.set_options(cr::StringTokenizer::RETURN_EMPTY_TOKENS);
}
  ...
  if (value_begin_ != value_end_ && IsQuote(*value_begin_)) {
    ...
    if (strict_quotes_) {
      if (!HttpUtil::StrictUnquote(
              cr::MakeStringPiece(value_begin_, value_end_),
              &unquoted_value_))
        return valid_ = false;
      return true;
    }
    ...
    if (*value_begin_ != *(value_end_ - 1) || value_begin_ + 1 == value_end_) {
      ...
      unquoted_value_ =
          HttpUtil::Unquote(cr::MakeStringPiece(value_begin_, value_end_));
    }
  }
...
bool HttpUtil::ParseAcceptEncoding(const std::string& accept_encoding,
  ...
  CR_DCHECK(allowed_encodings);
  if (accept_encoding.find_first_of("\"") != std::string::npos)
    return false;
  allowed_encodings->clear();

  cr::StringTokenizer tokenizer(accept_encoding.begin(),
                                  accept_encoding.end(), ",");
  while (tokenizer.GetNext()) {
    cr::StringPiece entry = tokenizer.token_piece();
    entry = TrimLWS(entry);
    size_t semicolon_pos = entry.find(';');
    if (semicolon_pos == cr::StringPiece::npos) {
      if (entry.find_first_of(HTTP_LWS) != cr::StringPiece::npos)
        return false;
      allowed_encodings->insert(cr::ToLowerASCII(entry));
      continue;
    }
    cr::StringPiece encoding = entry.substr(0, semicolon_pos);
    encoding = TrimLWS(encoding);
    if (encoding.find_first_of(HTTP_LWS) != cr::StringPiece::npos)
      return false;
    cr::StringPiece params = entry.substr(semicolon_pos + 1);
    params = TrimLWS(params);
    size_t equals_pos = params.find('=');
    if (equals_pos == cr::StringPiece::npos)
      return false;
    cr::StringPiece param_name = params.substr(0, equals_pos);
    param_name = TrimLWS(param_name);
    if (!cr::LowerCaseEqualsASCII(param_name, "q"))
      return false;
    cr::StringPiece qvalue = params.substr(equals_pos + 1);
    qvalue = TrimLWS(qvalue);
    if (qvalue.empty())
      return false;
    if (qvalue[0] == '1') {
      if (cr::StartsWith("1.000", qvalue)) {
        allowed_encodings->insert(cr::ToLowerASCII(encoding));
        continue;
      }
      return false;
    }
...
bool HttpUtil::ParseContentEncoding(const std::string& content_encoding,
  ...
  CR_DCHECK(used_encodings);
  if (content_encoding.find_first_of("\"=;*") != std::string::npos)
    return false;
  used_encodings->clear();

  cr::StringTokenizer encoding_tokenizer(content_encoding.begin(),
                                           content_encoding.end(), ",");
  while (encoding_tokenizer.GetNext()) {
    cr::StringPiece encoding = TrimLWS(encoding_tokenizer.token_piece());
    if (encoding.find_first_of(HTTP_LWS) != cr::StringPiece::npos)
      return false;
    used_encodings->insert(cr::ToLowerASCII(encoding));
  }
  return true;
}
