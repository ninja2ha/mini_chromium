...
bool StringToNumber(const cr::StringPiece& input, int32_t* output) {
  // This assumes ints are 32-bits (will fail compile if that ever changes).
  return cr::StringToInt(input, output);
}

bool StringToNumber(const cr::StringPiece& input, uint32_t* output) {
  // This assumes ints are 32-bits (will fail compile if that ever changes).
  return cr::StringToUint(input, output);
}

bool StringToNumber(const cr::StringPiece& input, int64_t* output) {
  return cr::StringToInt64(input, output);
}

bool StringToNumber(const cr::StringPiece& input, uint64_t* output) {
  return cr::StringToUint64(input, output);
}
...
template <typename T>
bool ParseIntHelper(const cr::StringPiece& input,
  ...
  bool starts_with_digit = cr::IsAsciiDigit(input[0]);
  ...
  // Dispatch to the appropriate flavor of cr::StringToXXX() by calling one of
  ...
  //
  // Note that the output set by cr::StringToXXX() on failure cannot be used
  // as it has ambiguity with parse errors.

  // Strip any leading negative sign off the number.
  cr::StringPiece numeric_portion =
...
bool ParseInt32(const cr::StringPiece& input,
                ParseIntFormat format,
                int32_t* output,
                ParseIntError* optional_error) {
...
bool ParseInt64(const cr::StringPiece& input,
                ParseIntFormat format,
                int64_t* output,
                ParseIntError* optional_error) {
  return ParseIntHelper(input, format, output, optional_error);
...
bool ParseUint32(const cr::StringPiece& input,
                 uint32_t* output,
                 ParseIntError* optional_error) {
...
bool ParseUint64(const cr::StringPiece& input,
                 uint64_t* output,
                 ParseIntError* optional_error) {
  return ParseIntHelper(input, ParseIntFormat::NON_NEGATIVE, output,
                        optional_error);
