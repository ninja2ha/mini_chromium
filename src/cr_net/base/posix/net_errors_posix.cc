...
#include "cr_base/logging/posix/logging_internal_posix.h"
#include "cr_build/build_config.h"

namespace crnet {

Error MapSystemError(cr::logging::SystemErrorCode os_error) {
  if (os_error != 0)
    CR_DVLOG(2) << "Error " << os_error << ": "
                << cr::logging::SystemErrorCodeToString(os_error);
  ...
  switch (os_error) {
    ...
    case 0:
      return OK;
    default:
      CR_LOG(Warning) << "Unknown error " << cr::logging::internal::safe_strerror(os_error) << " ("
