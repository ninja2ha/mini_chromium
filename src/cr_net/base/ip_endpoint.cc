...
///#if defined(MINI_CHROMIUM_OS_WIN)
///#include "cr_net/base/winsock_util.h"
///#endif
...
constexpr size_t kBluetoothAddressSize = 6;
#endif
...
bool GetIPAddressFromSockAddr(const struct sockaddr* sock_addr,
  ...
  if (sock_addr->sa_family == AF_INET) {
    ...
    *address_len = IPAddress::kIPv4AddressSize;
    if (port)
      *port = cr::NetToHost16(addr->sin_port);
    return true;
  }
  ...
  if (sock_addr->sa_family == AF_INET6) {
    ...
    *address_len = IPAddress::kIPv6AddressSize;
    if (port)
      *port = cr::NetToHost16(addr->sin6_port);
    return true;
  }
  ...
  switch (address_.size()) {
    case IPAddress::kIPv4AddressSize: {
      ...
      *address_length = kSockaddrInSize;
      ...
      addr->sin_port = cr::HostToNet16(port_);
    ...
    case IPAddress::kIPv6AddressSize: {
      ...
      *address_length = kSockaddrIn6Size;
      ...
      addr6->sin6_port = cr::HostToNet16(port_);
...
bool IPEndPoint::FromSockAddr(const struct sockaddr* sock_addr,
  ...
  address_ = crnet::IPAddress(address, address_len);
  port_ = port;
  return true;
}
