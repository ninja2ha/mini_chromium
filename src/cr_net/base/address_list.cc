...
#include "cr_net/base/sys_addrinfo.h"
...
void AddressList::AppendDnsAliases(std::vector<std::string> aliases) {
  CR_DCHECK(aliases != std::vector<std::string>({""}));
...
cr::Value AddressList::ToValue() const {
  cr::Value dict(cr::Value::Type::DICTIONARY);
  cr::Value list(cr::Value::Type::LIST);
...
void AddressList::Deduplicate() {
  if (size() > 1) {
    ...
    cr::FlatMap<IPEndPoint, int> inserted(std::move(make_me_into_a_map));
