...
///#include "cr_net/log/net_log_source_type.h"
///#include "cr_net/log/net_log_values.h"
#include "cr_net/socket/socket_descriptor.h"
///#include "cr_net/socket/socket_net_log_params.h"
...
int TCPSocketWin::Open(AddressFamily family) {
  ...
  if (!SetNonBlockingAndGetError(static_cast<int>(socket_), &os_error)) {
...
int TCPSocketWin::AcceptInternal(std::unique_ptr<TCPSocketWin>* socket,
                                 IPEndPoint* address) {
  SockaddrStorage storage;
  int new_socket = static_cast<int>(
