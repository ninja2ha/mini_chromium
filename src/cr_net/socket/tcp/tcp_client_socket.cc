...
///#include "cr_net/base/features.h"
#include "cr_event/io_buffer.h"
#include "cr_net/base/ip_endpoint.h"
#include "cr_net/base/net_errors.h"
///#include "cr_net/nqe/network_quality_estimator.h"
#include "cr_net/socket/socket_performance_watcher.h"
...
int TCPClientSocket::ReadCommon(cr::IOBuffer* buf,
  ...
  // is gone/closed. Therefore, it is safe to use cr::Unretained() here.
...
int TCPClientSocket::DoConnect() {
  ...
  start_connect_attempt_ = cr::TimeTicks::Now();

  // Start a timer to fail the connect attempt if it takes too long.
  cr::TimeDelta attempt_timeout = GetConnectAttemptTimeout();
  if (!attempt_timeout.is_max()) {
    CR_DCHECK(!connect_attempt_timer_.IsRunning());
    connect_attempt_timer_.Start(
        CR_FROM_HERE, attempt_timeout,
        cr::BindOnce(&TCPClientSocket::OnConnectAttemptTimeout,
                     cr::Unretained(this)));
  }
...
int TCPClientSocket::DoConnectComplete(int result) {
  if (start_connect_attempt_) {
    ...
    start_connect_attempt_ = cr::nullopt;
...
void TCPClientSocket::EmitConnectAttemptHistograms(int result) {
  ...
  CR_DCHECK(start_connect_attempt_);

  cr::TimeDelta duration =
      cr::TimeTicks::Now() - start_connect_attempt_.value();
  ...
  cr::Optional<cr::TimeDelta> transport_rtt = cr::nullopt;
...
cr::TimeDelta TCPClientSocket::GetConnectAttemptTimeout() {
   return cr::TimeDelta::Max();
  ///if (!base::FeatureList::IsEnabled(features::kTimeoutTcpConnectAttempt))
  ///  return base::TimeDelta::Max();

  ///cr::Optional<base::TimeDelta> transport_rtt = base::nullopt;
  ///if (network_quality_estimator_)
  ///  transport_rtt = network_quality_estimator_->GetTransportRTT();
  ///
  ///cr::TimeDelta min_timeout = features::kTimeoutTcpConnectAttemptMin.Get();
  ///cr::TimeDelta max_timeout = features::kTimeoutTcpConnectAttemptMax.Get();
  ///
  ///if (!transport_rtt)
  ///  return max_timeout;
  ///
  ///cr::TimeDelta adaptive_timeout =
  ///    transport_rtt.value() *
