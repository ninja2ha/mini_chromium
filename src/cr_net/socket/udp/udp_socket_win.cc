...
///#include "cr_base/task/post_task.h"
///#include "cr_base/task/thread_pool.h"
...
#include "cr_net/socket/socket_options.h"
#include "cr_net/socket/socket_tag.h"
///#include "cr_net/socket/udp_net_log_parameters.h"
...
void UDPSocketWin::Core::WatchForRead() {
  ...
  AddRef();
  read_watcher_.StartWatchingOnce(
      read_overlapped_.hEvent, &reader_, CR_FROM_HERE);
}
...
void UDPSocketWin::Core::WatchForWrite() {
  ...
  AddRef();
  write_watcher_.StartWatchingOnce(
      write_overlapped_.hEvent, &writer_, CR_FROM_HERE);
}
...
QwaveApi* QwaveApi::GetDefault() {
  static cr::NoDestructor<QwaveApi> qwave;
  return qwave.get();
}
...
void UDPSocketWin::WatchForReadWrite() {
  if (read_write_watcher_.IsWatching())
    return;
  bool watched =
      read_write_watcher_.StartWatchingOnce(
          read_write_event_.Get(), this, CR_FROM_HERE);
  CR_DCHECK(watched);
}
...
void DscpManager::RequestHandle() {
  ...
  OnHandleCreated(api_, weak_ptr_factory_.GetWeakPtr(), DoCreateHandle(api_));
  ///cr::ThreadPool::PostTaskAndReplyWithResult(
