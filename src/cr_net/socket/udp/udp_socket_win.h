...
#include "cr_net/base/address_family.h"
...
///#include "cr_net/log/net_log_with_source.h"
#include "cr_net/socket/udp/datagram_socket.h"
#include "cr_net/socket/udp/diff_serv_code_point.h"
...
class CRNET_EXPORT UDPSocketWin : public cr::win::ObjectWatcher::Delegate {
 public:
 ...
 private:
  ...
  // set to NULL.
  int SendToOrWrite(cr::IOBuffer* buf,
                    int buf_len,
                    const IPEndPoint* address,
                    CompletionOnceCallback callback);
  ...
  int InternalRecvFromOverlapped(cr::IOBuffer* buf,
                                 int buf_len,
                                 IPEndPoint* address);
  int InternalSendToOverlapped(cr::IOBuffer* buf,
                               int buf_len,
                               const IPEndPoint* address);

  // Version for using non-blocking IO.
  int InternalRecvFromNonBlocking(cr::IOBuffer* buf,
                                  int buf_len,
                                  IPEndPoint* address);
  int InternalSendToNonBlocking(cr::IOBuffer* buf,
                                int buf_len,
                                const IPEndPoint* address);
