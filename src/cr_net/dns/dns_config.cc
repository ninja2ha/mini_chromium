...
cr::Value DnsConfig::ToValue() const {
  ...
  list = cr::Value(cr::Value::Type::LIST);
  ...
  dict.SetIntKey("num_hosts", static_cast<int>(hosts.size()));
  list = cr::Value(cr::Value::Type::LIST);
  for (auto& server : dns_over_https_servers) {
    cr::Value val(cr::Value::Type::DICTIONARY);
