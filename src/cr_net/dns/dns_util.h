...
#ifndef MINI_CHROMIUM_SRC_CRNET_DNS_DNS_UTIL_H_
#define MINI_CHROMIUM_SRC_CRNET_DNS_DNS_UTIL_H_
...
#include "cr_net/dns/public/dns_query_type.h"
...
CRNET_EXPORT bool DNSDomainFromDot(const cr::StringPiece& dotted,
                                   std::string* out);
...
CRNET_EXPORT bool DNSDomainFromUnrestrictedDot(const cr::StringPiece& dotted,
                                               std::string* out);

// Checks that a hostname is valid. Simple wrapper around DNSDomainFromDot.
CRNET_EXPORT bool IsValidDNSDomain(const cr::StringPiece& dotted);

// Checks that a hostname is valid. Simple wrapper around
// DNSDomainFromUnrestrictedDot.
CRNET_EXPORT bool IsValidUnrestrictedDNSDomain(
    const cr::StringPiece& dotted);
...
CRNET_EXPORT bool IsValidHostLabelCharacter(char c, bool is_first_char);
...
CRNET_EXPORT std::string CreateNamePointer(uint16_t offset);

// Convert a DnsQueryType enum to the wire format integer representation.
///CRNET_EXPORT uint16_t DnsQueryTypeToQtype(DnsQueryType dns_query_type);

///CRNET_EXPORT DnsQueryType
...
CRNET_EXPORT std::vector<DnsOverHttpsServerConfig>
...
CRNET_EXPORT std::string GetDohProviderIdForHistogramFromDohConfig(
    const DnsOverHttpsServerConfig& doh_server);

// Returns the provider id to use in UMA histogram names. If there is no
// provider id that matches |nameserver|, returns "Other".
CRNET_EXPORT std::string GetDohProviderIdForHistogramFromNameserver(
    const IPEndPoint& nameserver);

CRNET_EXPORT std::string SecureDnsModeToString(
...
class CRNET_EXPORT DomainNameComparator {
 public:
  bool operator()(cr::StringPiece lhs, cr::StringPiece rhs) const {
    return cr::CompareCaseInsensitiveASCII(lhs, rhs) < 0;
  }
};
...
#endif  // MINI_CHROMIUM_SRC_CRNET_DNS_DNS_UTIL_H_