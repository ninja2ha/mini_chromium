...
uint32_t ReduceTransactionId(const std::string& transaction_id) {
  CR_DCHECK(transaction_id.length() == crnet::kStunTransactionIdLength ||
            transaction_id.length() == crnet::kStunLegacyTransactionIdLength);
  cr::NetByteBufferReader reader(transaction_id.c_str(), 
...
void StunMessage::AddAttribute(std::unique_ptr<StunAttribute> attr) {
  ...
  if (!DesignatedExpertRange(attr->type())) {
    CR_DCHECK(attr->value_type() == GetAttributeValueType(attr->type()));
  }
...
bool StunMessage::ValidateMessageIntegrity(const char* data,
  ...
  uint16_t msg_length = cr::GetBE16(&data[2]);
  ...
  while (current_pos + 4 <= size) {
    ...
    attr_type = cr::GetBE16(&data[current_pos]);
    attr_length = cr::GetBE16(&data[current_pos + sizeof(attr_type)]);
  ...
  if (size > mi_pos + kStunAttributeHeaderSize + kStunMessageIntegritySize) {
    ...
    //     +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    cr::SetBE16(temp_data.get() + 2, static_cast<uint16_t>(new_adjusted_len));
  }

  cr::SHA1Digest sh1;
  cr::SHA1Hmac(password, cr::StringPiece(temp_data.get(), mi_pos), &sh1);
  std::string hmac = cr::SHA1DigestToBase16(sh1);
  if (hmac.length() != kStunMessageIntegritySize)
    return false;

  // Comparing the calculated HMAC with the one present in the message.
  return memcmp(data + current_pos + kStunAttributeHeaderSize, hmac.data(),
                hmac.length()) == 0;
}
...
bool StunMessage::AddMessageIntegrity(const char* key, size_t keylen) {
  ...
  cr::SHA1Digest sh1;
  cr::SHA1Hmac(cr::StringPiece(key, keylen), cr::StringPiece(buf.Data(), msg_len_for_hmac), &sh1);
  std::string hmac = cr::SHA1DigestToBase16(sh1);
  if (hmac.length() != kStunMessageIntegritySize) {
    CR_LOG(Error) << "HMAC computation failed. Message-Integrity "
                     "has dummy value.";
    return false;
  }

  // Insert correct HMAC into the attribute.
  msg_integrity_attr->CopyBytes(hmac.data(), hmac.length());
  return true;
}
...
bool StunMessage::ValidateFingerprint(const char* data, size_t size) {
  ...
  if (cr::GetBE32(magic_cookie) != kStunMagicCookie)
    return false;

  // Check the fingerprint type and length.
  const char* fingerprint_attr_data = data + size - fingerprint_attr_size;
  if (cr::GetBE16(fingerprint_attr_data) != STUN_ATTR_FINGERPRINT ||
      cr::GetBE16(fingerprint_attr_data + sizeof(uint16_t)) !=
...
bool StunMessage::Read(cr::NetByteBufferReader* buf) {
  ...
  if (cr::NetToHost32(magic_cookie_int) != kStunMagicCookie) {
...
bool StunMessage::Write(cr::NetByteBufferWriter* buf) const {
...
void StunAttribute::ConsumePadding(cr::NetByteBufferReader* buf) const {
...
void StunAttribute::WritePadding(cr::NetByteBufferWriter* buf) const {
  int remainder = length_ % 4;
  if (remainder > 0) {
    char zeroes[4] = {0};
...
bool StunAddressAttribute::Read(cr::NetByteBufferReader* buf) {
  uint8_t dummy;
  ...
  if (stun_family == STUN_ADDRESS_IPV4) {
    ...
    IPAddress ipaddr(v4addr.S_un.S_un_b.s_b1, 
                     v4addr.S_un.S_un_b.s_b2, 
                     v4addr.S_un.S_un_b.s_b3, 
                     v4addr.S_un.S_un_b.s_b4);
    SetEndPoint(IPEndPoint(ipaddr, port));
    ...
    IPAddress ipaddr(v6addr.u.Byte);
    SetEndPoint(IPEndPoint(ipaddr, port));
  } else {
    return false;
  }
...
bool StunAddressAttribute::Write(cr::NetByteBufferWriter* buf) const {
...
bool StunXorAddressAttribute::Read(cr::NetByteBufferReader* buf) {
  if (!StunAddressAttribute::Read(buf))
    return false;
  uint16_t xoredport = port() ^ (kStunMagicCookie >> 16);
  IPAddress xored_ip = GetXoredIP();
  SetEndPoint(IPEndPoint(xored_ip, xoredport));
  return true;
}

bool StunXorAddressAttribute::Write(cr::NetByteBufferWriter* buf) const {
  StunAddressFamily address_family = family();
  if (address_family == STUN_ADDRESS_UNDEF) {
...
bool StunUInt32Attribute::Read(cr::NetByteBufferReader* buf) {
  if (length() != SIZE || !buf->ReadUInt32(&bits_))
    return false;
  return true;
}

bool StunUInt32Attribute::Write(cr::NetByteBufferWriter* buf) const {
...
bool StunUInt64Attribute::Read(cr::NetByteBufferReader* buf) {
  if (length() != SIZE || !buf->ReadUInt64(&bits_))
    return false;
  return true;
}

bool StunUInt64Attribute::Write(cr::NetByteBufferWriter* buf) const {
...
bool StunByteStringAttribute::Read(cr::NetByteBufferReader* buf) {
...
bool StunByteStringAttribute::Write(cr::NetByteBufferWriter* buf) const {
...
bool StunErrorCodeAttribute::Read(cr::NetByteBufferReader* buf) {
  uint32_t val;
...
bool StunErrorCodeAttribute::Write(cr::NetByteBufferWriter* buf) const {
...
bool StunUInt16ListAttribute::Read(cr::NetByteBufferReader* buf) {
  if (length() % 2)
    return false;
...
bool StunUInt16ListAttribute::Write(cr::NetByteBufferWriter* buf) const {
...
std::unique_ptr<StunAttribute> CopyStunAttribute(
    ...
    cr::NetByteBufferWriter* tmp_buffer_ptr) {
  cr::NetByteBufferWriter tmpBuffer;
  ...
  cr::NetByteBufferReader reader(tmp_buffer_ptr->Data(), 
                                 tmp_buffer_ptr->Length());
  if (!copy->Read(&reader)) {
    return nullptr;
  }
