...
#include "crbase_runtime/message_loop/message_loop_task_runner.h"
...
TimeTicks CalculateDelayedRuntime(const Location& from_here, TimeDelta delay) {
  CR_DLOG_IF(Warning, delay > kTaskDelayWarningThreshold)
      << "Requesting super-long task delay period of " << delay.InSeconds()
      << " seconds from here: " << from_here.ToString();

  CR_DCHECK(delay >= TimeDelta()) << "delay should not be negative";
...
PendingTask MessageLoopTaskRunner::TakeTask() {
  ...
  // MessageLoop.
  CR_DCHECK(
      RunsTasksInCurrentSequence() || valid_thread_id_ == kInvalidThreadId);
...
void MessageLoopTaskRunner::InjectTask(OnceClosure task) {
  ...
  bool success = this->PostTask(CR_FROM_HERE, std::move(task));
  CR_DCHECK(success) << "Injected a task in a dead task runner.";
}
...
bool MessageLoopTaskRunner::AddToIncomingQueue(const Location& from_here,
                                               ...
                                               TimeDelta delay,
                                               Nestable nestable) {
  CR_DCHECK(task_source_observer_)
      << "SetObserver() must be called before posting tasks";

  // Use CR_CHECK instead of DCHECK to crash earlier. 
  // See http://crbug.com/711167 for details.
  CR_CHECK(task);

  PendingTask pending_task(from_here, std::move(task),
                           CalculateDelayedRuntime(from_here, delay), nestable);

  if (cr::subtle::NoBarrier_Load(&add_queue_time_to_tasks_)) {
    if (pending_task.delayed_run_time.is_null()) {
      pending_task.queue_time = cr::TimeTicks::Now();
    } else {
