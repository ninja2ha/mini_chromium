...
void MessageLoop::RemoveTaskObserver(TaskObserver* task_observer) {
  ...
  CR_DCHECK(it != task_observers_.end());
...
void MessageLoop::BindToCurrentThread() {
  ...
  CR_DCHECK(!MessageLoopCurrent::IsSet())
...
void MessageLoop::SetTaskRunner(RefPtr<SingleThreadTaskRunner> task_runner) {
...
void MessageLoop::Run(bool application_tasks_allowed, TimeDelta timeout) {
  CR_DCHECK_CALLED_ON_VALID_THREAD(bound_thread_checker_);
...
void MessageLoop::RunTask(PendingTask* pending_task) {
  CR_DCHECK(task_execution_allowed_);
