...
#ifndef MINI_CHROMIUM_SRC_CRBASE_RT_TASK_SEQUENCE_MANAGER_LAZILY_DEALLOCATED_DEQUE_H_
#define MINI_CHROMIUM_SRC_CRBASE_RT_TASK_SEQUENCE_MANAGER_LAZILY_DEALLOCATED_DEQUE_H_
...
class LazilyDeallocatedDeque {
 public:
  ...
  void SetCapacity(size_t new_capacity) {
    ...
    CR_DCHECK(head_.get() == tail_);
    head_ = std::move(new_ring);
    tail_ = head_.get();
  }

 private:
  CR_FRIEND_GTEST_ALL_PREFIXES(LazilyDeallocatedDequeTest, RingPushFront);
  CR_FRIEND_GTEST_ALL_PREFIXES(LazilyDeallocatedDequeTest, RingPushBack);
  CR_FRIEND_GTEST_ALL_PREFIXES(LazilyDeallocatedDequeTest, RingCanPush);
  CR_FRIEND_GTEST_ALL_PREFIXES(LazilyDeallocatedDequeTest, RingPushPopPushPop);
  ...
  struct Ring {
    explicit Ring(size_t capacity)
        : capacity_(capacity),
          ...
          next_(nullptr) {
      CR_DCHECK(capacity_ >= kMinimumRingSize);
    }
    ...
    void push_front(T&& t) {
      ...
      CR_DCHECK(CircularDecrement(front_index_) != back_index_);
    ...
    void push_back(T&& t) {
      ...
      CR_DCHECK(!empty());  // Mustn't appear to become empty.
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_RT_TASK_SEQUENCE_MANAGER_LAZILY_DEALLOCATED_DEQUE_H_
