...
void WorkQueueSets::AddQueue(WorkQueue* work_queue, size_t set_index) {
  CR_DCHECK(!work_queue->work_queue_sets());
  CR_DCHECK(set_index < work_queue_heaps_.size());
  CR_DCHECK(!work_queue->heap_handle().IsValid());
...
void WorkQueueSets::RemoveQueue(WorkQueue* work_queue) {
  CR_DCHECK(this == work_queue->work_queue_sets());
...
void WorkQueueSets::ChangeSetIndex(WorkQueue* work_queue, size_t set_index) {
  ...
  CR_DCHECK(old_set < work_queue_heaps_.size());
  CR_DCHECK(old_set != set_index);
  work_queue->AssignSetIndex(set_index);
  CR_DCHECK(has_enqueue_order == work_queue->heap_handle().IsValid());
  if (!has_enqueue_order)
    return;
...
void WorkQueueSets::OnQueuesFrontTaskChanged(WorkQueue* work_queue) {
  ...
  CR_DCHECK(set_index < work_queue_heaps_.size());
  CR_DCHECK(work_queue->heap_handle().IsValid());
  CR_DCHECK(!work_queue_heaps_[set_index].empty()) << " set_index = " << set_index;
...
void WorkQueueSets::OnPopMinQueueInSet(WorkQueue* work_queue) {
  ...
  CR_DCHECK(set_index < work_queue_heaps_.size());
  CR_DCHECK(!work_queue_heaps_[set_index].empty()) << " set_index = " << set_index;
  CR_DCHECK(work_queue_heaps_[set_index].Min().value == work_queue)
      << " set_index = " << set_index;
  CR_DCHECK(work_queue->heap_handle().IsValid());
  EnqueueOrder enqueue_order;
  if (work_queue->GetFrontTaskEnqueueOrder(&enqueue_order)) {
    ...
    CR_DCHECK(!work_queue->heap_handle().IsValid());
    CR_DCHECK(work_queue_heaps_[set_index].empty() ||
...
void WorkQueueSets::OnQueueBlocked(WorkQueue* work_queue) {
  CR_DCHECK(this == work_queue->work_queue_sets());
  cr::internal::HeapHandle heap_handle = work_queue->heap_handle();
...
WorkQueue* WorkQueueSets::GetOldestQueueInSet(size_t set_index) const {
  ...
  CR_DCHECK(set_index == queue->work_queue_set_index());
  CR_DCHECK(queue->heap_handle().IsValid());
  return queue;
}
...
WorkQueue* WorkQueueSets::GetOldestQueueAndEnqueueOrderInSet(
  ...
  CR_DCHECK(oldest.value->heap_handle().IsValid());
  *out_enqueue_order = oldest.key;
  EnqueueOrder enqueue_order;
  CR_DCHECK(oldest.value->GetFrontTaskEnqueueOrder(&enqueue_order) &&
...
WorkQueue* WorkQueueSets::GetRandomQueueAndEnqueueOrderInSet(
  ...
  *out_enqueue_order = chosen.key;
  EnqueueOrder enqueue_order;
  CR_DCHECK(chosen.value->GetFrontTaskEnqueueOrder(&enqueue_order) &&
...
void WorkQueueSets::CollectSkippedOverLowerPriorityTasks(
  ...
  CR_CHECK(
      selected_work_queue->GetFrontTaskEnqueueOrder(&selected_enqueue_order));
