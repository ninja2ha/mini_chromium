...
#include "crurl_tests/url_test_utils.h"
...
TEST(URLCanonTest, DoAppendUTF8) {
  ...
  std::string out_str;
  for (size_t i = 0; i < cr::size(utf_cases); i++) {
    out_str.clear();
  ...
  for (size_t i = 0; i < cr::size(utf_cases); i++) {
    ...
    if (utf_cases[i].input8 && utf_cases[i].input16 &&
      ...
      std::u16string input16_str(
          test_utils::TruncateWStringToUTF16(utf_cases[i].input16));
      EXPECT_EQ(input8_str, cr::UTF16ToUTF8(input16_str));

      // UTF-8 -> UTF-16
      EXPECT_EQ(input16_str, cr::UTF8ToUTF16(input8_str));
    }
  }
}
...
TEST(URLCanonTest, Scheme) {
  ...
  for (size_t i = 0; i < cr::size(scheme_cases); i++) {
    ...
    std::u16string wide_input(cr::UTF8ToUTF16(scheme_cases[i].input));
...
TEST(URLCanonTest, Host) {
  ...
  std::string out_str;
  for (size_t i = 0; i < cr::size(host_cases); i++) {
    // Narrow version.
...
TEST(URLCanonTest, IPv4) {
  ...
  for (size_t i = 0; i < cr::size(cases); i++) {
    // 8-bit version.
...
TEST(URLCanonTest, UserInfo) {
  ...
  for (size_t i = 0; i < cr::size(user_info_cases); i++) {
    ...
    out_str.clear();
    StdStringCanonOutput output2(&out_str);
    std::u16string wide_input(cr::UTF8ToUTF16(user_info_cases[i].input));
...
TEST(URLCanonTest, Port) {
  ...
  for (size_t i = 0; i < cr::size(port_cases); i++) {
    ...
    out_str.clear();
    StdStringCanonOutput output2(&out_str);
    std::u16string wide_input(cr::UTF8ToUTF16(port_cases[i].input));
...
TEST(URLCanonTest, Path) {
  DoPathTest(kCommonPathCases, cr::size(kCommonPathCases), CanonicalizePath,
             CanonicalizePath);
...
TEST(URLCanonTest, PartialPath) {
  ...
  DoPathTest(kCommonPathCases, cr::size(kCommonPathCases),
             CanonicalizePartialPath, CanonicalizePartialPath);
  DoPathTest(partial_path_cases, cr::size(partial_path_cases),
             CanonicalizePartialPath, CanonicalizePartialPath);
}
...
TEST(URLCanonTest, Query) {
  ...
  for (size_t i = 0; i < cr::size(query_cases); i++) {
    Component out_comp;
...
TEST(URLCanonTest, Ref) {
  ...
  for (size_t i = 0; i < cr::size(ref_cases); i++) {
    // 8-bit input
...
TEST(URLCanonTest, ReplaceStandardURL) {
  ...
  for (size_t i = 0; i < cr::size(replace_cases); i++) {
...
TEST(URLCanonTest, CanonicalizePathURL) {
  ...
  for (size_t i = 0; i < cr::size(path_cases); i++) {
...
TEST(URLCanonTest, CanonicalizePathURLPath) {
  ...
  for (size_t i = 0; i < cr::size(path_cases); i++) {
    ...
    crurl::Component out_component16;
    std::u16string input16(
...
TEST(URLCanonTest, ResolveRelativeURL) {
  ...
  for (size_t i = 0; i < cr::size(rel_cases); i++) {
...
///  EXPECT_EQ("hello", std::string(output.data()));
///  output.set_length(0);
///
///  // Mixed ASCII/non-ASCII.
///  str = u"hellö";
///  EXPECT_TRUE(IDNToASCII(str.data(), str.length(), &output));
///  EXPECT_EQ("xn--hell-8qa", std::string(output.data()));
///  output.set_length(0);
///
///  // All non-ASCII.
///  str = u"你好";
///  EXPECT_TRUE(IDNToASCII(str.data(), str.length(), &output));
///  EXPECT_EQ("xn--6qq79v", std::string(output.data()));
///  output.set_length(0);
///
...
///  str = u"¼";
///  EXPECT_TRUE(IDNToASCII(str.data(), str.length(), &output));
///  EXPECT_EQ("xn--14-c6t", std::string(output.data()));
///  output.set_length(0);
///
...
TEST(URLCanonTest, URLSafetyStatus) {
  ...
  for (const auto& test_case : kTestCases) {
    ...
    // Test with UTF-16.
    std::u16string utf16 = cr::UTF8ToUTF16(test_case.host);
    EXPECT_EQ(test_case.expected_safety_status,
              CheckHostnameSafety(utf16.c_str(), Component(0, utf16.size())));

    // Test with UTF-16 and terminal dot.
    std::u16string utf16_with_dot = cr::UTF8ToUTF16(host_with_dot);
    EXPECT_EQ(test_case.expected_safety_status,
    ...
    std::u16string utf16_with_bonus_characters =
        cr::UTF8ToUTF16(host_with_bonus_characters);
    EXPECT_EQ(test_case.expected_safety_status,
