...
TEST_F(URLUtilTest, AddReferrerScheme) {
  ...
  crurl::ScopedSchemeRegistryForTests scoped_registry;
  AddReferrerScheme(kFooScheme, crurl::SCHEME_WITH_HOST);
...
TEST_F(URLUtilTest, GetStandardSchemeType) {
  ...
  const char kHTTPScheme[] = "http";
  scheme_type = crurl::SCHEME_WITHOUT_AUTHORITY;
  EXPECT_TRUE(GetStandardSchemeType(kHTTPScheme,
                                    Component(0, strlen(kHTTPScheme)),
                                    &scheme_type));
  EXPECT_EQ(crurl::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, scheme_type);

  const char kFilesystemScheme[] = "filesystem";
  scheme_type = crurl::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION;
  EXPECT_TRUE(GetStandardSchemeType(kFilesystemScheme,
                                    Component(0, strlen(kFilesystemScheme)),
                                    &scheme_type));
  EXPECT_EQ(crurl::SCHEME_WITHOUT_AUTHORITY, scheme_type);
...
TEST_F(URLUtilTest, GetStandardSchemes) {
  ...
  AddStandardScheme("foo", crurl::SCHEME_WITHOUT_AUTHORITY);
...
TEST_F(URLUtilTest, DecodeURLEscapeSequences) {
  ...
  for (size_t i = 0; i < cr::size(decode_cases); i++) {
    ...
    EXPECT_EQ(decode_cases[i].output, cr::UTF16ToUTF8(std::u16string(
  ...
  DecodeURLEscapeSequences(zero_input, strlen(zero_input), DecodeURLMode::kUTF8,
                           &zero_output);
  EXPECT_NE("%00", cr::UTF16ToUTF8(std::u16string(zero_output.data(),
                                                  zero_output.length())));
...
TEST_F(URLUtilTest, TestEncodeURIComponent) {
  ...
  for (size_t i = 0; i < cr::size(encode_cases); i++) {
...
TEST_F(URLUtilTest, TestResolveRelativeWithNonStandardBase) {
  ...
  for (size_t i = 0; i < cr::size(resolve_non_standard_cases); i++) {
