...
#include "crurl_tests/gurl_abstract_tests.h"
...
TEST(GURLTest, IsValid) {
  ...
  for (size_t i = 0; i < cr::size(valid_cases); i++) {
  ...
  for (size_t i = 0; i < cr::size(invalid_cases); i++) {
...
TEST(GURLTest, ComponentGettersWorkEvenForInvalidURL) {
  const struct InvalidURLTestExpectations {
    const char* url;
    const char* spec;
    const char* scheme;
    const char* host;
...
TEST(GURLTest, Resolve) {
  ...
  for (size_t i = 0; i < cr::size(resolve_cases); i++) {
    // 8-bit code path.
    ...
    // Wide code path.
    GURL inputw(cr::UTF8ToUTF16(resolve_cases[i].base));
    GURL outputw =
        input.Resolve(cr::UTF8ToUTF16(resolve_cases[i].relative));
...
TEST(GURLTest, EffectiveIntPort) {
  ...
  for (size_t i = 0; i < cr::size(port_tests); i++) {
...
TEST(GURLTest, IPAddress) {
  ...
  for (size_t i = 0; i < cr::size(ip_tests); i++) {
...
TEST(GURLTest, InvalidHost) {
  ...
  EXPECT_FALSE(url.is_valid());
  EXPECT_TRUE(url.SchemeIs(crurl::kHttpScheme));
...
TEST(GURLTest, PortZero) {
  ...
  EXPECT_EQ(port_zero_url.port(),
            cr::NumberToString(scheme_host_port.port()));
  ...
  crurl::Origin another_origin = 
      crurl::Origin::Create(GURL("http://other.com"));
  crurl::Origin resolved_origin =
      crurl::Origin::Resolve(port_zero_url, another_origin);
  ...
  EXPECT_EQ(port_zero_url.port(), cr::NumberToString(resolved_origin.port()));
  ...
  crurl::Origin default_port_origin = crurl::Origin::Create(default_port);
...
class GURLTestTraits {
 public:
  ...
  static UrlType CreateUrlFromString(cr::StringPiece s) { return GURL(s); }
