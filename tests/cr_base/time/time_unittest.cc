...
TEST(TimeTestOutOfBounds, FromExplodedOutOfBoundsTime) {
  ...
  for (const auto& test : kDateTestData) {
    ...
    cr::Time result;
    EXPECT_FALSE(cr::Time::FromUTCExploded(test.explode, &result));
    EXPECT_TRUE(result.is_null());
    EXPECT_FALSE(cr::Time::FromLocalExploded(test.explode, &result));
    EXPECT_TRUE(result.is_null());
  }
}
...
TEST_F(TimeTest, ParseTimeTest1) {
  ...
  asctime_s(time_buf, cr::size(time_buf), &local_time);
#elif defined(MINI_CHROMIUM_OS_POSIX)
...
TEST_F(TimeTest, ParseTimeDeltaTest) {
  ...
  EXPECT_EQ(TimeDelta::FromString("9323372036854775807us"), cr::nullopt);
...
TEST(TimeTicks, Deltas) {
  for (int index = 0; index < 50; index++) {
    ...
    cr::PlatformThread::Sleep(cr::TimeDelta::FromMilliseconds(10));
...
TEST(TimeTicks, SnappedToNextTickBasic) {
  cr::TimeTicks phase = cr::TimeTicks::FromInternalValue(4000);
  cr::TimeDelta interval = cr::TimeDelta::FromMicroseconds(1000);
  cr::TimeTicks timestamp;

  // Timestamp in previous interval.
  timestamp = cr::TimeTicks::FromInternalValue(3500);
  EXPECT_EQ(4000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());

  // Timestamp in next interval.
  timestamp = cr::TimeTicks::FromInternalValue(4500);
  EXPECT_EQ(5000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());

  // Timestamp multiple intervals before.
  timestamp = cr::TimeTicks::FromInternalValue(2500);
  EXPECT_EQ(3000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());

  // Timestamp multiple intervals after.
  timestamp = cr::TimeTicks::FromInternalValue(6500);
  EXPECT_EQ(7000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());

  // Timestamp on previous interval.
  timestamp = cr::TimeTicks::FromInternalValue(3000);
  EXPECT_EQ(3000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());

  // Timestamp on next interval.
  timestamp = cr::TimeTicks::FromInternalValue(5000);
  EXPECT_EQ(5000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());

  // Timestamp equal to phase.
  timestamp = cr::TimeTicks::FromInternalValue(4000);
  EXPECT_EQ(4000,
            timestamp.SnappedToNextTick(phase, interval).ToInternalValue());
}
...
TEST(TimeTicks, SnappedToNextTickOverflow) {
  ...
  cr::TimeTicks phase = cr::TimeTicks::FromInternalValue(0);
  cr::TimeDelta interval = cr::TimeDelta::FromMicroseconds(4000);
  cr::TimeTicks big_timestamp =
      cr::TimeTicks::FromInternalValue(8635916564000);
...
TEST(TimeDelta, Hz) {
  ...
  EXPECT_EQ(cr::ClampRound(TimeDelta::FromHz(60).ToHz()), 60);
}
...
TEST(TimeTicksLogging, FortyYearsLater) {
  TimeTicks forty_years_later =
      TimeTicks() + TimeDelta::FromDays(static_cast<int>(365.25 * 40));
