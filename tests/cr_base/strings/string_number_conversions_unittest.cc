...
TEST(StringNumberConversionsTest, NumberToString) {
  ...
  for (const auto& test : int_tests) {
    ...
    EXPECT_EQ(NumberToWString(test.num), UTF8ToWide(test.sexpected));
    ...
    EXPECT_EQ(NumberToWString(static_cast<unsigned>(test.num)),
              UTF8ToWide(test.uexpected));
  }
  for (const auto& test : int64_tests) {
    ...
    EXPECT_EQ(NumberToString16(static_cast<uint64_t>(test.num)),
              UTF8ToUTF16(test.uexpected));
    EXPECT_EQ(NumberToWString(static_cast<uint64_t>(test.num)),
              UTF8ToWide(test.uexpected));
  }
}
...
TEST(StringNumberConversionsTest, StringToInt) {
  ...
  for (const auto& i : cases) {
    ...
    std::wstring wide_input = UTF8ToWide(i.input);
    output = i.output ^ 1;  // Ensure StringToInt wrote something.
    EXPECT_EQ(i.success, StringToInt(wide_input, &output));
    EXPECT_EQ(i.output, output);
  }
  ...
  std::string input_string(input, cr::size(input) - 1);
  int output;
  ...
  std::wstring wide_input = UTF8ToWide(input_string);
  output = 0;
  EXPECT_FALSE(StringToInt(wide_input, &output));
  EXPECT_EQ(6, output);

  output = 0;
  const char16_t negative_u16_input[] = {0xFF4D, '4', '2', 0};
  EXPECT_FALSE(StringToInt(std::u16string(negative_u16_input), &output));
  EXPECT_EQ(0, output);
}
...
TEST(StringNumberConversionsTest, StringToUint) {
  ...
  for (const auto& i : cases) {
    ...
    std::wstring wide_input = UTF8ToWide(i.input);
    output = i.output ^ 1;  // Ensure StringToUint wrote something.
    EXPECT_EQ(i.success, StringToUint(wide_input, &output));
    EXPECT_EQ(i.output, output);
  }
  ...
  std::wstring wide_input = UTF8ToWide(input_string);
  output = 0;
  EXPECT_FALSE(StringToUint(wide_input, &output));
  EXPECT_EQ(6U, output);

  output = 0;
  const char16_t negative_u16_input[] = {0xFF4D, '4', '2', 0};
  EXPECT_FALSE(StringToUint(std::u16string(negative_u16_input), &output));
  EXPECT_EQ(0U, output);
}
...
TEST(StringNumberConversionsTest, StringToInt64) {
  ...
  for (const auto& i : cases) {
    ...
    std::wstring wide_input = UTF8ToWide(i.input);
    output = 0;
    EXPECT_EQ(i.success, StringToInt64(wide_input, &output));
    EXPECT_EQ(i.output, output);
  }
...
TEST(StringNumberConversionsTest, StringToUint64) {
  static const struct {
    ...
    bool success;
      ...
      {"9223372036854775807", 
          static_cast<uint64_t>(std::numeric_limits<int64_t>::max()), true},
      {"-9223372036854775808", 0ull, false},
      {"09", 9, true},
      {"-09", 0, false},
      {"", 0, false},
      {" 42", 42, false},
  ...
  for (const auto& i : cases) {
    ...
    std::wstring wide_input = UTF8ToWide(i.input);
    output = 0;
    EXPECT_EQ(i.success, StringToUint64(wide_input, &output));
    EXPECT_EQ(i.output, output);
  }
  ...
  std::wstring wide_input = UTF8ToWide(input_string);
  output = 0;
  EXPECT_FALSE(StringToUint64(wide_input, &output));
  EXPECT_EQ(6U, output);
}
...
TEST(StringNumberConversionsTest, StringToSizeT) {
  ...
  for (const auto& i : cases) {
    ...
    std::wstring wide_input = UTF8ToWide(i.input);
    output = 0;
    EXPECT_EQ(i.success, StringToSizeT(wide_input, &output));
    EXPECT_EQ(i.output, output);
  }
  ...
  std::wstring wide_input = UTF8ToWide(input_string);
  output = 0;
  EXPECT_FALSE(StringToSizeT(wide_input, &output));
  EXPECT_EQ(6U, output);
}
...
TEST(StringNumberConversionsTest, HexStringToBytesStringSpan) {
  ...
  for (size_t test_i = 0; test_i < cr::size(cases); ++test_i) {
...
TEST(StringNumberConversionsTest, DoubleToString) {
  ...
  double input = 0;
  memcpy(&input, input_bytes, cr::size(input_bytes));
  EXPECT_EQ("1.335179083776e+12", NumberToString(input));
  const char input_bytes2[8] = {0,      0,      0,      '\xa0',
                                '\xda', '\x6c', '\x73', '\x42'};
  input = 0;
  memcpy(&input, input_bytes2, cr::size(input_bytes2));
