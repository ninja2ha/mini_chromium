...
#include "cr_base/strings/char_traits.h"
...
TEST(UTFStringConversionsTest, ConvertUTF16ToUTF8) {
  struct UTF16ToUTF8Case {
    ...
    {u"\x4f60\x597d", "\xe4\xbd\xa0\xe5\xa5\xbd", true},
    // Test a non-BMP character.
    {u"\xd800\xdf00", "\xF0\x90\x8C\x80", true},
    // Non-characters are passed through.
    {u"\xffffHello", "\xEF\xBF\xBFHello", true},
    {u"\xdbff\xdffeHello", "\xF4\x8F\xBF\xBEHello", true},
    // The first character is a truncated UTF-16 character.
    {u"\xd800\x597d", "\xef\xbf\xbd\xe5\xa5\xbd", false},
    // Truncated at the end.
    {u"\x597d\xd800", "\xe5\xa5\xbd\xef\xbf\xbd", false},
  };

  for (const auto& test : convert_cases) {
    std::string converted;
    EXPECT_EQ(test.success,
              UTF16ToUTF8(test.utf16, 
                          CharTraits<char16_t>::length(test.utf16), 
                          &converted));
...
TEST(UTFStringConversionsTest, ConvertUTF32ToUTF8) {
  struct UTF32ToUTF8Case {
    ...
    {U"\x4f60\x597d", "\xe4\xbd\xa0\xe5\xa5\xbd", true},
    // Test a non-BMP character.
    {U"A\x10300z", "A\xF0\x90\x8C\x80z", true},
    // Non-characters are passed through.
    {U"\xffffHello", "\xEF\xBF\xBFHello", true},
    {U"\x10fffeHello", "\xF4\x8F\xBF\xBEHello", true},
    // Invalid Unicode code points.
    {U"\xfffffffHello", "\xEF\xBF\xBDHello", false},
    // The first character is a truncated UTF-16 character.
    {U"\xd800\x597d", "\xef\xbf\xbd\xe5\xa5\xbd", false},
    {U"\xdc01Hello", "\xef\xbf\xbdHello", false},
  };

  for (const auto& test : convert_cases) {
    std::string converted;
    EXPECT_EQ(test.success,
              UTF32ToUTF8(test.utf32, CharTraits<char32_t>::length(test.utf32), 
                          &converted));
    std::string expected(test.utf8);
    EXPECT_EQ(expected, converted);
  }
...
TEST(UTFStringConversionsTest, ConvertMultiString) {
  ...
  memcpy(WriteInto(&multistring16, cr::size(multi16)), multi16,
         sizeof(multi16));
  EXPECT_EQ(cr::size(multi16) - 1, multistring16.length());
  std::string expected;
  memcpy(WriteInto(&expected, cr::size(multi)), multi, sizeof(multi));
  EXPECT_EQ(cr::size(multi) - 1, expected.length());
  const std::string& converted = UTF16ToUTF8(multistring16);
  EXPECT_EQ(cr::size(multi) - 1, converted.length());
  EXPECT_EQ(expected, converted);
}
