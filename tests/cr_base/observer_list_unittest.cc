...
template <class ObserverListType>
class ObserverListCreator {
 public:
  std::unique_ptr<ObserverListType> Create(
      cr::Optional<cr::ObserverListPolicy> policy = nullopt) {
    policy_ = policy;
    std::thread thread(&ObserverListCreator::Run, this);
    thread.join();
 ...
 private:
  ...
  cr::Optional<cr::ObserverListPolicy> policy_;
};
...
TYPED_TEST(ObserverListTest, CreatedAndUsedOnDifferentThreads) {
  ...
  // Check with constructor taking explicit policy
  {
    std::unique_ptr<ObserverListFoo> observer_list =
        list_creator.Create(cr::ObserverListPolicy::EXISTING_ONLY);
    observer_list->AddObserver(&a);
    for (auto& observer : *observer_list) {
      observer.Observe(1);
    }
...
class MockLogAssertHandler {
 public:
  MOCK_METHOD4(
      HandleLogAssert,
      void(const char*, int, const cr::StringPiece, const cr::StringPiece));
};
