...
TEST(VersionTest, MoveSemantics) {
  ...
  cr::Version v1(std::move(components));
  EXPECT_TRUE(v1.IsValid());
  cr::Version v2("1.2.3.4");
  EXPECT_EQ(v1, v2);
}
...
TEST(VersionTest, Compare) {
  ...
  for (const auto& i : cases) {
    cr::Version lhs(i.lhs);
    cr::Version rhs(i.rhs);
...
TEST(VersionTest, CompareToWildcardString) {
  ...
  for (const auto& i : cases) {
    const cr::Version version(i.lhs);
...
TEST(VersionTest, IsValidWildcardString) {
  ...
  for (const auto& i : cases) {
    EXPECT_EQ(cr::Version::IsValidWildcardString(i.version), i.expected)
...
TEST(VersionTest, LeadingZeros) {
  ...
  EXPECT_GT(cr::Version("1.02"), cr::Version("1.1"));
}
