...
#include "crbase_tests/test/bind.h"
...
int FunctionWithScopedRefptrFirstParam(const RefPtr<HasRef>& o, int n) {
  return n;
}
...
TEST_F(BindTest, BindOnceWithNonConstRef) {
  ...
  auto cb3 = BindOnce(SetFromRef, cr::OwnedRef(v));
  std::move(cb3).Run();
  EXPECT_EQ(v, 1);

  int& ref = v;
  auto cb4 = BindOnce(SetFromRef, cr::OwnedRef(ref));
  std::move(cb4).Run();
  EXPECT_EQ(v, 1);

  const int cv = 1;
  auto cb5 = BindOnce(SetFromRef, cr::OwnedRef(cv));
  std::move(cb5).Run();
  EXPECT_EQ(cv, 1);

  const int& cref = v;
  auto cb6 = BindOnce(SetFromRef, cr::OwnedRef(cref));
  std::move(cb6).Run();
  EXPECT_EQ(cref, 1);

  auto cb7 = BindOnce(SetFromRef, cr::OwnedRef(1));
  std::move(cb7).Run();
}
...
TEST_F(BindTest, BindRepeatingWithNonConstRef) {
  ...
  // Everything past here following will make a copy of the argument. The copy
  // will be mutated and leave `v` unmodified.
  auto cb3 = BindRepeating(SetFromRef, cr::OwnedRef(v));
  std::move(cb3).Run();
  EXPECT_EQ(v, 1);

  int& ref = v;
  auto cb4 = BindRepeating(SetFromRef, cr::OwnedRef(ref));
  std::move(cb4).Run();
  EXPECT_EQ(v, 1);

  const int cv = 1;
  auto cb5 = BindRepeating(SetFromRef, cr::OwnedRef(cv));
  std::move(cb5).Run();
  EXPECT_EQ(cv, 1);

  const int& cref = v;
  auto cb6 = BindRepeating(SetFromRef, cr::OwnedRef(cref));
  std::move(cb6).Run();
  EXPECT_EQ(cref, 1);

  auto cb7 = BindRepeating(SetFromRef, cr::OwnedRef(1));
  std::move(cb7).Run();
}
...
TYPED_TEST(BindVariantsTest, ScopedRefptr) {
  ...
  const RefPtr<HasRef> refptr(&has_ref);
...
TEST_F(BindTest, BindMoveOnlyVector) {
  ...
  cr::RepeatingCallback<MoveOnlyVector()> bound_cb =
      cr::BindRepeating(&AcceptAndReturnMoveOnlyVector, Passed(&v));
  ...
  cr::RepeatingCallback<MoveOnlyVector(MoveOnlyVector)> unbound_cb =
      cr::BindRepeating(&AcceptAndReturnMoveOnlyVector);
...
TEST_F(BindTest, ArgumentCopies) {
  ...
  assigns = 0;
  BindRepeating(
      &VoidPolymorphic<CopyCounter>::Run).Run(CopyCounter(&copies, &assigns));
  EXPECT_EQ(1, copies);
  EXPECT_EQ(0, assigns);
...
TEST_F(BindTest, ArgumentCopiesAndMoves) {
  ...
  BindRepeating(&VoidPolymorphic<CopyMoveCounter>::Run,
                CopyMoveCounter(&copies, &assigns, &move_constructs, 
                &move_assigns));
  EXPECT_EQ(0, copies);
  EXPECT_EQ(0, assigns);
  EXPECT_EQ(1, move_constructs);
...
TEST_F(BindTest, CapturelessLambda) {
  ...
  int x = 1;
  cr::RepeatingCallback<void(int)> cb =
...
TEST_F(BindTest, WindowsCallingConventions) {
  ...
  auto stdcall_method_cb =
      BindRepeating(&MethodHolder::Func, cr::Unretained(&obj), 1);
  EXPECT_EQ(1, stdcall_method_cb.Run());

  const MethodHolder const_obj;
  auto stdcall_const_method_cb =
      BindRepeating(&MethodHolder::ConstFunc, cr::Unretained(&const_obj), 1);
...
TEST_F(BindTest, BindNoexcept) {
  EXPECT_EQ(42, cr::BindOnce(&Noexcept).Run());
  EXPECT_EQ(
      42,
      cr::BindOnce(&BindTest::NoexceptMethod, cr::Unretained(this)).Run());
  EXPECT_EQ(
      42, cr::BindOnce(&BindTest::ConstNoexceptMethod, cr::Unretained(this))
              .Run());
}

// Test null callbacks cause a DCHECK.
TEST(BindDeathTest, NullCallback) {
  cr::RepeatingCallback<void(int)> null_cb;
  ASSERT_TRUE(null_cb.is_null());
  EXPECT_CHECK_DEATH(cr::BindRepeating(null_cb, 42));
}

TEST(BindDeathTest, NullFunctionPointer) {
  void (*null_function)(int) = nullptr;
  EXPECT_DCHECK_DEATH(cr::BindRepeating(null_function, 42));
}

TEST(BindDeathTest, NullCallbackWithoutBoundArgs) {
  cr::OnceCallback<void(int)> null_cb;
  ASSERT_TRUE(null_cb.is_null());
  EXPECT_CHECK_DEATH(cr::BindOnce(std::move(null_cb)));
}
...
TEST(BindDeathTest, BanFirstOwnerOfRefCountedType) {
  ...
  EXPECT_DCHECK_DEATH({
    ...
    cr::BindOnce(&HasRef::VoidMethod0, &has_ref);
  });
}
