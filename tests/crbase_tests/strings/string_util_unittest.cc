...
TEST(StringUtilTest, TruncateUTF8ToByteSize) {
...

  {
    ...
    const std::string array_string(array, cr::size(array));
...
TEST(StringUtilTest, IsStringASCII) {
  ...
  {
    const size_t string_length = cr::size(char_ascii) - 1;
  ...
  {
    const size_t string_length = cr::size(char16_ascii) - 1;
...
#endif  // MINI_CHROMIUM_WCHAR_T_IS_UTF32
}
...
TEST(StringUtilTest, ConvertASCII) {
  ...
  for (size_t i = 0; i < cr::size(char_cases); ++i) {
  ...
  const size_t length_with_nul = cr::size(chars_with_nul) - 1;
...
TEST(StringUtilTest, FormatBytesUnlocalized) {
  ...
  for (const auto& i : cases) {
    EXPECT_EQ(i.expected, FormatBytesUnlocalized(i.bytes));
  }
}
...
TEST(StringUtilTest, StartsWith) {
  ...
  EXPECT_TRUE(StartsWith("JavaScript:url", "javascript",
                         cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_FALSE(StartsWith("java", "javascript", cr::CompareCase::SENSITIVE));
  ...
  EXPECT_TRUE(StartsWith("java", std::string(),
                         cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_TRUE(StartsWith("java", std::string(), cr::CompareCase::SENSITIVE));
  ...
  EXPECT_TRUE(StartsWith(u"JavaScript:url", u"javascript",
                         cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_FALSE(
      StartsWith(u"java", u"javascript", cr::CompareCase::SENSITIVE));
  EXPECT_FALSE(
      StartsWith(u"java", u"javascript", cr::CompareCase::INSENSITIVE_ASCII));
  ...
  EXPECT_TRUE(StartsWith(u"java", std::u16string(),
                         cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_TRUE(
      StartsWith(u"java", std::u16string(), cr::CompareCase::SENSITIVE));
}

TEST(StringUtilTest, EndsWith) {
  EXPECT_TRUE(
      EndsWith(u"Foo.plugin", u".plugin", cr::CompareCase::SENSITIVE));
  EXPECT_FALSE(
      EndsWith(u"Foo.Plugin", u".plugin", cr::CompareCase::SENSITIVE));
  ...
  EXPECT_TRUE(EndsWith(u"Foo.Plugin", u".plugin",
                       cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_FALSE(EndsWith(u".plug", u".plugin", cr::CompareCase::SENSITIVE));
  EXPECT_FALSE(
      EndsWith(u".plug", u".plugin", cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_FALSE(
      EndsWith(u"Foo.plugin Bar", u".plugin", cr::CompareCase::SENSITIVE));
  EXPECT_FALSE(EndsWith(u"Foo.plugin Bar", u".plugin",
                        cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_FALSE(EndsWith(std::u16string(), u".plugin",
                        cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_FALSE(
      EndsWith(std::u16string(), u".plugin", cr::CompareCase::SENSITIVE));
  EXPECT_TRUE(EndsWith(u"Foo.plugin", std::u16string(),
                       cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_TRUE(
      EndsWith(u"Foo.plugin", std::u16string(), cr::CompareCase::SENSITIVE));
  EXPECT_TRUE(
      EndsWith(u".plugin", u".plugin", cr::CompareCase::INSENSITIVE_ASCII));
  EXPECT_TRUE(EndsWith(u".plugin", u".plugin", cr::CompareCase::SENSITIVE));
...
TEST(StringUtilTest, LcpyTest) {
  // Test the normal case where we fit in our buffer.
  {
    char dst[10];
    wchar_t wdst[10];
    EXPECT_EQ(7U, strlcpy(dst, "abcdefg", cr::size(dst)));
    EXPECT_EQ(0, memcmp(dst, "abcdefg", 8));
    EXPECT_EQ(7U, wcslcpy(wdst, L"abcdefg", cr::size(wdst)));
...
class WriteIntoTest : public testing::Test {
 protected:
  static void WritesCorrectly(size_t num_chars) {
    ...
    EXPECT_EQ(
        std::string(kOriginal, std::min(num_chars, cr::size(kOriginal) - 1)),
